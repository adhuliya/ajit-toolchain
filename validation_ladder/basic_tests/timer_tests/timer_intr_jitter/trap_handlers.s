...
timer_isr_label:
	...
	sta  %l6, [ %l7 ] 0x20		!ffff3000 ADDR_INTERRUPT_CONTROL_REGISTER; IRC=0; ASI_MMU_REGISTER=0x20 for ASI_SCRATCHPAD
	sethi  %hi(0xffff3000), %l6
	lda  [ %l6 ] 0x20 , %l7		!Read IRC, (to create extra delay);ASI_MMU_REGISTER=0x20 for ASI_SCRATCHPAD

	clr  %l6
	sethi  %hi(0xffff3000), %l7
	or  %l7, 0x100, %l7		!ffff3100 ADDR_TIMER_CONTROL_REGISTER
	sta  %l6, [ %l7 ] 0x20 		!clear TIMER_CONTROL_REGISTER; ASI_MMU_REGISTER=0x20 for ASI_SCRATCHPAD

	call  set_intr_flag	
	nop 

	mov %l0,%psr			!Recover %psr
	jmpl %r17, %r0 ! old PC
	rett %r18 ! old nPC
	nop
