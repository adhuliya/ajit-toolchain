...
void fake_fflush (FILE* fp) {fflush(fp);}
...
*/
	...
	if (UfNGrad) {
		...
		if (! ((X == Z) || (X - Z != Zero))) {
			...
			sigsave = sigfpe;
			if (fake_setjmp(ovfl_buf)) printf("X / Z fails!\n");
			else printf("X / Z = 1 + %g .\n", (X / Z - Half) - Half);
			sigsave = 0;
			}
		}
...
*/
	...
	printf("CInvrse=%.17e, Hinvrse=%.17e\n", CInvrse, HInvrse);
	...
	do {
		...
		Y = V9;
		V9 = HInvrse * Y;
		printf("INFO:OVFLOW:  V9=%.17e (0x%llx)\n", V9, *((uint64_t*) &V9));
		printf("INFO:OVFLOW:  Y=%.17e (0x%llx)\n",  Y, *((uint64_t*) &Y));
		int  condition = (V9 < Y);
		printf("INFO:OVFLOW:  (V9 < Y)=%d\n", condition);
		} while(V9 < Y);
	I = 1;
overflow:
	sigsave = 0;
...
*/
	...
	if ((ch[0] == 'Y') || (ch[0] == 'y')) {
		...
		if (!fake_setjmp(ovfl_buf)) printf("  %.7e .\n", One / MyZero);
		sigsave = 0;
#ifndef NOPAUSE
		}
	else printf("O.K.\n");
