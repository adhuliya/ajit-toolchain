...
void matrix_mul_vect(ee_u32 N, MATRES *C, MATDAT *A, MATDAT *B) {
	...
	MATRES X=0;
	for (i=0; i<N; i++) {
		for (j=0, X=0; j<N1; j += 4) {
			X+=(MATRES)A[i*N+j] * (MATRES)B[j];
			X+=(MATRES)A[i*N+j+1] * (MATRES)B[j+1];
			X+=(MATRES)A[i*N+j+2] * (MATRES)B[j+2];
			X+=(MATRES)A[i*N+j+3] * (MATRES)B[j+3];
		}
...
void matrix_mul_matrix(ee_u32 N, MATRES *C, MATDAT *A, MATDAT *B) {
	...
	for (i=0; i<N; i++) {
		for (j=0; j<N; j++) {
			...
			for(k=0;k<N1; k += 4)
			{
				X += (MATRES)A[i*N+k] * (MATRES)B[k*N+j];
				X += (MATRES)A[i*N+(k+1)] * (MATRES)B[(k+1)*N+j];
				X += (MATRES)A[i*N+(k+2)] * (MATRES)B[(k+2)*N+j];
				X += (MATRES)A[i*N+(k+3)] * (MATRES)B[(k+3)*N+j];
			}
...
void matrix_mul_matrix_bitextract(ee_u32 N, MATRES *C, MATDAT *A, MATDAT *B) {
	...
	for (i=0; i<N; i++) {
		for (j=0; j<N; j++) {
			C[i*N+j]=0;
			for(k=0, X=0;k<N1;k+= 4)
			{
				MATRES tmp=(MATRES)A[i*N+k] * (MATRES)B[k*N+j];
				X += bit_extract(tmp,2,4)*bit_extract(tmp,5,7);

				tmp=(MATRES)A[i*N+(k+1)] * (MATRES)B[(k+1)*N+j];
				X += bit_extract(tmp,2,4)*bit_extract(tmp,5,7);

				tmp=(MATRES)A[i*N+(k+2)] * (MATRES)B[(k+2)*N+j];
				X += bit_extract(tmp,2,4)*bit_extract(tmp,5,7);

				tmp=(MATRES)A[i*N+(k+3)] * (MATRES)B[(k+3)*N+j];
				X += bit_extract(tmp,2,4)*bit_extract(tmp,5,7);
			}
