...
# define LDA (N)
  ...
  //timestamp ( );
  cortos_printf ( "\n" );
  cortos_printf ( "LINPACK_BENCH\n" );
  cortos_printf ( "  C version\n" );
  cortos_printf ( "\n" );
  cortos_printf ( "  The LINPACK benchmark.\n" );
  cortos_printf ( "  Language: C\n" );
  cortos_printf ( "  Datatype: Double precision real\n" );
  cortos_printf ( "  Matrix order N               = %d\n", N );
  cortos_printf ( "  Leading matrix dimension LDA = %d\n", LDA );
...
*/
  ...
  b = ( double * ) bget ( N * sizeof ( double ) );
  ipvt = ( int * ) bget ( N * sizeof ( int ) );
  resid = ( double * ) bget ( N * sizeof ( double ) );
  rhs = ( double * ) bget ( N * sizeof ( double ) );
  x = ( double * ) bget ( N * sizeof ( double ) );
  a_max = 0.0;
...
*/
  ...
  {
    cortos_printf ( "\n" );
    cortos_printf ( "LINPACK_BENCH - Fatal error!\n" );
    cortos_printf ( "  The matrix A is apparently singular.\n" );
    cortos_printf ( "  Abnormal end of execution.\n" );
    return 1;
  }
cortos_printf ( " matrix is non-singular \n");
  time[0] = t2 - t1;
...
*/
  ...
  cortos_printf ( "\n" );
  cortos_printf ( "  %9f  %9f  %9f  %9f  %9f  %9f\n", 
...
*/
  cortos_printf ( "\n" );
  cortos_printf ( "LINPACK_BENCH\n" );
  cortos_printf ( "  Normal end of execution.\n" );
...
void callfn(void* varg)
{
	CORTOS_DEBUG("entering callfn\n");
        ...
        adaxpy (arg->num,arg->da,arg->dx, arg->incx,arg->dy, arg->incy);
...
*/
{
  ...
  a = ( double * ) bget ( lda * n * sizeof ( double ) );
