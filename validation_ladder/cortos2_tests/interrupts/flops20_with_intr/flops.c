...
int dtime(double p[])
{
 uint32_t uq = barebones_clock();
 double q = p[2];
 p[2] = ((double) uq)*(1.0/((double)EE_TICKS_PER_SEC));
 p[1] = p[2] - q;
 return 0;
}
#endif
