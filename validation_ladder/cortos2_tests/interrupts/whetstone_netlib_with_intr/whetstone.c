...
double dtime ()
{
	uint64_t t = __ajit_get_clock_time();
	double ret_val = ((double) t) / CLK_FREQUENCY;
	return(ret_val);
}
...
main(int argc, char *argv[])
{
	...
	loopstart  = D_LOOP_COUNT;		/* see the note about LOOP below */
	continuous = D_CONTINUOUS;
	...
	while(timer_interrupt_counter < 1024)
	{
	}
	cortos_printf("Starting at interrupt-counter %d.\n", timer_interrupt_counter);
LCONT:
/*
C
...
*/
	PRINTF("\n");
	if (finisec-startsec < 0.0) {
		PRINTF("Insufficient duration (%f)- Increase the LOOP count\n", finisec-startsec);
		return(1);
	}
...
{
	PRINTF("%7ld %7ld %7ld %18.8f %18.8f %18.8f %18.8f\n",
						N, J, K, X1, X2, X3, X4);
}
#endif
