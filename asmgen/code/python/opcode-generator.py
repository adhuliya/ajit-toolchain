...
def setup_globals():
    ...
    global insn_asm_args_notes_at # Added to improve the syntax of generated ASM
    ...
    global sparc_fpregs_single_precision
    global sparc_fpregs_double_precision
    global sparc_fpregs_quad_precision
    ...
    insn_asm_args_notes_at  = 16
    insn_asm_num_args_at    = 17
    insn_asm_arg_1_at       = 18
    insn_asm_arg_2_at       = 19
    insn_asm_arg_3_at       = 20
    insn_asm_arg_4_at       = 21
    insn_info_end_at        = 22
    insn_expanded_fields_at = 23
    ...
    sparc_fpregs_single_precision = list (filter (is_single_precision, list (range (MAX_SPARC_REGS))))
    # print ("xxx Single Precision Register Numbers: {l}".format (l=sparc_fpregs_single_precision))
    sparc_fpregs_double_precision = list (filter (is_double_precision, list (range (MAX_SPARC_REGS))))
    # print ("xxx Double Precision Register Numbers: {l}".format (l=sparc_fpregs_double_precision))
    sparc_fpregs_quad_precision   = list (filter (is_quad_precision, list (range (MAX_SPARC_REGS))))
    # print ("xxx Quad Precision Register Numbers: {l}".format (l=sparc_fpregs_quad_precision))
    fin_expans = []
    expanded_insns = []
...
def is_single_precision (x):
    return (x % 1) == 0

def is_double_precision (x):
    return (x % 2) == 0

def is_quad_precision (x):
    return (x % 4) == 0
...
def reorganise_formats_list (fl, hdr):
    ...
    num_bit_fields = int (fl[fmt_num_bit_fields_at])
...
def do_given (bitfld, insn, arglst):
    ...
    if (verbosity_level > 0):   # detail level: 4
       print ("=== {c:02d}/{mi:02d} {p:{w}s}{m:17s}: {l:032b} ({l1:d}, 0x{l2:08X})".
              format(c=call_depth, mi=maxindent, p=".", w=(3*5),
                     m="GIVEN value", l=useval, l1=useval, l2=useval))
...
def do_lookup (bitfld, insn, arglst):
    ...
    if (bf_name == "cond"):
        ...
        if (bf_name == insn[insn_format_type_at]):
            value_str_found = insn [insn_format_value_at]
        else:
            if (bf_name == insn[insn_other_field_at]):
                value_str_found = insn [insn_other_value_at]

    print ("=== {m:20s}: {v}".format (m="do_lookup(): Bit field", v=bitfld))
    print ("===    {m:20s}: {v}".format (m="Name",         v=bf_name))
    print ("===    {m:20s}: {v}".format (m="Start",        v=bf_start))
    print ("===    {m:20s}: {v}".format (m="End",          v=bf_end))
    print ("===    {m:20s}: {v}".format (m="Length/NBits", v=bf_nbits))
    ...
    print ("===    {m:20s}: {v}".format (m="Value String Found",   v=value_str_found))
    ...
    if (verbosity_level > 0):   # detail level: 4
       print ("=== {c:02d}/{mi:02d} {p:{w}s}{m:17s}: {l:032b} ({l1:d}, 0x{l2:08X})".
              format(c=call_depth, mi=maxindent, p=".", w=(3*5),
                     m="LOOKUP value", l=useval, l1=useval, l2=useval))
...
def do_generate (bitfld, insn, arglst):
    ...
    out_lst = []
    out_opc_lst = []
    arg_note = insn [insn_asm_args_notes_at]
    ...
    if (bf_nbits <= max_list_length_bits):
        ...
        step = 1
        if (bf_name == "rs1" or bf_name == "rs2" or bf_name == "rd"):
            if (arg_note == "single"):
                step = 1
            elif (arg_note == "double"):
                step = 2
            elif (arg_note == "quad"):
                step = 4
        # for i in range (num_vals):
        for i in range (0, num_vals, step):
            useval = (i << bf_end)
...
def do_range (bitfld, insn, arglst):
    ...
    useval = min
    useval = (useval << bf_end)
    # print ("===    {m:20s}: {v:032b} ({v1:d})".format (m="Min", v=min, v1=min))
    # print ("===    {m:20s}: {v:032b}".format (m="Min", v=useval))
    
    out_opc_lst.append (useval)
    
    useval = max - 1
    useval = (useval << bf_end)
    # print ("===    {m:20s}: {v:032b} ({v1:d})".format (m="Max", v=max, v1=max))
    # print ("===    {m:20s}: {v:032b}".format (m="Max", v=useval))
...
def get_regs_list_for_given_type (arg_type, arg_note):
    thereglst = []              # AMV

    if   (arg_type == "r"):
        thereglst = sparc_regs
    elif (arg_type == "f"):
        thereglst = sparc_fpregs # default value
        if (arg_note == "single"):
            thereglst = sparc_fpregs_single_precision
        elif (arg_note == "double"):
            thereglst = sparc_fpregs_double_precision
        elif (arg_note == "quad"):
            thereglst = sparc_fpregs_quad_precision
    elif (arg_type == "c"):
...
def analyse_args (bitfld, insn):
    ...
    # print ("===    {m:20s}: {v}".format (m="The INSN", v=pp.pformat (insn)))
    ...
    for i in range (int (num_args, 10)):
        if   (bf_name == theargs[i]):
            ...
            argval   = []
            typecode = arg_type [i]
            reglst   = get_regs_list_for_given_type (typecode, arg_note)
...
def combine_args (insn, arg1, arg2, arg3, arg4):
    newargslst = []
    argsnotes = insn [insn_asm_args_notes_at]
    insncategory = insn [insn_category_at]
    insncategoryindex = insn [insn_category_ind_at]

    if (verbosity_level > 1):   # Detail level: 2
        print ("XXX {m:25s}:".format (m="Entered combine_args()"))
        print ("XXX    {m:25s}: {v}".format (m="I/P Insn", v=pp.pformat (insn)))
        print ("XXX    {m:25s}: {v}".format (m="I/P Arg1", v=pp.pformat (arg1)))
        print ("XXX    {m:25s}: {v}".format (m="I/P Arg2", v=pp.pformat (arg2)))
        print ("XXX    {m:25s}: {v}".format (m="I/P Arg3", v=pp.pformat (arg3)))
        print ("XXX    {m:25s}: {v}".format (m="I/P Arg4", v=pp.pformat (arg4)))
        print ("XXX    {m:25s}: {v}".format (m="I/P Note", v=pp.pformat (argsnotes)))
        print ("XXX    {m:25s}: {v}".format (m="Insn Cat", v=pp.pformat (insncategory)))
        print ("XXX    {m:25s}: {v}".format (m="Insn#Cat", v=pp.pformat (insncategoryindex)))
        
    newargslst.append (arg1)
    newargslst.append (arg2)
    newargslst.append (arg3)
    newargslst.append (arg4)
    
    if (argsnotes == "[address]"     or
        argsnotes == "[regaddr]"):
        if (insncategory == "Store"  and # Exchange the arguments for the "Store"s.
            insncategoryindex == "1"):
            a1 = "[" + arg2 + "+" + arg3 + "]"
            newargslst[0] = arg1
            newargslst[1] = a1
            ...
            newargslst[3] = ""
            if (verbosity_level > 0):   # Detail level: 1
                print ("XXX [address/regaddr] Store family INSN: Arg1 : {v1}, Arg2: {v2}".
                       format (v1=newargslst[0], v2=newargslst[1]))
        else:
            a1 = "[" + arg1 + "+" + arg2 + "]"
            newargslst[0] = a1
            newargslst[1] = arg3
            ...
            newargslst[3] = ""
            if (verbosity_level > 0):   # Detail level: 1
                print ("XXX [address/regaddr] NOT Store family INSN: Arg1 : {v1}, Arg2: {v2}".
                       format (v1=newargslst[0], v2=newargslst[1]))
        if (verbosity_level > 3):   # Detail level: 4
            print ("===    {m:25s}: {v}".format (m="Form of Arg1", v=pp.pformat (a1)))
            print ("===    {m:25s}: {v}".format (m="[Address/ST] Arg1", v=pp.pformat (newargslst[0])))
            print ("===    {m:25s}: {v}".format (m="[Address/ST] Arg2", v=pp.pformat (newargslst[1])))
            print ("===    {m:25s}: {v}".format (m="[Address/ST] Arg3", v=pp.pformat (newargslst[2])))
            print ("===    {m:25s}: {v}".format (m="[Address/ST] Arg4", v=pp.pformat (newargslst[3])))

    if (argsnotes == "address"     or
        argsnotes == "software_trap"):
        if (insncategory == "Store"  and # Exchange the arguments for the "Store"s.
            insncategoryindex == "1"):
            a1 = arg2 + "+" + arg3
            newargslst[0] = arg1
            ...
            newargslst[3] = ""
            if (verbosity_level > 0):   # Detail level: 1
                print ("XXX address/software_trap Store family INSN: Arg1 : {v1}, Arg2: {v2}".
                       format (v1=newargslst[0], v2=newargslst[1]))
        else:
            a1 = arg1 + "+" + arg2
            newargslst[0] = a1
            ...
            newargslst[3] = ""
            if (verbosity_level > 0):   # Detail level: 1
                print ("XXX address/software_trap NOT Store family INSN: Arg1 : {v1}, Arg2: {v2}".
                       format (v1=newargslst[0], v2=newargslst[1]))
        if (verbosity_level > 3):   # Detail level: 4
            print ("===    {m:25s}: {v}".format (m="Form of Arg1", v=pp.pformat (a1)))
            print ("===    {m:25s}: {v}".format (m="Address Arg1", v=pp.pformat (newargslst[0])))
            print ("===    {m:25s}: {v}".format (m="Address Arg2", v=pp.pformat (newargslst[1])))
            print ("===    {m:25s}: {v}".format (m="Address Arg3", v=pp.pformat (newargslst[2])))
            print ("===    {m:25s}: {v}".format (m="Address Arg4", v=pp.pformat (newargslst[3])))
    ...
    #     newargslst[3] = ""
    #     if (verbosity_level > 0):   # Detail level: 1
    #         print ("XXX Write Ancillary Register INSN: Arg1 : {v1}, Arg2: {v2}".format (v1=newargslst[0], v2=newargslst[1]))
    ...
    if ( argsnotes == "[address] asi"     or
         argsnotes == "[regaddr] asi"):
        if (insncategory == "Store"  and # Exchange the arguments for the "Store"s.
            insncategoryindex == "1"):
            a1 = "[" + arg2 + "+" + arg3 + "]" + " " + str(int (arg4, base=16) >> 5)
            newargslst[0] = arg1
            newargslst[1] = a1
            newargslst[2] = ""
            newargslst[3] = ""
            if (verbosity_level > 0):   # Detail level: 1
                print ("XXX [address/regaddr] asi Store family INSN: Arg1 : {v1}, Arg2: {v2}".
                       format (v1=newargslst[0], v2=newargslst[1]))
        else:
            a1 = "[" + arg1 + "+" + arg2 + "]" + " " + str(int (arg4, base=16) >> 5)
            newargslst[0] = a1
            newargslst[1] = arg3
            newargslst[2] = ""
            newargslst[3] = ""
            if (verbosity_level > 0):   # Detail level: 1
                print ("XXX [address/regaddr] asi NOT Store family INSN: Arg1 : {v1}, Arg2: {v2}".
                       format (v1=newargslst[0], v2=newargslst[1]))
        if (verbosity_level > 3):   # Detail level: 4
            print ("===    {m:25s}: {v}".format (m="Form of Arg1", v=pp.pformat (a1)))
            print ("===    {m:25s}: {v}".format (m="[Address] asi Arg1", v=pp.pformat (newargslst[0])))
            print ("===    {m:25s}: {v}".format (m="[Address] asi Arg2", v=pp.pformat (newargslst[1])))
            print ("===    {m:25s}: {v}".format (m="[Address] asi Arg3", v=pp.pformat (newargslst[2])))
            print ("===    {m:25s}: {v}".format (m="[Address] asi Arg4", v=pp.pformat (newargslst[3])))

    # if (insncategory == "Store"  and # Exchange the arguments for the "Store"s.
    #     insncategoryindex == 1):
    #     tmp = newargslst[0]
    #     newargslst[0] = newargslst[1]
    #     newargslst[1] = tmp
    ...
    #         newargslst.append (arg4)
    #         newargslst.append ("")
    #         # print ("===    {m:25s}: {v}".format (m="Plain Arg1", v=pp.pformat (newargslst[0])))
    #         # print ("===    {m:25s}: {v}".format (m="Plain Arg2", v=pp.pformat (newargslst[1])))
    #         # print ("===    {m:25s}: {v}".format (m="Plain Arg3", v=pp.pformat (newargslst[2])))
    #         # print ("===    {m:25s}: {v}".format (m="Plain Arg4", v=pp.pformat (newargslst[3])))
    #     else:
    #         newargslst.append (arg1)
    #         newargslst.append (arg2)
    #         newargslst.append (arg3)
    ...
    if (verbosity_level > 3):   # Detail level: 4
        print ("===    {m:25s}: {v}".format (m="O/P Insn", v=pp.pformat (insn)))
        print ("===    {m:25s}: {v}".format (m="O/P Arg1", v=pp.pformat (newargslst[0])))
        print ("===    {m:25s}: {v}".format (m="O/P Arg2", v=pp.pformat (newargslst[1])))
        print ("===    {m:25s}: {v}".format (m="O/P Arg3", v=pp.pformat (newargslst[2])))
        print ("===    {m:25s}: {v}".format (m="O/P Arg4", v=pp.pformat (newargslst[3])))
        # print ("===    {m:25s}: {v}".format (m="O/P Arg1", v=pp.pformat (arg1)))
        # print ("===    {m:25s}: {v}".format (m="O/P Arg2", v=pp.pformat (arg2)))
        # print ("===    {m:25s}: {v}".format (m="O/P Arg3", v=pp.pformat (arg3)))
        # print ("===    {m:25s}: {v}".format (m="O/P Arg4", v=pp.pformat (arg4)))
        print ("===    {m:25s}: {v}".format (m="O/P NewArgs", v=pp.pformat (newargslst)))
        print ("=== {m:25s}:".format (m="Leaving combine_args()"))
    
    return newargslst

def iter_build_list_of_opcodes (insn, l, bf, res, rec):
    ...
    # regvalofarg  = -1
    # otherstr     = ""
    insn_opc_lst_at    = insn_fmt_index_at # = 10
    opc_val_lst_at     = 3
    ...
    # print ("=== {m:25s}:".format (m="Entering build_list_of_opcodes()"))
    
    # print ("===    {m:25s}: {v}".format (m="I/P Insn", v=pp.pformat (insn)))
    # print ("===    {m:25s}: {v}".format (m="I/P l",    v=pp.pformat (l)))
    # print ("===    {m:25s}: {v}".format (m="I/P bf/not needed",   v=pp.pformat (bf)))
    # print ("===    {m:25s}: {v}".format (m="I/P res",  v=pp.pformat (res)))
    # print ("===    {m:25s}: {v}".format (m="I/P rec",  v=pp.pformat (rec)))

    # print ("===    {m:25s}: {v}".format (m="Insn Opc", v=pp.pformat (insn_opc_lst)))
    # # List fields offsets: INSN list
    # insn_name_at            = 0
    # insn_format_type_at     = 1
    # insn_format_value_at    = 2
    ...
    # insn_info_end_at        = 22
    # insn_expanded_fields_at = 23
    # print ("===    {m:25s}: {v}".format (m="Insn #fields", v=pp.pformat (len (insn_opc_lst))))
    ...
    finalopc = 0                # Final opcode
    for nf in (range (len (insn_opc_lst))):
        fieldopc = 0            # initialise Op code part of this field
        outlst = insn_opc_lst [nf]
        bitlst = outlst [out_bitlst_out_opc_lst_at]
        # print ("===       {m:25s}: {v}".format (m="Insn bit field #", v=pp.pformat (nf)))
        # print ("===       {m:25s}: {v}".format (m="Insn bit field", v=pp.pformat (outlst)))
        # print ("===       Insn bit field [{i:d}]: {v}".format (i=nf, v=pp.pformat (outlst)))
        # print ("===       {m:25s}: {v}".format (m=pp.pformat("Insn bit field [{i:d}]".format (i=nf)), v=pp.pformat (outlst)))
        # print ("===       {m:25s}: {v}".format (m="Insn bit field [{i:2d}]".format (i=nf), v=pp.pformat (outlst)))
        # print ("===       {m:25s}: {v}".format (m="Insn bit field", v=pp.pformat (outlst)))
        # print ("===       {m:25s}: {v}".format (m="Bit field values list", v=pp.pformat (bitlst)))
        for val in (range (len (bitlst))):
            # print ("===             {m:22s}:=   7   6   5   4   3   2   1   0=".format (m="32 bit layout"))
            # print ("===             {m:22s}:=32103210321032103210321032103210=".format (m="32 bit layout"))
            # print ("===          {m:25s}:={v:32b} (= 0x{x:0X} = {d:d} d)".
            #        format (m="Bit field value [{i:2d}]".format (i=val),
            #                v=bitlst [val],
            #                x=bitlst[val],
            #                d=bitlst[val]))
            # print ("===             {m:22s}: {v:32b}".format (m="Field opcode", v=fieldopc))
            fieldopc = fieldopc + bitlst [val]
            # finalopc = finalopc + bitlst [val]
            
        # print ("===             {m:22s}: {v:32b} (0x{x:0X})".format (m="Final contribution", v=finalopc, x=finalopc))
        finalopc = finalopc + fieldopc # + bitlst [val]
    
    # print ("===          {m:25s}: {v:32b} (0x{v1:08X})".format (m="Final Bit field value", v=finalopc, v1=finalopc))
    ...
    #     arg2    = get_argnum_str (res, 2) if (num_asm_args >= 2) else ""
    #     arg3    = get_argnum_str (res, 3) if (num_asm_args >= 3) else ""
    #     arg4    = get_argnum_str (res, 4) if (num_asm_args >= 4) else ""
    ...
    #     argslst = combine_args (insn, arg1, arg2, arg3, arg4)
    #     print ("=== {m:25s}: {v}".format (m="Combined args are", v=argslst))
    #     arg1 = argslst[0]
    #     arg2 = argslst[1]
    #     arg3 = argslst[2]
    #     arg4 = argslst[3]
    ...
    # print ("=== {m:25s}:".format (m="Leaving  build_list_of_opcodes()"))
...
def build_list_of_opcodes (insn, l, bf, res, rec):
    ...
    if (l == []):
        ...
        # print ("yyy fst: {b}".format (b=pp.pformat(fst)))
        # print ("yyy #fst: {b}".format (b=numfst))
        for i in range (numfst):
            reslst = res.copy()
            ...
            if (argnum != -1):  # We are some argument in the opcode pattern
                ...
                # print ("zzz inargtype: {v}".format (v=inargtype))
                # print ("zzz inargstr:  {v}".format (v=inargstr))
                # print ("zzz inargtouse: {v}".format (v=inargtouse))
                # print ("zzz numvalofarg: {v}".format (v=numvalofarg))
                if ((bftouse [out_bitlst_arg_type_at] == "r") or
                    (bftouse [out_bitlst_arg_type_at] == "f") or
                    (bftouse [out_bitlst_arg_type_at] == "c") or
                    (bftouse [out_bitlst_arg_type_at] == "a") or
                    False):
                    # print ("xxx bftouse: {b}".format (b=pp.pformat(bftouse)))
                    # print ("xxx out_bitlst_reg_val_lst_at: {b}".format (b=out_bitlst_reg_val_lst_at))
                    # print ("xxx i: {b}".format (b=i))
                    regvalofarg = bftouse[out_bitlst_reg_val_lst_at][i]
                else:
                    regvalofarg = -1
...
def do_final_expansion (insn):
    ...
    for bf in bit_fields:
        ...
        print ("===   BF[{i:d}] = ".format (i=out_bitlst_is_arg_num_at))
        print ("===   {v}".format (v=bf[out_bitlst_is_arg_num_at]))
    ...
    # fe1 = iter_build_list_of_opcodes (insn, oclst, bit_fields, [], 0)
...
def print_nibbled_binary (in_num):
    nib7 = "{o:04b}".format (o=(in_num & 0xF0000000) >> 28)
    nib6 = "{o:04b}".format (o=(in_num & 0x0F000000) >> 24)
    nib5 = "{o:04b}".format (o=(in_num & 0x00F00000) >> 20)
    nib4 = "{o:04b}".format (o=(in_num & 0x000F0000) >> 16)
    nib3 = "{o:04b}".format (o=(in_num & 0x0000F000) >> 12)
    nib2 = "{o:04b}".format (o=(in_num & 0x00000F00) >>  8)
    nib1 = "{o:04b}".format (o=(in_num & 0x000000F0) >>  4)
    nib0 = "{o:04b}".format (o=(in_num & 0x0000000F))

    res = nib7 + " " + nib6 + " " + nib5 + " " + nib4 + " " + nib3 + " " + nib2 + " " + nib1 + " " + nib0
    return res
...
def final_expanded_insns (fe):
    ...
    for the_i in fe:
        ...
        b = print_nibbled_binary (the_i[0])    # Opcode printed as a 32 bit binary string
    ...
    for insn in insn_iterator:
        if (verbosity_level > 0):   # Detail level: 1
            print ("=== {c:02d}/{mi:02d} {p:{w}s}Processing insn: {i}/{e}".
                   format(c=call_depth, mi=maxindent, p=".",
                          w=(3*1), i=insn [insn_name_at], e=insntoexpand))
