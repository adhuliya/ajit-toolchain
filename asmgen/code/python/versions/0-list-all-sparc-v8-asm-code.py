...
def setup_globals():
    ...
    #insn_str_template = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" # 32 bit string to fill; using "x" to ease debugging
    ...
    insn_integer_code_at          = 3 # 8 hex digits representation - the actual machine code!
    insn_asm_string_list_at       = 4 # ASM code
    insn_asm_comment_at           = 5 # Brief explanation. Particularly for ranges, and manual page/section!
...
def reorganise_formats_list (fl, hdr):
    ...
    # reformatted_list.append (fl[fmt_index_at])
    reformatted_list.append (fl[fmt_num_bit_fields_at])
    # reformatted_list.append (fl[fmt_bit_field_list_at])
    ...
    for i in range (0, num_bit_fields_lists): 
        ...
        if (hdr == 0):          # First row has labels (strings) instead of any numbers
            ...
            # print ("Examining bit field #{d} ...".format (d=i))
            # print ("   {n:20s}: {v:s} ...".format (n="Name", v=name))
...
def make_bit_mask (end, start):
    ...
    # print ("GENERATING MASK: Start: {s} -- End: {e} - Str Val : 0x{m:0{width}b} ".
    #        format (s=start, e=end, m=theval, width=32))
    # print ("GENERATING MASK: Start: {s} -- End: {e} - Str Mask: 0x{m:0{width}b} ".
    #        format (s=start, e=end, m=maskval, width=32))
    # print ("GENERATING MASK: Start: {s} -- End: {e} - Str MinM: 0x{m:0{width}b} ".
    #        format (s=start, e=end, m=minusmaskval, width=32))
...
#     #     maskval = maskval | 0x1 << i
#     for i in range (0, 32):
#         resetthebit = False if ( (i >= start) and (i <= end) ) else True
#         # if ( (i >= start) and (i <= end) ):
#         #     thebit = 0
#         # else:
#         #     thebit = 1
#         # maskval = maskval | resetthebit << i
#         print ("=== BitSet  MASK: i: {i:02d} (Set/Reset: {b}) - Mask: 0x{m:0{width}b} ".
#                format (i=i, b=resetthebit, m=set_bit (i,  resetthebit), width=32))
#         maskval = maskval & set_bit (i,  resetthebit)
#         print ("=== Interim MASK: i: {i:02d} (Set/Reset: {b}) - Mask: 0x{m:0{width}b} ".
#                format (i=i, b=resetthebit, m=maskval, width=32))

#     # maskval = 0xFFFFFFFF - maskval
#     print ("GENERATED  MASK: Start: {s} -- End: {e} - Mask: 0x{m:0{width}b} ".
#            format (s=start, e=end, m=maskval, width=32))
...
def do_given (bitfld, insn, is_arg_num):
    ...
    # print ("=== ... ... ... ... ... {m:16s}: {l:0{w}b}/{s}".format(m="Binary MASK", l=bf_mask, w=32, s=bf_end))
    useval = int (bf_given, 16)
    # print ("=== ... ... ... ... ... {m:16s}: {l:0{width}b}/{w}".format(m="Given Binary Val", l=useval, width=bf_nbits, w=bf_nbits))
    # print ("=== ... ... ... ... ... {m:16s}: {l:X}/{w}".format(m="Given Hex Val", l=useval, w=8))
    # print ("=== ... ... ... ... ... {m:16s}: {l:d}".format(m="Given Dec Val", l=useval))

    # print ("=== ... ... ... ... ... {m:16s}: {l:0d}".format(m="Left shift by", l=bf_end))
    # useval = 0xFFFFFFFF & (useval << bf_end)
    # useval = bf_mask | (useval << bf_end)
    useval = (useval << bf_end)
    # print ("=== ... ... ... ... ... {m:16s}: {l:0{width}b}/{w}".format(m="0xFBinary Val", l=useval, width=32, w=32))
    # print ("=== ... ... ... ... ... {m:16s}: {l:0{width}b}/{w}".format(m="0xMinusFB Val", l=(0xFFFFFFFF - useval), width=32, w=32))
    # print ("=== ... ... ... ... ... {m:16s}: {l:08X}/{w}".format(m="Given Hex Val", l=useval, w=8))
    # print ("=== ... ... ... ... ... {m:16s}: {l:0d}".format(m="Given Dec Val", l=useval))
...
def do_lookup (bitfld, insn, is_arg_num):
    ...
    # print ("=== ... ... ... ... ... {m:16s}: {l:0{width}b}/{w}".format(m="Binary Val", l=useval, width=bf_nbits, w=32))
    # print ("=== ... ... ... ... ... {m:16s}: {l:X}/{w}".format(m="Hexadecimal Val", l=useval, w=8))
    # print ("=== ... ... ... ... ... {m:16s}: {l:d}".format(m="Decimal Val", l=useval))

    # print ("=== ... ... ... ... ... {m:16s}: {l:0d}".format(m="Left shift by", l=bf_end))
    useval = bf_mask | (useval << bf_end)
    # print ("=== ... ... ... ... ... {m:16s}: {l:0{width}b}/{w}".format(m="Binary Val", l=useval, width=32, w=32))
    # print ("=== ... ... ... ... ... {m:16s}: {l:08X}/{w}".format(m="Hexadecimal Val", l=useval, w=8))
    # print ("=== ... ... ... ... ... {m:16s}: {l:0d}".format(m="Decimal Val", l=useval))
...
def do_generate (bitfld, insn, is_arg_num):
    ...
    if (bf_nbits <= max_list_length_bits):
        ...
        for i in range (num_vals):
            # print ("=== ... ... ... ... ... {m:16s}: {l:0{w}b}/{s}".format(m="Binary MASK", l=bf_mask, w=32, s=bf_end))
            useval = i
            # print ("=== ... ... ... ... ... {m:16s}: {l:0{w}b}/{w}".format(m="Binary Val", l=useval, w=bf_nbits))
            #print ("=== ... ... ... ... ... {m:16s}: {l:X}/{w}".format(m="Hexadecimal Val", l=useval, w=8))
            #print ("=== ... ... ... ... ... {m:16s}: {l:d}".format(m="Decimal Val", l=useval))
            ...
            # useval = bf_mask | (useval << bf_end)
            useval = (useval << bf_end)
            # print ("=== ... ... ... ... ... {m:16s}: {l:0{w}b}/{s}".format(m="Binary Val", l=useval, w=32, s=bf_end))
            # print ("=== ... ... ... ... ... {m:16s}: {l:0{w}b}/{s}".format(m="MinusB Val", l=(0xFFFFFFFF - useval), w=32, s=bf_end))
            #print ("=== ... ... ... ... ... {m:16s}: {l:08X}/{w}".format(m="Hexadecimal Val", l=useval, w=8))
            #print ("=== ... ... ... ... ... {m:16s}: {l:0d}".format(m="Decimal Val", l=useval))
...
def do_range (bitfld, insn, is_arg_num):
    ...
    # print ("=== ... ... ... ... ... {m:16s}: [{l1} : {l2}] values".
    #        format(m="Generating", l1=min, l2=max))
    ...
    # out_lst = [is_arg_num, bf_name, out_opc_lst]
    out_lst = [is_arg_num, bf_name, bf_val_type, out_opc_lst, reg_val_lst]
    print ("=== ... ... ... ... {m:20s}: {l}".format(m="DONE Doing GENERATE", l=out_lst))
...
def expand_bit_field (bitfld, insn):
    ...
    if (bf_val_type == "generate"):
       ...
       # bitlst.append (do_generate (bitfld, insn, bf_is_arg_num))
       # gen_bitvals_lst = do_generate (bitfld, insn, bf_is_arg_num)
    
    if (bf_val_type == "given"):
        bitlst = do_given (bitfld, insn, bf_is_arg_num)
        # bitlst.append (do_given (bitfld, insn, bf_is_arg_num))
    
    if (bf_val_type == "range"):
        bitlst = do_range (bitfld, insn, bf_is_arg_num)
        # bitlst.append (do_range (bitfld, insn, bf_is_arg_num))
    
    if (bf_val_type == "lookup"):
        bitlst = do_lookup (bitfld, insn, bf_is_arg_num)
        # bitlst.append (do_lookup (bitfld, insn, bf_is_arg_num))
...
def expand_insn (insn):
    ...
    out_insn = [insn[insn_name_at],            # Name of the source database entry of this expansion
                ...
                False]                         # Bit expansion not yet complete!
    # out_insn = [insn[insn_name_at],            # Name of the source database entry of this expansion
    #             insn_str_template,             # 32 bit string to fill; using "x" to ease debugging
    #             0x0,                           # 32 bit integer code of the bit representation; the machine code
    #             insn [insn_asm_mnemonic_at],   # Concatenate the arguments to this string
    #             insn [insn_op_desc_at],        # The comment string to enhance!
    #             False]                         # Bit expansion not yet complete!
    ...
    for fmt in fmt_iterator:
        ...
        ### status = "YES" if (num_bitflds_processed == num_bitflds) else "NO"    
        # if (num_bitflds_processed == num_bitflds):
        #     status = "YES"
        # else:
        #     status = "NO"
        # print ("         {m:20s}: {l}".format (m="Expansion OK?",        l=status))
        ...
        # print ("         {m:20s}: {l}".format (m="Expanded bitlists",    l=expanded_outlst))
        print ("         {m:20s}:".format (m="Expanded bitlists"))
        pp.pprint (expanded_outlst)
            # theOpcode = theOpcode | tmpintval
            # print ("            {m:20s}: {l:{width}b}".format (m="INSN Opcode so far", l=theOpcode, width=32))

        # oc_lst = make_opcodes_lst (expanded_outlst)
        # print ("            {m:20s}: {l}".format (m="Expanded List", l=oc_lst))
            
        ### out_insn [insn_integer_code_at] = oc_lst
        ### insn [insn_expanded_fields_at] = expanded_outlst
        ### insn.append (expanded_outlst)
        ### insn [insn_info_end_at] = expanded_outlst
...
def build_list_of_opcodes (l, res, rec):
    ...
    if (x == 0):
        ...
        rst = l[1:]
        # print ("+++")
        # print ("{d}: FST is a (is list?: {t}) and is: {l}".format (d=rec, t=isinstance (fst, list), l=fst))
        # print ("{d}: RST is a (is list?: {t}) and is: {l}".format (d=rec, t=isinstance (rst, list), l=rst))
        # print ("  {m:>15s} : {r}".format (m="res", r=res))
        # print ("  {m:>15s} : \"{r}\"".format (m="recursion level", r=rec))
        # # print ("  {m:>15s} : \"{r}\"".format (m="ident(l)", r=id(l)))
        # # print ("  {m:>15s} : \"{r}\"".format (m="ident(rec)", r=id(rec)))
        # # print ("  {m:>15s} : \"{r}\"".format (m="ident(res)", r=id(res)))
        # # print ("  {m:>15s} : \"{r}\"".format (m="ident(reslst)", r=id(reslst)))
        # print ("   +++")
        numfst = len (fst)
        for i in range (numfst):
            reslst = res.copy()
            # # reslst.extend(res)
            # print ("   {m:15s}: {v}".format (m="Iteration #", v=i))
            # print ("   {m:15s}: {v}".format (m="res before", v=res))
            # print ("   {m:15s}: {v}".format (m="reslst before", v=reslst))
            # print ("   {m:15s}: {v}".format (m="== fst[i] ==", v=fst[i]))
            reslst.append (fst[i])
            # # reslst = [fst[i]] if (res == []) else [res[0], fst[i]]
            # print ("   {m:15s}: {v}".format (m="res after", v=res))
            # print ("   {m:15s}: {v}".format (m="reslst after", v=reslst))
...
def expand_all_insns (il):
    ...
    for insn in insn_iterator:
        if (insn [insn_name_at] != "Opcode"): # SKIP the headers list
            ...
            # insn_expansion_lst = expand_insn (insn)
            # out_insns_lst.append (insn_expansion_lst)
...
# print ("=== len (insn_str_template) = {l}".format (l=len(insn_str_template)))
# for i in range (len (insn_str_template)):
#     print ("===   insn_str_template[{i:02d}] = {l}".
#            format (i=i, l=insn_str_template[i]))
print ("=== INSNs list ...")
pp.pprint (insnlst)
...
# the_v = make_bit_mask (30, 31)
# print ("{v:0{width}b}".format (v=the_v, width=32))
# print ("{v:0{width}b}".format (v=(0xFFFFFFFF - the_v), width=32))
# the_v = make_bit_mask (12, 19)
# print ("{v:0{width}b}".format (v=the_v, width=32))
# print ("{v:0{width}b}".format (v=(0xFFFFFFFF - the_v), width=32))
# the_v = make_bit_mask (1, 3)
# print ("{v:0{width}b}".format (v=the_v, width=32))
# print ("{v:0{width}b}".format (v=(0xFFFFFFFF - the_v), width=32))
# the_v = make_bit_mask (0, 7)
# print ("{v:0{width}b}".format (v=the_v, width=32))
# print ("{v:0{width}b}".format (v=(0xFFFFFFFF - the_v), width=32))
# the_v = make_bit_mask (13, 13)
# print ("{v:0{width}b}".format (v=the_v, width=32))
# print ("{v:0{width}b}".format (v=(0xFFFFFFFF - the_v), width=32))
