    ...
    if (verbosity_level > 0):   # detail level: 1
       print ("=== {c:02d}/{mi:02d} {p:{w}s}{m:20s}: {l}".
              format(c=call_depth, mi=maxindent, p=".", w=(3*1),
                     ...
                     l=fl if (verbosity_level > 3) else []))
    ...
    if (verbosity_level > 0):   # detail level: 3
       print ("=== {c:02d}/{mi:02d} {p:{w}s}{m:20s} \"{n}\": {l}".
              format(c=call_depth, mi=maxindent, p=".", w=(3*3),
                     ...
                     l="Ignoring headers row" if (fmt_str == "Format Index") else (fl if (verbosity_level > 3) else [])))
...
# def is_which_arg (arglst):
#     '''
#     Return the ASM argument numner this bitfield describes.
#     '''
#     return arglst[0]

# def is_which_arg (bitfld, insn, arglst):
#     '''
#     Return the ASM argument numner this bitfield describes.
#     '''
#     global call_depth, maxindent
...
#     arglst = analyse_args (bitfld, insn)
#     print ("zzz {m:30s}: {v}".format (m="is_which_arg lst Bitfield", v=arglst))
    
#     argnum = arglst[0]
#     print ("zzz {m:30s}: {v}".format (m="is_which_arg NUM Bitfield", v=argnum))
...
def analyse_args (bitfld, insn):
    ...
    for i in range (int (num_args, 10)):
        if   (bf_name == theargs[i]):
            ...
            reglst = []
            # [argnum, bf_name, argname, argtype, argstr, argvallst, reglsttouse]
            thelst = [argnum, bf_name, argstr, arg_type[argnum - 1], argstr, [], []]
            break

    # print ("yyy {m:30s}: {v}".format (m="ArgNum Matched by field name", v=argnum))
    print ("yyy {m:30s}: {v}".format (m="ArgNum Matched by field name", v=thelst))
        ...
        for i in range (int (num_args, 10)):
            ...
            if ("=" in theargs[i]):
                ...
                if (bf_name == argsplits[0]):
                    ...
                    reglst = []
                    # [argnum, bf_name, argname, argtype, argstr, argvallst, reglsttouse]
                    thelst = [argnum, bf_name, argstr, arg_type[argnum - 1], argsplits[1], [], []]
                    break
            else:
                print (" ... NOT FOUND.")
...
def expand_bit_field (bitfld, insn):
    ...
    if (verbosity_level > 0):   # Detail level 3
       print ("=== {c:02d}/{mi:02d} {p:{w}s}{m:20s}: {l}".
              format(c=call_depth, mi=maxindent, p=".", w=(3*3),
                     m="ARGS LIST IS", l=args_lst))
    
    bf_val_type = bitfld [bit_field_val_type_at]
    
    if (bf_val_type == "generate"):
       bitlst = do_generate (bitfld, insn, args_lst)
    
    if (bf_val_type == "given"):
        bitlst = do_given (bitfld, insn, args_lst)
    
    if (bf_val_type == "range"):
        bitlst = do_range (bitfld, insn, args_lst)
    
    if (bf_val_type == "set"):
        bitlst = do_set (bitfld, insn, args_lst)
    
    if (bf_val_type == "lookup"):
        bitlst = do_lookup (bitfld, insn, args_lst)
...
def build_list_of_opcodes (insn, l, il, res, rec):
    ...
    if (l == []):
        ...
        for i in range (numfst):
            ...
            opsubcode = fst[i]
            if (infolstrec[out_bitlst_given_val_at] != -1): # Record "static" information
                if ((infolstrec[out_bitlst_bf_val_type_at] == "given")   or
                    (infolstrec[out_bitlst_bf_val_type_at] == "lookup")  or
                    (infolstrec[out_bitlst_bf_val_type_at] == "range")  or
                    (infolstrec[out_bitlst_bf_val_type_at] == "UNUSED")):
                    otherstr = "{n} = {v}".format (n=infolstrec[out_bitlst_bf_name_at], v=infolstrec[out_bitlst_given_val_at])
                    
            argnum = infolstrec[out_bitlst_is_arg_num_at]

            if (argnum != -1):  # We are some argument in the opcode pattern

                # inargtype can be one of [rfncg]
                print ("xxx {m:30s}: {v}".format (m="INSN Info Lst Rec", v=infolstrec))
                ...
                inargstr    = infolstrec[out_bitlst_bf_name_at].casefold()
                print ("xxx {m:30s}: {v}".format (m="INSN Arg Str", v=inargstr))
                numvalofarg = infolstrec[out_bitlst_out_opc_lst_at][i]
                print ("xxx {m:30s}: {v}".format (m="INSN Reg OpC Lst", v=infolstrec[out_bitlst_out_opc_lst_at]))
                ...
                #     numvalofarg = infolstrec[out_bitlst_reg_val_lst_at][i]
                #     argstr = get_reg_gname (numvalofarg)
