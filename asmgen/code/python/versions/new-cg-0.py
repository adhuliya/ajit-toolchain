...
def reorganise_formats_list (fl, hdr):
    ...
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Bit Field Value Type at",
    #                   v1=bit_field_val_type_at))
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Bit Field Name at",
    #                   v1=bit_field_name_at))
    ...
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Number of bit fields",
    #                   v1=int(fl[fmt_num_bit_fields_at])))
    sl = fl[fmt_bit_field_list_at:]
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Bit Fields list",
    #                   v1=sl))
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Length of Bit Fields",
    #                   v1=len(sl)))
    # num_bit_fields_lists = (int) (len (sl) / num_bit_fields) ## if (hdr == 0) else 1
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Type of len(sl)",
    #                   v1=type(len(sl))))
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Type of sl",
    #                   v1=type(sl)))
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Type of num_bit_fields",
    #                   v1=type(num_bit_fields)))
    num_bit_fields_lists = len (sl) / num_bit_fields ## if (hdr == 0) else 1
    num_bit_fields_lists = 6 * num_bit_fields ## if (hdr == 0) else 1
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Num_bit_fields_lists",
    #                   v1=num_bit_fields_lists))
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Type of num_bit_fields",
    #                   v1=type(num_bit_fields_lists)))
    ...
    for i in range (0, num_bit_fields): 
        ...
        field_offset = (i * 6)
        pp.pprint ("=== {m:30s}: {v:02d} (Type: '{v2}')".
                   format(m="Field offset index",
                          v=i,
                          v2=type(i)))
        pp.pprint ("=== {m:30s}: {v:02d} = {v1} (Type: '{v2}')".
                   format(m="Field Offset at",
                          v=field_offset,
                          v1=sl[field_offset],
                          v2=type(sl[field_offset])))
        # pp.pprint ("=== Printing the fields and their type before processing.")
        # pp.pprint ("====== {m:30s}: -{v}- has type = {v1}".
        #            format(m="Name",
        #                   v=sl[field_offset + bit_field_name_at],
        #                   v1=type(sl[field_offset + bit_field_name_at])))
        # pp.pprint ("====== {m:30s}: -{v}- has type = {v1}".
        #            format(m="Start",
        #                   v=sl[field_offset + bit_field_start_at],
        #                   v1=type(sl[field_offset + bit_field_start_at])))
        # pp.pprint ("====== {m:30s}: -{v}- has type = {v1}".
        #            format(m="End",
        #                   v=sl[field_offset + bit_field_end_at],
        #                   v1=type(sl[field_offset + bit_field_end_at])))
        # pp.pprint ("====== {m:30s}: -{v}- has type = {v1}".
        #            format(m="Length",
        #                   v=sl[field_offset + bit_field_length_at],
        #                   v1=type(sl[field_offset + bit_field_length_at])))
        # pp.pprint ("====== {m:30s}: -{v}- has type = {v1}".
        #            format(m="Value String",
        #                   v=sl[field_offset + bit_field_length_at],
        #                   v1=type(sl[field_offset + bit_field_val_str_at])))
        # pp.pprint ("====== {m:30s}: -{v}- has type = {v1}".
        #            format(m="Value Type",
        #                   v=sl[field_offset + bit_field_length_at],
        #                   v1=type(sl[field_offset + bit_field_val_type_at])))
        # pp.pprint ("=== DONE Printing the fields and their type before processing.")
        ...
        if (hdr == 0):          # First row has labels (strings) instead of any numbers
            ...
            pp.pprint ("=== {m:30s}: {v:02d} = {v1} (Type: '{v2}')".
                        format(m="Bit Field Name at",
                               ...
                               v1=name,
                               v2=type(name)))
            # pp.pprint ("=== {m:30s}: {v:02d} = {v1}".
            #                format(m="TYPE Bit Field Start at",
            #                 v=bit_field_start_at,
            #                 v1=type(sl[field_offset + bit_field_start_at])))
            # pp.pprint ("=== {m:30s}: {v:02d} = {v1}".
            #                format(m="TYPE Bit Field Start at",
            #                 v=bit_field_start_at,
            #                 v1=type(int (sl[field_offset + bit_field_start_at]))))
            start            = int (sl[field_offset + bit_field_start_at])
            pp.pprint ("=== {m:30s}: {v:02d} = {v1} (Type: '{v2}')".
                           format(m="Bit Field Start at",
                                  v=bit_field_start_at,
                                  v1=start,
                                  v2=type(start)))
            end              = int (sl[field_offset + bit_field_end_at])
            pp.pprint ("=== {m:30s}: {v:02d} = {v1} (Type: '{v2}')".
                        format(m="Bit Field End at",
                               v=bit_field_end_at,
                               v1=end,
                               v2=type(end)))
            # length           = end - start + 1
            length           = start - end + 1
            pp.pprint ("=== {m:30s}: {v:02d} = -{v1}- (Type: '{v2}')".
                        format(m="Bit Field Length at",
                               v=bit_field_length_at,
                               v1=length,
                               v2=type(length)))
            # pp.pprint ("=== {m:30s}: {v:02d} = -{v1}- (Type: '{v2}')".
            #                format(m="Bit field Value String at",
            #                       v=bit_field_val_str_at,
            #                       v1=sl[field_offset + bit_field_val_str_at],
            #                       v2=type(sl[field_offset + bit_field_val_str_at])))
            # value            = -1 if (sl[field_offset + bit_field_val_str_at] == "") else int (sl[field_offset + bit_field_val_str_at], 0)
            ...
            pp.pprint ("=== {m:30s}: {v:02d} = {v1} (Type: '{v2}')".
                           format(m="Bit field Value Type at",
                                  v=bit_field_val_type_at,
                                  v1=value_type,
                                  v2=type(value_type)))
            total_bits       = total_bits + length

        pp.pprint ("=== {m:30s}: {v:02d}".
                       format(m="Total bits accumulated are",
                                  v=total_bits))
