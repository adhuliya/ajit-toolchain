...
uint8_t connect_option = mL_CONNECT_WITH_PIPEHANDLER;
void setMonitorLoggerInSocketMode()
{
	connect_option = mL_CONNECT_WITH_SOCKET;
}
						...
						uint32_t reg_val_l)
{
	...
	uint8_t fsr_signature =  ((fsr_val >> 10) & 0x3) | ((fsr_val >> 14) & 0x7);
...
void monitorLogger(void)
{
	...
	while(1)
	{
		...
		if(connect_option == mL_CONNECT_WITH_PIPEHANDLER)
			reg_write_log = read_uint32("AJIT_to_ENV_logger");
		else
			reg_write_log = sock_read_uint32("AJIT_to_ENV_logger");



		// 32-bit fp register write signature
		//   31 write-fpreg-from-fp
		//   30 write-fpreg-from-ccu
		//   29 write-fsr-from-fp
		//   28 write-fsr-from-ccu
		...
		if(connect_option == mL_CONNECT_WITH_PIPEHANDLER)
			fp_reg_write_log = read_uint32("AJIT_to_ENV_logger");
		else
			fp_reg_write_log = sock_read_uint32("AJIT_to_ENV_logger");
		...
		if(connect_option == mL_CONNECT_WITH_PIPEHANDLER)
			store_log = read_uint32("AJIT_to_ENV_logger");
		else
			store_log = sock_read_uint32("AJIT_to_ENV_logger");


		// 32-bit PC
		uint32_t pc_log = 0;
		if(connect_option == mL_CONNECT_WITH_PIPEHANDLER)
			pc_log = read_uint32("AJIT_to_ENV_logger");
		else
			pc_log = sock_read_uint32("AJIT_to_ENV_logger");
		...
		if(reg_write_file)
		{
			...
			fprintf(reg_write_file,"%d. PC=%08x, Reg-Log=%08x Fp-Reg-log=%08x  Store-log= %08x\n", counter, 
					pc_log,
					reg_write_log,
					fp_reg_write_log,
					store_log);
