...
DebugServerCoreMode debug_server_core_mode = dbg_SINGLE_CORE_MODE;
void setDebugServerCoreMode(DebugServerCoreMode m)
{
	debug_server_core_mode = m;
}
int isDebugServerMultiCoreMode()
{
	return(debug_server_core_mode == dbg_MULTI_CORE_MODE);
}
...
void probeCcu(DebugServerState* server_state, int CORE_ID)
{
	//////////////////////////////////////////////  send info to ccu ////////////////////////////////////////
	...
	{
		if(isDebugServerMultiCoreMode())
		{
			perCoreSendU32FromDebugServerToCcu (CORE_ID, server_connect_mode, words_to_ccu[IDX]);
		}
		else
		{
	...
	if(isDebugServerMultiCoreMode())
	{
		ccu_word_1_valid = perCoreRecvValidCcuMessage (CORE_ID, 0, &ccu_word_1);
	}
	else
	{
	...
	if(get_second_word)
	{
		...
		if(isDebugServerMultiCoreMode())
		{
			ccu_word_2_valid = perCoreRecvValidCcuMessage(CORE_ID, 1,&ccu_word_2);
		}
		else
		{
	...
	if(get_third_word)
	{
		...
		if(isDebugServerMultiCoreMode())
		{
			ccu_word_3_valid  = perCoreRecvValidCcuMessage(CORE_ID, 1,&ccu_word_3);
		}
		else
		{
	...
	if(get_fourth_word)
	{
		...
		if(isDebugServerMultiCoreMode())
		{
			ccu_word_4_valid =   perCoreRecvValidCcuMessage(CORE_ID, 1,&ccu_word_4);
		}
		else
		{
...
void probeGdb(DebugServerState* server_state, int CORE_ID)
{
	...
	if(ccu_command_exists)
	{
		...
		if(send_first_word_to_gdb)
		{
			...
			if(isDebugServerMultiCoreMode())
			{
				perCoreSendU32FromDebugServerToGdb(CORE_ID, first_word_to_gdb);
			}
			else
			{
		...
		if(send_second_word_to_gdb)
		{
			...
			if(isDebugServerMultiCoreMode())
			{
				perCoreSendU32FromDebugServerToGdb(CORE_ID, second_word_to_gdb);
			}
			else
			{
	...
	if(!gdb_in_invalid_state)
	{
		if(isDebugServerMultiCoreMode())
		{
			gdb_word_1_valid = perCoreRecvValidGdbMessage (CORE_ID, 0, &gdb_word_1);
		}
		else
		{
		...
		if(get_second_word_from_gdb)
		{
			...
			if(isDebugServerMultiCoreMode())
			{
				gdb_word_2_valid = perCoreRecvValidGdbMessage (CORE_ID, 1, &gdb_word_2);
			}
			else
			{
		...
		if(get_third_word_from_gdb)
		{
			...
			if(isDebugServerMultiCoreMode())
			{
				gdb_word_3_valid = perCoreRecvValidGdbMessage (CORE_ID, 1, &gdb_word_3);
			}
			else
			{
		...
		if(send_pc_or_npc)
		{
			...
			if(isDebugServerMultiCoreMode())
			{
				perCoreSendU32FromDebugServerToGdb(CORE_ID, quick_debug_response_word);
			}
			else
			{
...
void debugServerDaemon()
{
	...
	while(1) 
	{
		probeCcu(&server_state,-1);
		probeGdb(&server_state,-1);
		usleep(100);
	}
}
...
void fromCcuDemuxDaemon () 
{
	while(1)
	{
		int core_id;
		if(server_connect_mode == dbg_CONNECT_WITH_SOCKET_BYTE_MODE)
			core_id  = sock_read_uint8 ("DEBUG_to_MONITOR");
		else if (server_connect_mode == dbg_CONNECT_WITH_PIPE_HANDLER)
			core_id  = read_uint8 ("DEBUG_to_MONITOR");
		else
			assert(0);

		char pname[256];
		sprintf(pname,"DEBUG_to_MONITOR_%d\n", core_id);
		int I;
		for(I=0; I < 4; I++)
		{
			uint8_t B;
			if(server_connect_mode == dbg_CONNECT_WITH_SOCKET_BYTE_MODE)
				B  = sock_read_uint8 ("DEBUG_to_MONITOR");
			else if (server_connect_mode == dbg_CONNECT_WITH_PIPE_HANDLER)
				B  = read_uint8 ("DEBUG_to_MONITOR");
			else
				assert(0);
...
DEFINE_THREAD(fromCcuDemuxDaemon);
PTHREAD_DECL(fromCcuDemuxDaemon);


void perCoreDebugServerDaemon(void* vid)
{
	int CORE_ID = *((int*)vid);
	...
	while(1) 
	{
		probeCcu(&server_state, CORE_ID);
		probeGdb(&server_state, CORE_ID);
		usleep(100);
	}
}

DEFINE_THREAD_WITH_ARG (perCoreDebugServerDaemon, core_id);
PTHREAD_DECL(perCoreDebugServerDaemon);

void startPerCoreDebugServerDaemons (int NCPUS)
{
	int CORE_ID;

	CORE_ID = 0;
	startPerCoreNoblockDaemon(0);
	PTHREAD_CREATE_WITH_ARG(perCoreDebugServerDaemon, &CORE_ID);

	if(NCPUS > 0)
	{
		startPerCoreNoblockDaemon(1);
		CORE_ID = 1;
		PTHREAD_CREATE_WITH_ARG(perCoreDebugServerDaemon, &CORE_ID);
	}
	if(NCPUS > 1)
	{
		startPerCoreNoblockDaemon(2);
		CORE_ID = 2;
		PTHREAD_CREATE_WITH_ARG(perCoreDebugServerDaemon, &CORE_ID);
	}
	if(NCPUS > 2)
	{
		startPerCoreNoblockDaemon(3);
		CORE_ID = 3;
		PTHREAD_CREATE_WITH_ARG(perCoreDebugServerDaemon, &CORE_ID);
	}

	if((server_connect_mode == dbg_CONNECT_WITH_SOCKET_BYTE_MODE) ||
			(server_connect_mode == dbg_CONNECT_WITH_PIPE_HANDLER))
	{
		fprintf(stderr,"Info: starting fromCcuDemuxDaeon\n");
		PTHREAD_CREATE(fromCcuDemuxDaemon);
	}
}
