/* Test for "flush" instruction with MMU and caches enabled.
First, setup page tables such that Virtual addr = phy addr 
for entire address space. Then enable the mmu.
This enables caching.
Then test the behavior of the FLUSH instruction.

NOTE : this test needs to be run with env V1 
(caches and mmu present in the model)
...
_start:
	...
	! Setup page tables.
	!======================================
	! Create a L0 PTE that maps all 
	! 4GB virt addreses to phy addresses 
	! Page Table Entry (PTE) = 0x8E;
	
	! Store PTE in ContextTable
	mov 0x8E, %o0	!PTE
	set ContextTable, %o1	!address of Context Table
	st  %o0, [%o1]	!Store PTE in ContextTable
	

	!Set Mmu's context table pointer register
	!First we need to convert an address into a CtxTablePtr entry
	!by shifting the address right by 4.
	srl %o1, 0x4, %o1
	set 0x100, %o0  !address of Mmu's context table ptr register
	sta %o1, [%o0] 0x4 ! sta with ASI=0x4 and address=0x100 writes to Mmu ctxTablePtr register

	!Enable the MMU by writing into MMUControlRegister
	set 0x1, %o0
	sta %o0, [%g0] 0x4 ! sta with ASI=0x4 and address=0x0 writes to Mmu Control register

	!Do some memory loads after the mmu has been enabled,
	!to test if address conversion is working fine
	set data_A, %o1			
	ld [%o1], %l3  		! l3 = 0xABCD
	...
	!=============================================================
	!OK. Now we test the behavior of FLUSH.
	!Basic idea : 
	!  modify code, flush, jump to modified code
	!=============================================================
	label1:
	...
	modify_code :
		! copy 5 words from label2 and write it at label1
		set label2, %l0
		set label1, %l1
		ld [%l0], %l2; st %l2, [%l1]; add %l0, 0x4, %l0 ; add %l1, 0x4, %l1;
		ld [%l0], %l2; st %l2, [%l1]; add %l0, 0x4, %l0 ; add %l1, 0x4, %l1;
		ld [%l0], %l2; st %l2, [%l1]; add %l0, 0x4, %l0 ; add %l1, 0x4, %l1;
		ld [%l0], %l2; st %l2, [%l1]; add %l0, 0x4, %l0 ; add %l1, 0x4, %l1;
		ld [%l0], %l2; st %l2, [%l1]; add %l0, 0x4, %l0 ; add %l1, 0x4, %l1;
		...
		! perform FLUSH (should flush dcache followed by Icache)
		flush
		nop
		nop

		!Flush D cache followed by Icache
		!Its commented out because "flush" should include
		!these two actions implicitly

		!sta %g0, [%g0] 0x13 ! sta with ASI=0x13 flushes Dcache
		!flush
		!nop
		!nop
	...
	! code to be inserted at label1.
	!this code snippette consists of 5 words.
	label2:
		mov 0x123, %g4
		mov 0x456, %g5
		ta 0
	...
	!!!!Context Table has to be aligned on Page boundary (4KiB)
	!!!for correct operation.
	.align 4096
	ContextTable: .word 0x00
