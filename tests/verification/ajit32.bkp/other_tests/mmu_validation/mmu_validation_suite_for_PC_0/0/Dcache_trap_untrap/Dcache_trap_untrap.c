// Check trap-untrap behavior in Dcache.
// The cache traps upon receiving an mae=1 from the MMU.
// The cpu must untrap the cache as soon as a new thread starts.
//
...
//
//Scheme:
//1.	Setup the context table and page tables 
//	such that all addresses are mapped such that VA=PA
//	except the range 0xA0000000=0xA1000000 (which is left un-mapped
//	so as to create a fault)
//
//2. Perform a load to an unmapped address. This should :
//	* causes a data-access exception
//	* as soon as the trap-handler begins, the Dcache should be untrapped.
//
//3. in the trap handler, update a register, 
//	perform some loads/stores for testing and return.
//=============================================================
...
void setupPageTables()
{
	...
	//Don't map the range 0xA0000000 to 0xA1000000, so as to generate a fault
	...
	for(i=0x00;i<=0xFF;i++)
	{
		...
		{
			process1_L1PageTable[i] = 0;//this addr range is not mapped
		}
	}
...
int __attribute__((section(".text.main"))) startTest()
{
	...
	//Now enable the MMU, setting NF(no fault)=0 
	uint32_t MMU_control_word = 0x1; //Enable(bit 0)=1, NF(bit 1)=0
	store_word_mmureg(MMU_control_word, (uint32_t*) MMU_REG_CONTROL);


	__asm__ __volatile__( "set 0xCC1, %g3"); //checkpoint
	
	
	//Now perform a load to an un-mapped address 
	//This should cause a page fault and 
	//update FSR and FAR. 
	//Control should go to the trap handler.
	__asm__ __volatile__( "set 0xA0000000, %l6");
	...
	//Perform some loads/stores to check if the
	//Dcache was un-trapped.
	__asm__ __volatile__( "set 0x00000004, %l7"); // changes from Harshal 7/09/2016 0xf0004004 to 0x00000004
	__asm__ __volatile__( "ld [%l7], %g7  ");
	__asm__ __volatile__( "set 0xF00BAF, %l5");
	__asm__ __volatile__( "sta %l5, [%l7] 0x20 ");
	__asm__ __volatile__( "nop");
	__asm__ __volatile__( "nop");
	__asm__ __volatile__( "nop");
	
	__asm__ __volatile__( "set 0xCC3, %g3"); //checkpoint
	


	//This point should always be reached...
	//halt
...
//============================================
//NOTE: HW_trap_0x09 is data_access_exception
...
HW_trap_0x09: mov 0x09, %g2; rett %r18; nop; nop;\
