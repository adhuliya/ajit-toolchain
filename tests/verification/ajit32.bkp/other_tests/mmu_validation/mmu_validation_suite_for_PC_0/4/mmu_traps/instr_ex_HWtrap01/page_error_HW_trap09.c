...
//--------------------------------setting page table---------------------------------------------------
	...
	process3_L1PageTable[0x10] = convertPhyAddrToPTD((uint32_t)process3_L2PageTable);
	...
	// 1 0 0 110 10 = 0x9A user:No Acess. Super:Read/Execute.
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
	uint32_t process1_PTE = ((0xA0<<12)<<8) | 0x8E ;//0x8E
	uint32_t process2_PTE = ((0xA0<<12)<<8) | 0x8E ;//0x9E or 0x9A 0r 0x 92
	uint32_t process3_PTE = ((0xA0<<12)<<8) | 0x8E ;//0x8E
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	j=(uint32_t*) 0xC0000000; //store it at A000 0000 address.
	*j=10; 
	...
	// **Note: check entry for this page uint32_t PTE_level1 = (PPN<<8) | (0x8E);
	// //1)this checks page translation for user space process.

	//  __asm__ __volatile__("set 0x10000000, %l3 \n\t");
	//  __asm__ __volatile__("ld  [%l3], %g7");	


	// //2)Trying to acess memory address that is not mapped yet. No page table entry for this address is in context.ACC=9A or ACC=9E or ACC=8E in PTE;
	// // It will generate fault L=0 AT=0,FT=1 i.e Load from user data space & Invalid Address Error. FSR=0x06
	// //D0 to FF->ACC=0x8E(User process) in PTE;0xA0000->Anything

	//  __asm__ __volatile__("set 0x20000000, %l3 \n\t"); 
	//  __asm__ __volatile__("ld  [%l3], %g7");


	// //3)Trying to acess memory address that is execute only permission.  
	// //D0 to FF->ACC=0x8E(User process) in PTE;0xA0000->0x92
	// // It will generate fault L=0 AT=0,FT=2 i.e Load from user data space & Protection error.

	//  __asm__ __volatile__("set 0x10000000, %l3 \n\t"); 
	//  __asm__ __volatile__("ld  [%l3], %g7");


	//4)Trying to acess memory address that which can be access by only Supervisor.  
	//D0 to FF->ACC=0x8E(User process) in PTE;0xA0000-> PTE.ACC=0x9A or 0x9E
	// It will generate fault L=0 AT=0,FT=3 i.e Load from user data space & Privilege violation error
...
//-----------------------------------------END Test for AT=0--------------------------------------------------------
	...
	//context_switch(1);//In supervisor space
	// **Note: check entry for this page uint32_t PTE_level2 = (PPN<<8) | (0x9E);
	//Process_2 has Supervisor access
	//1)Trying to acess memory address that is not mapped yet. No page table entry for this address is in context  
	//D0 to FF->ACC=0x9E(User process) in PTE;0xA0000-> PTE.ACC=0x9A or 0x9E
	// It will generate fault L=0 AT=1,FT=1 i.e Load from user data space & Invalid Address Error. 
	...
	//2)Trying to acess memory address which is read only 
	//D0 to FF->ACC=0x9E(User process) in PTE;0xA0000-> PTE.ACC=0x92
	//process2_PTE = ((0xA0<<12)<<8) | 0x82 ;//0x8E
	// It will generate fault L=0 AT=1,FT=2 i.e Load from user data space & Invalid Address Error. 
	...
	__asm__ __volatile__("set 0x10000000, %l4 \n\t");
	//--------------------------------------------------------------------------------------------------------------	
	// __asm__ __volatile__( "mov 0xE0, %l4 \n\t "); //80 only supervisor and no traps. //E0 only supervisor and traps.
	// __asm__ __volatile__("wr %l4, %psr \n\t nop\n\t");
	// // 	! insert nops here because\n\t 
	// __asm__ __volatile__( "	nop\n\t nop \n\t "); 

	__asm__ __volatile__("set 0xC0000000, %l4 ");
	__asm__ __volatile__( "mov 0x3, %g6  ");
	//__asm__ __volatile__("st %g4 , [%l4]" );
	__asm__ __volatile__("ld [%l4] , %g4" );
	__asm__ __volatile__( "mov 0x4, %g6  ");
...
__asm__ (".org 4096; HW_trap_0x00: mov 0xf0, %g5; restore; ta 0; nop;\
