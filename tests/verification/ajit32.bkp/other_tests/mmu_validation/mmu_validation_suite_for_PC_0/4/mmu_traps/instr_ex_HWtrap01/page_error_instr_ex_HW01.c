/*----------------------------------------------------------------------------
Filename:			page_error_instr_ex_HW01.c
Version:			1.00
Goal: 				Generate HW trap of no 9. instr access exception (0x01 )
Author:				Harshal Kalyane
Description:	
			Traps are generated by MMU when illegal memory reference take place. Mainly
four traps are generated.
entry for 0xF0005000 VA in process 2 or context 1 page table has permission for read only. Will generate protection error 
hence instr access exception
line 293:	process2_PTE = ((0x0F000500) | 0x82 );/instruction memory is read only**
-----------------------------------------------------------------------------*/
...
//--------------------------------setting page table---------------------------------------------------
	...
	uint32_t process2_PTE = ((0x00000100) | 0x9A );//0x9E or 0x9A 0r 0x 92
	...
	process2_PTE = ((0x00000000) | 0x9A );//***instruction memory is read only**
	process2_L3PageTable[0x00] = process2_PTE;

	process2_PTE = ((0x00000100) | 0x82 );//***instruction memory is read only**
	process2_L3PageTable[0x01] = process2_PTE;
	...
	process2_PTE = ((0x00000400) | 0x9A );//0x9E or 0x9A 0r 0x 92
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	...
	//--------------------------------------------------------------------------------------------------------------
	__asm__ __volatile__("set 0xF0008010, %l4 \n\t");
	//--------------------------------------------------------------------------------------------------------------	
