/*----------------------------------------------------------------------------
Filename:			page_error_AT7_S6.c
Version:			1.00
Goal: 				Fault Type=2 for Acess Type=7 and PTE.ACC=6
...
2.	Setup a context table, and page tables 
	...
	0x10000000 - 0x10001000 	0xA0000000-0xA00001000 with PTE.ACC=6

3.	Write a 0x12 using physical address(0xA0000000)
3. 	Initial context table ptr, context register, and enable MMU.
4.	MOV 0x3 to %g6
5.	In context 0 write data(%g6) using virtual addr(0x10000000) using ASI=0x09.
6. 	Check FSR/FAR in .vprj 
-----------------------------------------------------------------------------*/
...
//--------------------------------setting page table---------------------------------------------------
	...
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
		uint32_t PPN = (i<<12);
		uint32_t PTE_level1 = (PPN<<8) | (0x9A);//In this any one can read/write/ execute
	...
	// 1 0 0 110 10 = 0x9A user:No Acess. Super:Read/Execute.
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
	uint32_t process1_PTE = ((0xA0<<12)<<8) | 0x9A ;//PTE.ACC=7
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	...
	__asm__ __volatile__("sta %g6 , [%l4] 0x09 " );//asi=0x09 for supervisor instruction space
