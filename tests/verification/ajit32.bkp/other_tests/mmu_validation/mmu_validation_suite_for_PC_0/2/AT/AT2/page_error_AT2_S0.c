/*----------------------------------------------------------------------------
Filename:			page_error_AT2_S0.c
Version:			1.00
Goal: 				Fault Type=2 for Acess Type=2 and PTE.ACC=0
...
2.	Setup a context table, and page tables 
	...
	For context 1:
	virtual address			phy address
	0x00001000 - 0x00001FFF 	0x00001000 - 0x00001FFF with PTE.ACC=0
...
//--------------------------------setting page table---------------------------------------------------
	...
	process2_L3PageTable[0x00] = process_code_PTE;

	process2_PTE = (0x00000100) | 0x82 ;//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x01] = process2_PTE;		

	process2_PTE = ((0x00000200) | 0x8E );//PTE.ACC=0
	process2_L3PageTable[0x02] = process2_PTE;

	process2_PTE = ((0x00000300) | 0x8E );
	process2_L3PageTable[0x03] = process2_PTE;

	process2_PTE = ((0x00000400) | 0x8E );
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	...
	//======================================Test for AT=2,Serial no:0===========================================================
	//-----------------------------*******In Context[1] or process_2*******-----------------------------------------
	//--------------------------------------MMU Enabled-------------------------------------------------------------
	uint32_t MMU_control_word = 0x1; //set the Enable bit
	store_word_mmureg(0x1 , (uint32_t*) MMU_REG_CONTEXT);//process 2 
	...
	__asm__ (".align 4096");//these instruction lies at PA=0xF0005xxx
	__asm__ __volatile__("set 0xD0000000, %l4 ");
