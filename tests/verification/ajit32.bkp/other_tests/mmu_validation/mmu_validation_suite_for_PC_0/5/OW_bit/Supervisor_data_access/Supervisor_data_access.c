...
//	Perform a supervisor-level load to 
//	an address that is not mapepd in the mmu
//	This should cause a fault in the MMU, and
// 	modify FSR and FAR	
// 	but this should *not* lead to a trap. 
// 	
// 	Now cause another load fault 
// 	This should cause OW (overwrite bit) in the FSR to be set
...
int  __attribute__((section(".text.main")))  startTest()
{
	...
	//update FSR and FAR, but a trap is not caused
	...
	//Now perform another load to an unmapped address. 
	//This should cause a page fault and 
	//update FSR and FAR, but again a trap is not caused
	__asm__ __volatile__( "set 0xA0000010, %l6");
	__asm__ __volatile__( "ld [%l6], %g6  ");
