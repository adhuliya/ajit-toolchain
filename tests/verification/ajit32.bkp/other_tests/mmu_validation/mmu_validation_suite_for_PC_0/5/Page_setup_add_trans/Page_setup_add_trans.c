/*----------------------------------------------------------------------------
Filename:			Page_setup_add_trans.c
Version:			1.00
Goal: 				Page setup add trans
Author:				Harshal Kalyane
Description:	
			Goal of this test case is to check working of Context Table pointer, Page Setup and address translation.

Page Table Set-up:
					This test program first Disable MMU then stores base address
of context table in Context table pointer. Context table contains page tables(Level
1) for respective process. Level 1 page store base address of Level2 Page Table
(In PTD format). Similarly Level 2 contains base address of level 3 Page table.
Level 3 page table contains PTE.

Read data from 0x10000000 and store it 0xF0000000 with mmu disable.

Address translation: 
						Test program stores two Test Data to Physical memory such
that First data stores at Virtual Address=Physical address and second data stores
at physical address where virtual address is mapped.
First it reads data from address from where test Data 1 is stored.But MMU is
disabled so VA=PA. It should read Test Data 1. After this it enables MMU then
read from same virtual address. Here it should get Test Data 2.
Read data from 0x10000000 and store it g7 register with mmu enable.
0x10000000 VA map to 0xA0000000 (PA) for all process.
-----------------------------------------------------------------------------*/
...
void halt_test(void)
{
	__asm__ __volatile__("ta 8; nop; nop;");
}

void disable_mmu(void)
{
	uint32_t MMU_control_word = 0x0; //set the Enable bit
...
//--------------------------------setting page table---------------------------------------------------
	...
	//such that VA=PA
	for(i=0x00; i<=0xFF; i++) //0xD0 to 0x00 by harshal 7/09/2016
	{
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	uint32_t *u=(uint32_t*) 0x10000000; //store it at A000 0000 address.
	*u=15;

	__asm__ __volatile__("set 0x10000000, %l3 \n\t");
	__asm__ __volatile__("ld  [%l3], %g4");	
	__asm__ __volatile__("set 0xF0000000, %l3 \n\t");
	__asm__ __volatile__("st  %g4, [%l3]");	
	//======================================Test for AT=0,FT=0==========================================================
	//putString("\n----------------Test for AT=0,FT=0------------------------");
	//--------------------------------------MMU Enabled------------------------------------------------------------
	...
	// __asm__ __volatile__("set 0xF0000001, %l3 \n\t");
	// __asm__ __volatile__("st  %g7, [%l3]");	
//-----------------------------------------END Test for AT=0--------------------------------------------------------
...
//-----------------------------------------------END----------------------------------------------------------------
	//halt();
	__asm__ __volatile__("ta 6; nop; nop;");
	return 0;
...
SW_trap_0x86: mov 0x86, %g5; restore; call disable_mmu,0; nop;\
