...
//For Supervisor instruction accesses, the NF bit should be ignored.
//Thus the instruction fetch should trap, and control should go to 
...
//	Perform a supervisor-level instruction fetch to 
//	an address that is not mapepd in the mmu.
//	This should cause a fault in the MMU, and
// 	modify FSR and FAR.
// 	Control should go to trap handler 1
...
int  __attribute__((section(".text.main")))  startTest()
{
	...
	//Now perform a supervisor-level instruction fetch
	//from an address that is not mapped in the mmu
	//This should cause a page fault and 
	//update FSR and FAR.
	//A trap is always generated. Control goes to trap handler (HW trap) 
...
HW_trap_0x01: mov 0x01, %g5; nop; ta 0; nop;    ! Instruction access exception \
