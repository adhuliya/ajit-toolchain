/*----------------------------------------------------------------------------
Filename:			context_switching.c
Version:			1.00
Goal: 				check context switching in MMU
Author:				Harshal Kalyane
Description:	
			Goal of this Test case is to check context switching in MMU.Context 
switching is done by switching between page tables.Switching between page tab is 
done by changing value of context register.
It Set-ups two different page table for two different process as described in. 
It maps logical address X to physical address Y for process 0 and logical address X to 
physical address Z for process1.
It stores Test data 0 at physical address Y and Test data 1 at physical address Z.
Then Test program enable MMU With context register value 0 and read string
from logical address X.It should read Test data 0. After this it switches to context
1(How to do context switching is explained in Appendix) and read string from
same logical address.It should read Test data 1.

Here X=0X10000000, Y=0XA0000000 , Z=0XB0000000 
-----------------------------------------------------------------------------*/
...
void  startTest()
{
	...
	uint32_t process2_PTE = ((0xB0<<12)<<8) | 0x8E ;//0x9E or 0x9A 0r 0x 92
	uint32_t process3_PTE = ((0xC0<<12)<<8) | 0x8E ;//0x8E
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	j=(uint32_t*) 0xB0000000; *j=13;
	j=(uint32_t*) 0xC0000000; *j=14;
