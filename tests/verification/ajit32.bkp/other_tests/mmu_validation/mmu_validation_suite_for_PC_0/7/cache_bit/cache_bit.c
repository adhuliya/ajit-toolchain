/*----------------------------------------------------------------------------
Filename:			cache_bit.c
Version:			1.00
Goal: 				The Cacheable bit in a PTE specifies if a page should is cacheable or non cacheable
Author:				Harshal Kalyane
Description:	
			This test program first stores test data 1 at Physical address Y. It maps X to Y and
enables MMU with cacheable bit 1. Read data from VA X. It should read test data
1. Then it store test data 2 at PA Y using asi=0x20 to 0x2F using MMU bypass.
Then again read data from VA X. Again it should read test data 1 instead of test
data 2 because test data 1 is in cache.
Then test program stores test data 2 at Physical address Y. It maps X to Y and
enables MMU with cacheable bit 0. Read data from VA X. It should read test data
1. Then it store test data 2 at PA Y using asi=0x20 to 0x2F using MMU bypass.
Then again read data from VA X. But this time it should read test data 2 because
data is not in cache.

Here X=0X10000000, Y=0XA0000000
process 1 or context 0 has cacheable pages at 0XA0000000
process 2 or context 1 has non cacheable pages at 0XA0000000
...
//--------------------------------setting page table---------------------------------------------------
	...
	uint32_t process2_PTE = ((0xA0<<12)<<8) | 0x0E ;//process 2 or context 1 has non cacheable pages at 0XA0000000
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	...
	__asm__ __volatile__("mov  0x25, %l2");
	__asm__ __volatile__("set 0xA0000000, %l3 \n\t");	
	__asm__ __volatile__("sta %l2,[%l3] 0x20");	
	__asm__ __volatile__("set 0x10000000, %l3 \n\t");
...
//-----------------------------------------END Test for AT=0--------------------------------------------------------
	...
	__asm__ __volatile__("mov  0x27, %l2");	
	__asm__ __volatile__("set 0xA0000000, %l3 \n\t");	
	__asm__ __volatile__("sta %l2,[%l3] 0x20");	
	__asm__ __volatile__("set 0x10000000, %l3 \n\t");
	__asm__ __volatile__("ld  [%l3], %g3");	
