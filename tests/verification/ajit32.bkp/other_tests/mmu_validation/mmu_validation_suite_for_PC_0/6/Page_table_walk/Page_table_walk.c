/*----------------------------------------------------------------------------
Filename:			Page_table_walk.c
Version:			1.00
Goal: 				Page table walk
Author:				Harshal Kalyane
Description:	
			Setup page table as explained in Page table setup section. Then Store PTE of
different three pages at each level (1 2 3).
Then read/write data from these pages or virtual addresses. If reading/writing is
done successfully then Page table walk is working correctly.
-----------------------------------------------------------------------------*/
...
//--------------------------------setting page table---------------------------------------------------
	...
	uint32_t process1_PTE_l2 = ((0xC0<<12)<<8) | 0x8E ;//0x8E
	process1_L2PageTable[0x01] = process1_PTE_l2;//----------------------------------------PTE at level2
	...
	// 1 0 0 110 10 = 0x9A user:No Acess. Super:Read/Execute.
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
	uint32_t process1_PTE = ((0xA0<<12)<<8) | 0x8E ;////----------------------------------------PTE at level3
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	j=(uint32_t*) 0x10000000; //store it at 1000 0000 address.
	*j=15;
	j=(uint32_t*) 0xC0000000; //store it at 1000 0000 address.
	*j=17;
	j=(uint32_t*) 0xD0000000; //store it at 1000 0000 address.
	*j=18;

	//======================================Test for AT=0,FT=0==========================================================
	//putString("\n----------------Test for Page_table_walk------------------------");
	//--------------------------------------MMU Enabled------------------------------------------------------------
	...
	__asm__ __volatile__("set 0xD0000000, %l3 \n\t");//level1
