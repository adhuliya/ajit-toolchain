// 
// A simple test case to control interrupts using the PIL
// field in the PSR.
//
// 
//  Main progam:
...
//make PIL 0 inside subroutine
void make_PIL_0()
{
	__asm__ __volatile__("restore;");
	__asm__ __volatile__("rd %psr, %g1;");
	__asm__ __volatile__("andn %g1, 0xF00, %g1;");
...
//Initialize all registers
	...
	//set PSR with PIL=15, ET=1 PS=1 S=1, all other fields=0
	__asm__ __volatile__("mov 0xFE0, %l0	;");
	...
	{
		// make PIL = 15
		__asm__ __volatile__("mov 0xFE0, %l0	;");   // interruptible
		__asm__ __volatile__("wr %l0, %psr 	;");   // interruptible
		...
		//__asm__ __volatile__("wr %l0, %psr 	;");
		__asm__ __volatile__("call make_PIL_0;");
		// interrupt possible now.
		__asm__ __volatile__("nop;");
		__asm__ __volatile__("nop;");

		__asm__ __volatile__("mov 0xF00, %g3;");
		__asm__ __volatile__("mov 0xF00, %g3;");
