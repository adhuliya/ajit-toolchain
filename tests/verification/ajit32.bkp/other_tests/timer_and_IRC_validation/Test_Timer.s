...
! 22 September 2015
! Run this test with the following settings in Ajit_Hardware_Configuration.h :

! #define USE_TIMER_MODEL 1
! 
! Address of the memory-mapper IRC control register :
! #define ADDR_INTERRUPT_CONTROLLER_CONTROL_REGISTER 0xFFFF3000
! #define TIMER_REGISTER_ADDRESS 0xFFFF3100
! #define TIMER_TICK_REAL_FREQUENCY       100

.set ADDR_INTERRUPT_CONTROLLER_CONTROL_REGISTER, 0xFFFF3000
.set ADDR_TIMER_CONTROL_REGISTER, 0xFFFF3100 

! The test program stays in an infinite loop.
! The Timer thread causes an interrupt (irl=10) after 
! a programmed delay (1 second)

! Upon receiving IRL=10, control should go to hardware trap
! handler number 0x1A. So expected g1=0x1A  
! Inside the trap handler, register g2 is incremented
! and the interrupt is lowered by reading the timer register.
! As soon as the number of interrupts = INTERRUPT_COUNT,
! exit program.
!
! number of timer interrupts is stored as INTERRUPT_COUNT
! So the program should run for about INTERRUPT_COUNT seconds.
.set INTERRUPT_COUNT, 5
...
_start:
	...
	mov 0xE0, %l0	; wr %l0, %psr ;nop;nop;nop	
	...
	mov 0xF01, %g3	!WATCHPOINT1
	
	
	!======================================
	! Enable interrupt controller by writing 
	! into its control register
	mov 0x1, %g5  ! enable bit=1
	set ADDR_INTERRUPT_CONTROLLER_CONTROL_REGISTER, %g6 ! Interrupt controller's control register address 
	sta %g5, [%g6] 0x20
	! write into interrupt controller register.
	!use alternate store with asi=MMU_bypass

	
	! Enable timer by writing into its control register
	! control word :
	! 	timer_enabled (bit 0) =1
	! 	timer_max_count (bits 31:1)=100
	! 	(delay = timer_max_count * 10 millisecond)

	mov 100, %g5  ! timer_max_count
	sll %g5,1, %g5
	or %g5, 0x1, %g5 ! enable bit
	set ADDR_TIMER_CONTROL_REGISTER, %g6 ! Timer register address 
	nop
	nop
	sta %g5, [%g6] 0x20
	! write into timer register.
	!use alternate store with asi=MMU_bypass


	mov 0xF02, %g3	!WATCHPOINT

	! stay in a loop waiting for 
	! timer interrupt (level 10).
	!
	! Inside interrupt handler, increment g2,
	! reset the timer, and keep waiting for the next interrupt.
	! As soon as the number of interrupts = INTERRUPT_COUNT,
	! exit program.
	!
	! number of timer interrupts is stored in %g2
	nop
	nop
	nop
	...
	label1:
		...
		nop
		nop
		cmp %g2, INTERRUPT_COUNT
		bl label1
		nop
		nop
		nop
		mov 0xFFF, %g3	!WATCHPOINT
	exit_test:
		ta 0
		nop
	!======================================
...
HW_trap_0x1a: 
		...
		mov 0xF03, %g3	!WATCHPOINT
		! **** NOTE usually the trap handler can be only
		! 4 words long, but since we're not using any other traps,
		! we're making this handler longer than 4 words.
		!
		!
		!======Timer 0 Interrupt Handler
		inc %g2; 
		!copy timer count in g7
		ld [%g6], %g7;
		...
		set ADDR_INTERRUPT_CONTROLLER_CONTROL_REGISTER, %g6 ! IRC register address 
		sta %g0, [%g6] 0x20
		
		mov 0xF04, %g3	!WATCHPOINT

		! after INTERRUPT_COUNT calls to this routine,
		! keep the timer disabled
		cmp %g2, INTERRUPT_COUNT
		bge return_from_trap_handler
		nop
		...
		or %g5, 0x1, %g5 ! enable bit
		set ADDR_TIMER_CONTROL_REGISTER, %g6 ! Timer register address 
		sta %g5, [%g6] 0x20 !write timer control word

		mov 0xF05, %g3	!WATCHPOINT
		
		!Enable Interrupt controller
		mov 0x1, %g5  ! enable bit=1
		set ADDR_INTERRUPT_CONTROLLER_CONTROL_REGISTER, %g6 
		sta %g5, [%g6] 0x20 ! write interrupt controller control word
				
			
		mov 0xF06, %g3	!WATCHPOINT
		!go back
