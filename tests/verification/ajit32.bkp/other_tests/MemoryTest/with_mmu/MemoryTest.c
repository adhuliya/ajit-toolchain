...
uint32_t ContextTable __attribute__((aligned(0x4)));
uint32_t access_array[ORDER]  __attribute__((aligned(0x1000)));

int  __attribute__((section(".text.main")))  startTest()
{


	ContextTable = (0x1 << 7) | (0x3 << 2) | 0x2; // map VA to PA. all pages cacheable.

	// set up level 0 page table.
	uint32_t ctaddr = (uint32_t) &ContextTable;
	//uint32_t context_table_ptr_value = convertPhyAddrToPTD(ctaddr);
	uint32_t context_table_ptr_value = ((ctaddr >> 6) << 2) | 0x1;
