...
main:
_start:
	! Initialize PSR, enable traps and FP unit
	...
	!
	! instruction = FBGE (Branch on Greater or Equal) (G or E)
	...
	! -7.105427E-15 = a7ffffff
	set A, %g2   	! move  address of -7.105427E-15 into g2

	! 1.0842021E-19 = 1fffffff
	set B, %g3   	! move  address of 1.0842021E-19 into g3

	! NaN = 0x7fffffff
	set C, %g4	! move  address of NaN into g4


	ld [%r0 + %g2], %f0 	! load -7.105427E-15 into f0
	ld [%r0 + %g3], %f1 	! load 1.0842021E-19 into f1
	ld [%r0 + %g4], %f2 	! load NaN into f2


	fcmps %f0, %f1  ! compare f1, f0 (less)
	fbge label1	! conditional branch that is *not* taken 
	mov 0xF00, %o0	! delay slot instruction is always executed
	mov 0x66, %o2	! control should reach here

label1:	
	fcmps %f1, %f0  ! compare f0, f1 (greater)
	fbge label2	! conditional branch that is taken
	mov 0xF00, %o5	! delay slot instruction is always executed
	mov 0x99, %o6	! control shouldn't reach here

label2:
	fcmps %f0, %f0  ! compare f0, f0 (equal)
	fbge label3	! conditional branch that is taken
	mov 0xF00, %i2	! delay slot instruction is always executed
	mov 0x55, %i3	! control shouldn't reach here

label3: 
	fcmps %f2, %f0  ! one is unordered
	fbge label4	! conditional branch that is *not* taken 
	mov 0xF00, %i4	! delay slot instruction is always executed
	mov 0xF0, %i5	! control should reach here

label4:
	fcmps %f2, %f2  ! both are unordered
	fbge label5	! conditional branch that is *not* taken 
	mov 0xF00, %i6	! delay slot instruction is always executed
	mov 0xF0, %i7	! control should reach here



label5:
	fcmps %f0, %f1  ! compare f1, f0 (less)
	fbge,a label6	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o1	! delay slot instruction is *not* executed (annulled)
	mov 0x77, %o3	! control reaches here because branch is not taken

label6:	
	fcmps %f1, %f0  ! compare f0, f1 (greater)
	fbge,a label7	! conditional ANNULLED branch that is taken
	mov 0xF00, %o7	! delay slot instruction is always executed
	mov 0x55, %o4	! control shouldn't reach here

label7:	
	fcmps %f1, %f1  ! compare f1, f1 (equal)
	fbge,a label8	! conditional ANNULLED branch that is taken
	mov 0xF00, %i0	! delay slot instruction is always executed
	mov 0x55, %i1	! control shouldn't reach here

label8:
	!fcmps %f1, %f2
	fcmps %f2, %f1  ! one is unordered
	fbge,a label9	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %l0	! delay slot instruction is *not* executed (annulled)
	mov 0x55, %l1	! control reaches here because branch is not taken

label9:
	fcmps %f2, %f2  ! both are unordered
	fbge,a label10	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %l2	! delay slot instruction is *not* executed (annulled)
	mov 0x55, %l3	! control reaches here because branch is not taken
...
not_reached:
	...
	.align 4
	B: .single 0r1.0842021E-19
	A: .single 0r-7.105427E-15
	C: .single 0rNaN
