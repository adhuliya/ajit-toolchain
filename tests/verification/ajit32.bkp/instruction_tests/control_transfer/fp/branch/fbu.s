...
_start:
	...
	!
	! instruction = FBU (U) (Branch on Unordered)
	...
	fbu label1	! conditional branch that is *not* taken 
...
label1:	
	...
	fbu label2	! conditional branch that is *not* taken
...
label2:
	...
	fbu label3	! conditional branch that is *not* taken
	mov 0xF00, %i2	! delay slot instruction is always executed
	mov 0x55, %i3	! control should reach here

label3: 
	fcmps %f2, %f0  ! one is unordered
	fbu label4	! conditional branch that is taken 
	mov 0xF00, %i4	! delay slot instruction is always executed
	mov 0xF0, %i5	! control shouldn't reach here

label4:
	fcmps %f2, %f2  ! both are unordered
	fbu label5	! conditional branch that is taken 
	mov 0xF00, %i6	! delay slot instruction is always executed
	mov 0xF0, %i7	! control shouldn't reach here



label5:
	fcmps %f0, %f0  ! compare f0, f0 (equal)
	fbu,a label6	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o1	! delay slot instruction is *not* executed (annulled)
	mov 0x77, %o3	! control reaches here because branch is not taken

label6:	
	fcmps %f0, %f1  ! compare f0, f1 (less)
	fbu,a label7	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o7	! delay slot instruction is *not* executed (annulled)
	mov 0x55, %o4	! control reaches here because branch is not taken

label7:	
	fcmps %f1, %f0  ! compare f0, f1 (greater)
	fbu,a label8	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %i0	! delay slot instruction is *not* executed (annulled)
	mov 0x55, %i1	! control reaches here because branch is not taken

label8:
	fcmps %f1, %f2
	!fcmps %f2, %f1  ! one is unordered
	fbu,a label9	! conditional ANNULLED branch that is taken
	mov 0xF00, %l0	! delay slot instruction is always executed
	mov 0x55, %l1	! control shouldn't reach here

label9:
	fcmps %f2, %f2  ! both are unordered
	fbu,a label10	! conditional ANNULLED branch that is taken
	mov 0xF00, %l2	! delay slot instruction is always executed
	mov 0x55, %l3	! control shouldn't reach here
