...
_start:
	...
	fbo label1	! conditional branch that is taken 
	mov 0xF00, %o0	! delay slot instruction is always executed
	mov 0xBAD, %o2	! control shouldn't reach here

label1:	
	fcmps %f0, %f1  ! compare f0, f1 (less)
	fbo label2	! conditional branch that is taken
...
label2:
	...
	fbo label3	! conditional branch that is taken
...
label3: 
	...
	fbo label4	! conditional branch that is *not* taken 
	mov 0xF00, %i4	! delay slot instruction is always executed
	mov 0xF0, %i5	! control reaches here because branch is not taken

label4:
	fcmps %f2, %f2  ! both are unordered
	fbo label5	! conditional branch that is *not* taken 
	mov 0xF00, %i6	! delay slot instruction is always executed
	mov 0xF0, %i7	! control reaches here because branch is not taken



label5:
	fcmps %f1, %f0  ! compare f1, f0 (greater)
	fbo,a label6	! conditional ANNULLED branch that is taken
	mov 0xF00, %o1	! delay slot instruction is always executed
	mov 0xBAD, %o3	! control shouldn't reach here

label6:	
	fcmps %f0, %f1  ! compare f0, f1 (less)
	fbo,a label7	! conditional ANNULLED branch that is taken
	mov 0xF00, %o7	! delay slot instruction is always executed (annulled)
	mov 0x55, %o4	! control shouldn't reach here

label7:	
	fcmps %f1, %f1  ! compare f1, f1 (equal)
	fbo,a label8	! conditional ANNULLED branch that is taken
