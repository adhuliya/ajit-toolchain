...
_start:
	...
	!
	! instruction = BE (Branch on Equal) (Z)
	...
	cmp %l0, %l0	! Z set
	be label1	! conditional branch that is taken
...
label1:
	set 0x3FFFFFFF,%l0
	set 0x8fffffff,%l1
	cmp %l0, %l1	! not Z, greater, overflow from positive to negative side
	be label2	! conditional branch that is *not* taken
	mov 0xF00, %o1	! delay slot instruction is always executed 
	mov 0x99, %o5	! control reaches here because branch is not taken

label2: 
	cmp %l1, %l0	! not Z, less, overflow from negative to positive side
	be label3	! conditional branch that is *not* taken
...
label3:
	set 0x3FFFFFFF,%l0
	cmp %l0, %l0	! Z set
	be,a label4	! conditional ANNULLED branch that is taken
...
label4: 
	...
	set 0x8fffffff,%l1
	cmp %l0, %l1	! not Z, greater, overflow from positive to negative side
	be,a label5	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o3	! delay slot instruction is *not* executed 
	mov 0x88, %o7	! control reaches here because branch is not taken

label5:
	cmp %l1, %l0	! not Z, less, overflow from negative to positive side
	be,a label6	! conditional ANNULLED branch that is *not* taken
