...
_start:
	...
	!
	! instruction = BGE - not(N xor V) (Branch on Greater or Equal) (= N xnor V)
	...
	cmp %l1, %l0	! (overflow and negative flags)	
	bge label1	! conditional branch that is taken
	mov 0xF00, %o0	! delay slot instruction is always executed 
	mov 0xBAD, %o4	! control shouldn't reach here

label1:			
	cmp %l0, %l1	! (overflow flag)
	bge label2	! conditional branch that is *not* taken
	mov 0xF00, %o1	! delay slot instruction is always executed 
	mov 0x99, %o5	! control reaches here because branch is not taken

label2: 
	cmp %l1, %l0	! (overflow and negative flags)
	bge,a label3	! conditional ANNULLED branch that is taken
	mov 0xF00, %o2	! delay slot instruction is always executed 
	mov 0xBAD, %o6	! control shouldn't reach here

label3: 	
	cmp %l0, %l1	! (overflow flag)
	bge,a label4	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o3	! delay slot instruction is *not* executed 
	mov 0x88, %o7	! control reaches here because branch is not taken
...
label4:
	...
	cmp %l1, %l0	! (neither of the flags set)
	bge label5	! conditional branch that is taken
	mov 0xF00, %i0	! delay slot instruction is always executed 
	mov 0xBAD, %i4	! control shouldn't reach here

label5:
	cmp %l0, %l1	! (negative flag)
	bge label6	! conditional branch that is *not* taken
	mov 0xF00, %i1	! delay slot instruction is always executed 
	mov 0x99, %i5	! control reaches here because branch is not taken

label6: 
	cmp %l1, %l0	! (neither of the flags set)
	bge,a label7	! conditional ANNULLED branch that is taken
	mov 0xF00, %i2	! delay slot instruction is always executed 
	mov 0xBAD, %i6	! control shouldn't reach here

label7: 
	cmp %l0, %l1	! (negative flag)
	bge,a label8	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %i3	! delay slot instruction is *not* executed 
	mov 0x88, %i7	! control reaches here because branch is not taken

label8:
	cmp %l1, %l1	! (Zero flag set) (equal)
	ble label9	! conditional branch that is taken
	mov 0xF00, %l4	! delay slot instruction is always executed 
	mov 0xBAD, %l5	! control shouldn't reach here

label9: 
	cmp %l1, %l1	! (Zero flag set) (equal)
	ble,a label10	! conditional ANNULLED branch that is taken
	mov 0xF00, %l6	! delay slot instruction is always executed 
	mov 0x88, %l7	! control shouldn't reach here
