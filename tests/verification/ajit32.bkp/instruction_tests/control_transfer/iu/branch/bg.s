...
_start:
	...
	!
	! instruction = BG - not (Z or (N xor V)) (Branch on Greater) (= Z_bar(N xnor V))
...
label8:
	...
	bl label9	! conditional branch that is *not* taken
	mov 0xF00, %l4	! delay slot instruction is always executed 
	mov 0x56, %l5	! control should reach here

label9: 
	cmp %l1, %l1	! (Zero flag set)
	bl,a label10	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %l6	! delay slot instruction is *not* executed 
	mov 0x88, %l7	! control should reach here
