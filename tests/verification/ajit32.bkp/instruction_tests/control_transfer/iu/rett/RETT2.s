! Test for instruction: RETT (Test privileged instruction trap) 
...
_start:
	...
	! instruction = RETT
	!
	! Test privileged instruction trap (ET=1, S=0, PS=1, CWP=0)
	mov 0x60, %l0	
	wr %l0, %psr
	nop;nop;nop
	...
	set label2, %l0
	rett %l0	!causes privileged instr trap
	mov 0xDEF, %g4	!<--- NOT executed because rett traps.
			! Expected g4=0x0

	! Expected g1=0x3 (set in privileged_instruction trap handler)	
