...
_start:
	...
	! instruction = RETT
	!
	! First make ET=0, PS=1, S=1, CWP=0,
	! so that RETT does not cause any traps.
	mov 0xC0, %l0	
	wr %l0, %psr
	nop;nop;nop
	...
	set label2, %l0
	rett %l0
	mov 0xDEF, %g4	!delay slot of rett should be executed
			! Expected g4=0xDEF

label1 : mov 0x123, %g3 ! control shouldn't reach here
	 mov 0x456, %g4
	 nop
	 nop

label2 : rd %psr, %g5 ! Expected  g5=0xE1
	nop
	nop
