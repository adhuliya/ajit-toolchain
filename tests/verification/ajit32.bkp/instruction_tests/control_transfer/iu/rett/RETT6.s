! Test for instruction: RETT (Check if RETT causes mem_address_not_aligned trap and goes to error state) 
...
_start:
	...
	! test rett
	set trap_table_base, %g5
	mov 0xABC, %g3  ! Expected g3=0xABC
	set label2, %l0
	inc %l0
	rett %l0	! sets mem_addr_unaligned trap and goes to error mode
	mov 0xDEF, %g4	!<--- NOT executed because rett traps.
			! Expected g4=0x0

	! Upon entering error condition:
	! trap_table_base = 0x1000
	! tt=0x7
	! Expected TBR=0xf0005070



label2 : mov 0x123, %g3 ! control shouldn't reach here
	 mov 0x456, %g4
	 nop
	 nop
