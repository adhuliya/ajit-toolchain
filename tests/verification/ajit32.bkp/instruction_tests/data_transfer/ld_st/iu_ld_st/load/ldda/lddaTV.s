! Instruction being verified : ldda
! Author : Titto Thomas
! 30 August 2015
!
...
_start:
	...
	set data_A, %o3
	ldda [%o3]0x20, %l3  		! should illegal_instruction trap

	!load the second number and the adjacent one
	set data_B, %o1
	ldda [%o1]0x20, %l0  		! should mem_not_aligned_trap

	! Make the supervisor bit S=0
	rd %psr, %l1
	set 0x07F, %l2
	and %l1, %l2, %l1	
	wr %l1, %psr
	...
	!load the second number and the adjacent one
	set data_C, %o2
	ldda [%o2]0x20, %l4  		! should give a previliged_instruction trap
...
HW_trap_0x02: inc %g2; rett %r18; nop; nop	!ILLEGAL INSTRUCTION TRAP : increment g2 and return
HW_trap_0x03: inc %g2; rett %r18; nop; nop	! PREVILIGED INSTRUCTION TRAP : increment g2 and return
...
HW_trap_0x07: inc %g2; rett %r18; nop; nop	!MEM NOT ALIGNED TRAP : increment g2 and return
