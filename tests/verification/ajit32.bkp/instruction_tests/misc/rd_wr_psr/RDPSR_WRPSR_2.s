...
_start:
	...
	!
	! instructions = WRPSR, RDPSR
	! note : two operands are xor'd and result is placed in psr
	...
	nop
	nop
	rd %psr, %o0		! Expected o0=0xAAAA00E0
	...
	nop
	nop
	rd %psr, %o1		! Expected o0=0xFFFF00E0
	...
	!check if illegal instruction trap occurs when 
	! result[4:0]>=NWINDOWS
	! set PSR with ET=1 PS=1 S=1, CWP = 16
	mov 0xF0, %l0	
	wr %l0, %psr	! should cause illegal instr trap
	nop	
	nop	
	nop	
	rd %psr, %o3		! o3=0xFFFF00E0


	!check if privileged instruction trap occurs when S=0
	! set PSR with ET=1 PS=1 S=0, all other fields=0
	mov 0x60, %l0	
	wr %l0, %psr
	nop	
	nop	
	...
	rd %psr, %o4		! Should cause a trap. o4=0x00000000
...
HW_trap_0x02: inc %g3; rett %r18; nop; nop	!ILLEGAL INSTRUCTION TRAP :inc g3 and return
HW_trap_0x03: inc %g2; rett %r18; nop; nop	!PRIVILEGED INSTRUCTION TRAP : increment g2 and return
