...
!
! note: sNan in double-precision is 0x7ff80000 00000000
!  (MPD)
...
_start:
	...
	ldd [%l0], %f0		! loads 4.3[0x401133333333333] to f0-f1 pair
	ldd [%l1], %f2		! loads 8.2[0x4020666666666666] to f2-f3 pair
	fdivd %f0, %f2, %f4	! f4-f5 = f0-f1 / ff2-f3 =>[0x4029000000000000]
	rd %psr, %o1		! Expected o3=0x10E0

	
	!add negative and positive
	set C, %l2		! move  address of -1.5 into l2
	ldd [%l2], %f6		! loads -1.5 to f6-f7 (0xBFF8000000000000)
	fdivd %f0, %f6, %f8	! f8-f9 = f0-f1 / f6-f7 => [0x40333333]
	fmuld %f8, %f6, %f10
	...
	set E, %l4		! move  address of NaN into l4
	set 0x4, %o0		! to store the fsr value
	ld [%l3], %f12		! loads NaN to f10
	ld [%l4], %f13		! loads NaN to f11
	fdivd %f6, %f12, %f16	! f16-f17: 0x0
	st %fsr, [%o0] 		! Expected FSR: 0x00004000
	rd %psr, %o2		! Expected o3=0x10E0
...
not_reached:
	...
	.align 8
        ...
        B: .double 0r8.2
        C: .double 0r-1.5
	D: .word 0x7FF10000	! 0x7FF100000000000 = sNaN
	E: .word 0x00000000
 	mFSR: .word 0x0
 	mFSRN: .word 0x0
