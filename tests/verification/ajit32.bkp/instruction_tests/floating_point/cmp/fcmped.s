...
_start:
	...
	ldd [%l0], %f0		! loads 7.6 to f0-f1 pair
	ldd [%l1], %f2		! loads 3.1 to f2-f3 pair
	fcmped %f0, %f2		! freg1 > freg2 => fcc=2
	set 0x00, %o4		! o4 = 0x00
	st %fsr, [%o4]	 	! m[0x0] = 0x
	rd %psr, %o1		! Expected o3=0x10E0

	fcmped %f2, %f0		! freg1 < freg2 => fcc=1
	st %fsr, [%o4 + 4]	! m[0x4] = 0x
	rd %psr, %o2		! Expected o3=0x10E0

	set C, %l2		! move  address of 7.6 into l2
	ldd [%l2], %f4		! loads 7.6 to f4-f5 pair
	fcmpd %f4, %f0		! freg1 = freg2 => fcc=0
	st %fsr, [%o4 + 8]	! m[0x4] = 0x
