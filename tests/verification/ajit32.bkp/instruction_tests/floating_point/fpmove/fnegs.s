...
_start:
	...
	!
	! instruction = fmovs, fnegs, fabss
	!  souce and desination regs are different.


	! Initially loading values to f regs
	...
	! single-precison negation(absolute) is perfomed by 
	! simply fnegs(fabss) insturction
	fnegs %f0, %f2		! f2=-6.7
	fnegs %f1, %f3		! f3=2.4
	rd %psr, %o1		! Expected o3=0x10E0

	! Loading Double precision values
	set C, %l2
	set D, %l3
	ldd [%l2], %f4
	ldd [%l3], %f6
	
	! If source and destination registers are diffrent and
	! a double-precison negation(absolute value) is performed by
	! an fnegs(fabss) and followed by a fmovs instruction
	fnegs %f4, %f8		! f8-f9 = -5.5
	fmovs %f5, %f9
	fnegs %f6, %f10		! f10-f11 = 3.9
	fmovs %f7, %f11
	rd %psr, %o2		! Expected o3=0x10E0

	! Source and destination are same f-register
	ld [%l0], %f12
	ld [%l1], %f13
	fnegs %f12, %f12
	fnegs %f13, %f13
	...
	! If source and destination are same single FNEGS(FABSs)
	!  instruction performs for any precision of operand
