! Author : Madhav Desai
!
! Invalid op combinations, but trap is masked off.
! Only the FSR values will be updated.
!
...
_start:
	...
	mov 0xBAD, %g1
	set I, %l0		! move  address of I into l0
	ld [%l0], %f7		! loads deadfeed into f7.

	!
	! FPU-FSR will have NVM = 0, so no traps will be reported!
	!



	!======================================
	! Excite invalid sqrt,add,mul,div operations.
	!

	! square-root of negative number.
	set A, %l0		! move  address of A into l0
	set mFSR, %l1
	ld [%l0], %f0		! loads A to f0
	fnegs %f0, %f1          ! negate the number
	fsqrts %f1, %f7		! f7 = sqrt(f1) should cause invalid-op exception.
	rd %psr, %o1		! Expected o1=0x10E0
        st %fsr, [%l1]		! l1 = fsr
	ld [%l1], %o2		! o2 = 0x00000210
	...
	fsubs %f1, %f1, %f2     ! f2 = 0
        st %fsr, [%l1]		! l1 = fsr
	ld [%l1], %o3		! o3 = 0x00000200
        fdivs %f2, %f2, %f7     ! 0/0 should cause invalid-op exception.
        st %fsr, [%l1]		! mFSR = fsr
	ld [%l1], %o4		! o4 = 0x00000210

	!
	! divide infty/infty
	!
	set F, %l0		! address of F
	ld [%l0], %f0		! loads pos-infty into f0
	fdivs %f0, %f0, %f7     ! should cause invalid-op exception.
        st %fsr, [%l1]		! mFSR = fsr
	ld [%l1], %o5		! o5 = 0x00000210

	! 
	! multiply infty X 0
	!
	set G, %l0
	ld [%l0], %f1		! loads 0 into f1
	fmuls %f0, %f1, %f7	! should cause invalid-op exception
        st %fsr, [%l1]		! mFSR = fsr
	ld [%l1], %o6		! o6 = 0x00000210
	...
	set F, %l0		! address of F
	ld [%l0], %f0		! loads pos-infty into f0
	set E, %l0		! address of E
	ld [%l0], %f1		! loads neg-infty into f0
	fadds %f0, %f1, %f7     ! should cause invalid-op exception.
	fsubs %f0, %f0, %f7     ! infty-infty, should cause invalid-op exception.
	fsubs %f1, %f1, %f7     ! -infty - (-infty), should cause invalid-op exception.
        st %fsr, [%l1]		! mFSR = fsr
	ld [%l1], %o7		! o7 = 0x00000210

	! conversion from snan/qnan to int
	set D, %l0		! address of D
	ld [%l0], %f0		! loads snan into f0
	fstoi %f0, %f7          ! invalid operation ... conversion of snan to int.
        st %fsr, [%l1]		! mFSR = fsr
	ld [%l1], %l5		! l5 = 0x00000210
	fcmps %f0, %f0          ! invalid operation..
        st %fsr, [%l1]		! mFSR = fsr
	ld [%l1], %l6		! l6 = 0x00000e10  (fcc will be updated)
...
not_reached:
	...
	E: .word 0xff800000 ! neg-infty
	F: .word 0x7f800000 ! pos-infty
        G: .word 0x00000000 ! pos-zero
        H: .word 0x80000000 ! neg-zero
	I: .word 0xdeadfeed ! special..
 	mFSR: .word 0x0
 	mFSRN: .word 0x0
