...
_start:
	...
	set 0xA0, %l1
	set 0xB0, %l2
	st %l1, [%o0 + 1]		! generates mem_not_aligned trap
	st %l2, [%o0 + 4]		! m[0x8] = 0xB0

	!store the fourth number and then the previous one
	set 0x10, %o1
	set 0xC0, %l3
	set 0xD0, %l4
	st %l3, [%o1 - 3]		! generates mem_not_aligned trap
	st %l4, [%o1 - 4]		! m[0x0C] = 0xD0
