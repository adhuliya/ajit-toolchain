...
! NOTE : Run this test with a processor model with cache enabled and Dummy MMU (which makes all data cacheable).
! if real MMU is used, the MMU disables caching of all loads/stores by default.
