...
! On normal exit a test should end with 'ta 0' causing g1=0x80.
...
_start:
	...
	!
	! instruction = TADDccTV
	!
	! psr FLAG fields :
	! 31:24	 23 22 21 20  19:8   7:0
	!  0x00  N  Z  V  C   0x000  0xE0
	! 
	! NOTE: the tag overflow handler increments register g2
	! and returns to the instruction following the trapped one.
	! g2 keeps count of how many times the trap occured.


	
	!taddcctv two positive numbers, CAUSES tag_overflow
	set 0x2, %l0
	taddcctv %l0, 0x3, %o0	! Expected o0=0x00000000
	rd %psr, %o1		! Expected o1=0x000000E0

	
	!taddcctv two negative numbers, NO tag_overflow
	set -0x20, %l0
	set -0x30, %l1
	taddcctv %l0, %l1, %o2	! Expected o2=0xFFFFFFB0
	rd %psr, %o3		! Expected o3=0x009000E0

	
	
	!taddcctv negative and positive, result is positive, CAUSES tag_overflow
	set -0x2, %l0
	set  0x3, %l1
	taddcctv %l0, %l1, %o4	! Expected o4=0x00000000
	rd %psr, %o5		! Expected o5=0x009000E0
	
	
	!taddcctv negative and positive, result is zero, CAUSES tag_overflow
	set 0x3, %l0
	set -0x3, %l1
	taddcctv %l0, %l1, %i0	! Expected i0=0x00000000
	rd %psr, %i1		! Expected l1=0x009000E0


	!taddcctv two large positive numbers to 
	!create a negative result and an overflow. CAUSES tag_overflow
	set 0x7FFFFFFF, %l0
	set 0x7FFFFFFF, %l1
	taddcctv %l0, %l1, %i2	! Expected i2=0x00000000
	rd %psr, %i3		! Expected i3=0x009000E0
	
	!taddcctv two large negative numbers to 
	!create a positive result and an overflow CAUSES tag_overflow
	set -0x7FFFFFFF, %l0
	set -0x7FFFFFFF, %l1
	taddcctv %l0, %l1, %i4	! Expected i4=0x00000000
	rd %psr, %i5		! Expected i5=0x009000E0

	!taddcctv two positive numbers, NO tag_overflow
	set 0x30, %l0
	set 0x20, %l1
        set 0x00, %i6
        set 0x00, %i7
	taddcctv %l0, %l1, %i6	! Expected i6=0x50
	rd %psr, %i7		! Expected i7=0x000000E0
...
HW_trap_0x0a: inc %g2; rett %r18; nop; nop	!TAG OVERFLOW TRAP : increment g2 and return
