...
_start:
	...
	!
	! instruction = BCS (Less than, Unsigned)
...
label4: 
	...
	cmp %l0, %l1
	bcs label5	! conditional branch that is *not* taken
	mov 0xF00, %i0	! delay slot instruction is always executed 
	mov 0x98, %i1	! control shouldn reach here

label5:
	cmp %l1, %l0	
	bcs,a label6	! conditional ANNULLED branch that is *not* taken
