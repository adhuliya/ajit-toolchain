...
_start:
	...
	!
	! instruction = BNEG (Branch on Negative) (N)
	...
	cmp %l1, %l0	! N set, positive side overflows and goes to negative
	bneg label3	! conditional branch that is taken
	mov 0xF00, %o0	! delay slot instruction is always executed 
	mov 0xBAD, %o4	! control shouldn't reach here

label3:
	cmp %l0, %l1	! N not set, negative side overflows and goes to positive
	bneg label4	! conditional branch that is *not* taken
	mov 0xF00, %o1	! delay slot instruction is always executed 
	mov 0x99, %o5	! control reaches here because branch is not taken

label4: 
	cmp %l1, %l0	! N set, positive side overflows and goes to negative
	bneg,a label5	! conditional ANNULLED branch that is taken
	mov 0xF00, %o2	! delay slot instruction is always executed 
	mov 0xBAD, %o6	! control shouldn't reach here

label5: 
	cmp %l0, %l1	! N not set, negative side overflows and goes to positive
	bneg,a label6	! conditional ANNULLED branch that is *not* taken
...
label6:
	...
	bneg label7	! conditional branch that is *not* taken
	mov 0xF00, %l4	! delay slot instruction is always executed 
	mov 0x56, %l5	! control should reach here

label7: 
	cmp %l1, %l1	! (Zero flag set)
	bneg,a label8	! conditional ANNULLED branch that is *not* taken
