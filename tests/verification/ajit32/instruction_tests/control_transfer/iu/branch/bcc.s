...
_start:
	...
	!
	! instruction = BCC (Greater than or Equal, Unsigned)
...
label4: 
	...
	cmp %l0, %l1
	bcc label5	! conditional branch that is taken
	mov 0xF00, %i0	! delay slot instruction is always executed 
	mov 0xBAD, %i1	! control shouldn't reach here

label5:
	cmp %l1, %l0	
	bcc,a label6	! conditional ANNULLED branch that is taken
	mov 0xF00, %i2	! delay slot instruction is always executed 
	mov 0xBAD, %i3	! control shouldn't reach here
