...
_start:
	...
	!
	! instruction = BVC (Branch on Overflow Clear) (not V)
	...
	cmp %l0, %l1	! no overflow
	bvc label1	! conditional branch that is taken
...
label1:
	set 0x3FFFFFFF,%l0
	set 0x8fffffff,%l1
	cmp %l0, %l1	! overflow from positive to negative side
	bvc label2	! conditional branch that is *not* taken
	mov 0xF00, %o1	! delay slot instruction is always executed 
	mov 0x99, %o5	! control reaches here because branch is not taken

label2: 
	cmp %l1, %l0	! overflow from negative to positive side
	bvc label3	! conditional branch that is *not* taken
	mov 0xF00, %l4	! delay slot instruction is always executed 
	mov 0x56, %l5	! control reaches here because branch is not taken

label3:
	mov 0x3, %l0	
	mov 0x1, %l1
	cmp %l0, %l1	! no overflow
	bvc,a label4	! conditional ANNULLED branch that is taken
...
label4: 
	...
	set 0x8fffffff,%l1
	cmp %l0, %l1	! overflow from positive to negative side
	bvc,a label5	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o3	! delay slot instruction is *not* executed 
	mov 0x88, %o7	! control reaches here because branch is not taken

label5:
	cmp %l1, %l0	! overflow from negative to positive side
	bvc,a label6	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %l6	! delay slot instruction is *not* executed 
	mov 0x88, %l7	! control reaches here because branch is not taken
