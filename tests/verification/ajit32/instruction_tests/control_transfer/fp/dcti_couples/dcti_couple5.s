...
!
! instruction = dcti couple : 
! first branch is FBA, a=1 (annulled), second branch any CTI	
...
_start:
	...
	fba,a label1	! branch always, annulled
	fbe label2	! conditional branch that should have been taken
			! but is not taken, it is annulled


	mov 0xF00, %g3	! delay slot instruction is never executed 

label1:
	add %o1, 0x3, %o0	! control reaches here because branch (fba) is taken
				! Expected o0=0x00000005
	mov 0x88, %g4		! control reaches here because fbe is annulled


label2: add %o0, 0x3, %o1	! Expected o1=0x00000008

	
			
	fba,a label3	! branch always, annulled
	fbl label4	! conditional branch that should *not* have been taken
			! and is not taken, because it is annulled


	mov 0xF00, %g7	! delay slot instruction is never executed 

label3:
	add %o3, 0x3, %o2	! control reaches here because branch (fba) is taken
				! Expected o2=0x00000005
	mov 0x88, %g6		! control reaches here


label4: add %o2, 0x3, %o3	! Expected o3=0x00000008
