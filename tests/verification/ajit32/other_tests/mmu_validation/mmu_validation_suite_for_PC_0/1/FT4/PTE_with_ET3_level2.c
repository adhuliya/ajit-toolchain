/*----------------------------------------------------------------------------
Filename:			PTE_with_ET3_level2.c
Version:			1.00
Goal: 				translation error for PTE with 3 at level2
Author:				Harshal Kalyane
Description:	
			A translation error is indicated if an external bus error occurs while the MMU is
fetching an entry from a page table, a PTD is found in a level-3 page table, or a
PTE has ET=3. It will generate fault type = 4.
line 243:uint32_t process1_PTE2 = ((0xA0<<12)<<8) | 0x8F ;//PTE with ET=3
-----------------------------------------------------------------------------*/
...
//--------------------------------setting page table---------------------------------------------------
	...
	uint32_t process1_PTE2 = ((0xA0<<12)<<8) | 0x8F ;//PTE with ET=3
	process1_L2PageTable[0x00] = process1_PTE2;
