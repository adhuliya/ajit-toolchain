/*----------------------------------------------------------------------------
Filename:			small_TLB.c
Version:			1.00
Goal: 				Fault Type=0 for Acess Type=0 and PTE.ACC=0
Author:				Harshal Kalyane
Description:
Note: Run this test case with TLB size of 1.
...
2.	Setup a context table, and page tables 
	...
	0x00011000 - 0x00012000 	0x00031000 - 0x00032000 with PTE.ACC=0

3.	Write a 0x12 using physical address(0x00031000)
3. 	Initial context table ptr, context register, and enable MMU.
4.	In context 0 read data using virtual addr(0x00011000) and load it in g7 register.
...
//--------------------------------setting page table---------------------------------------------------
	...
	uint32_t context_table_ptr_value = convertPhyAddrToPTD((uint32_t)ContextTable);//It will point to the context table in physical memory.
	...
	*/	
	...
	uint32_t process1_PTE = (0x00003100) | 0x8E ;//PTE.ACC=0
	//uint32_t process2_PTE = (0x00003100) | 0x8E ;
	//uint32_t process3_PTE = (0x00003100) | 0x8E ;
	process1_L3PageTable[0x00] = process_code_PTE;//program code is here
	process1_L3PageTable[0x11] = process1_PTE;
	//process2_L3PageTable[0x11] = process2_PTE;
	//process3_L3PageTable[0x11] = process3_PTE;

	process1_PTE = (0x00000100) | 0x8E ;//PTE.ACC=0
	process1_L3PageTable[0x01] = process1_PTE;
	process1_PTE = (0x00000200) | 0x8E ;//PTE.ACC=0
	process1_L3PageTable[0x02] = process1_PTE;
//------------------------------------------------------------------------------------------------------------------
	uint32_t *j=(uint32_t*) 0x00031000; //store it at 0x00031000 address.
	*j=12;
	...
	store_word_mmureg(0x0 , (uint32_t*) MMU_REG_CONTEXT);//context zero
	...
	__asm__ __volatile__("set 0x00011000, %l3 \n\t");
