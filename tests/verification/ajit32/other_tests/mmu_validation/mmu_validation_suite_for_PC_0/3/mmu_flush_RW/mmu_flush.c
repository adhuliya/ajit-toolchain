/*----------------------------------------------------------------------------
Filename:			mmu_flush.c
Version:			1.00
Goal: 				check working of MMU flush.
Author:				Harshal Kalyane
Description:	
			This test case mainly check working of MMU flush.Flushing leads to clearing
contains of TLB. First it stores two Test data in physical memory at address Y
and Z. Test program set-up page table such that logical address X map to physical
address Y then it reads data using logical address X. It should read Test data 1.
Before second read operation it disables MMU and change PTE such that it
maps logical address X to Z and then enables MMU.It should read Test data 1
because old PTE is in TLB cache.
It flushes TLB before 3rd read operation and again read from same logical
address. This time it should read Test Data 2, Because TLB contain new updated
PTE.

Note: this is a lousy test.  What if the implementation has no TLB?
The test will fail.  MPD
-----------------------------------------------------------------------------*/

//Version 2.0
//
//Test cleaned-up and simplified
// by: Neha Karanjkar
// 17 nov 2016

//LOGIC:
//	map VA=0x10000000-0x10ffffff to PA=0xA0000000-0xA0ffffff. 
//	Enable the mmu and check the mapping
//	by doing a read using the virtual address.
//
//	Now, change the mapping during runtime and flush the mmu.
//	map VA=0x10000000-0x10ffffff to PA=0xB0000000-0xB0ffffff. 
//	
//	Check if the new mapping has taken effect by performing
//	a read using the virtual address.
//
...
int  __attribute__((section(".text.main")))  startTest()
{
	...
	uint32_t *ptr_B=(uint32_t*) 0xB0000000; 
	...
	*ptr_VA= 0x10101010;
	...
	//Disable the Mmu and flush the mmu and caches
	MMU_control_word = 0x0;
	store_word_mmureg(MMU_control_word, (uint32_t*) MMU_REG_CONTROL);
	mmu_flush(0x0000, (uint32_t*)0x00000000);
	flush_icache();
	flush_dcache();
	mmu_flush(0x0000, (uint32_t*)0x00000000);
	
	
	//read the data at addr 0x10000000
	__asm__ __volatile__("set 0x10000000, %l3 \n\t");
	__asm__ __volatile__("ld  [%l3], %g6");  //g6 should be 0x10101010


	//now change the mapping
	//Map VA range 0x10000000 to 0x10ffffff to PA 0xB0000000-0xB0ffffff
	{
		uint32_t PTE_level1 = (0xB0<<20) | (0x8E);//all can read/write/execute this page
		process1_L1PageTable[0x10] = PTE_level1;
	}

	//Enable the Mmu and flush the mmu and caches
	MMU_control_word = 0x1;
	store_word_mmureg(MMU_control_word, (uint32_t*) MMU_REG_CONTROL);
	mmu_flush(0x0000, (uint32_t*)0x00000000);
	flush_icache();
	flush_dcache();
	...
	//read the data using virtual address.
	__asm__ __volatile__("set 0x10000000, %l3 \n\t");
	__asm__ __volatile__("ld  [%l3], %g7");  //g7 should be 0xB0B0B0B0
