/*----------------------------------------------------------------------------
Filename:			page_error_AT2_S4.c
Version:			1.00
Goal: 				Fault Type=0 for Acess Type=2 and PTE.ACC=4
...
2.	Setup a context table, and page tables 
	...
	For context 1:
	virtual address			phy address
	0x00001000 - 0x00001FFF 	0x00001000 - 0x00001FFF with PTE.ACC=4
	
3.	Try to execute instruction from Address Space = 0x00001xxx from user mode.
...
//--------------------------------setting page table---------------------------------------------------
	...
	uint32_t process3_PTE = ((0x00000000) | 0x8E);//0x8E
	...
	process2_PTE = ((0x00000100) | 0x92 );//change permission here PTE.ACC=4
	process2_L3PageTable[0x01] = process2_PTE;

	process2_PTE = ((0x00000200) | 0x8E );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x02] = process2_PTE;

	process2_PTE = ((0x00000300) | 0x8E );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x03] = process2_PTE;
//------------------------------------------------------------------------------------------------------------------
	 uint32_t *j=(uint32_t*) 0x00003100; //store it at A000 0000 address.
	*j=12;
	
	//======================================Test for AT=2,Serial no:4===========================================================
	...
	__asm__ (".align 4096");
	__asm__ __volatile__("set 0x00001100, %l4 ");
