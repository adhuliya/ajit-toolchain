#!/bin/sh
GLIBC=$AJIT_TOOLS_RELEASE/tools/glibc/2.21_sectioned
MAIN=sat_pos

genVmapAsm vmap.txt setup_page_tables.s

compileToSparc.py -o 3 -I GLIBC/include -I /usr/include -I ./ -s init.s -s setup_page_tables.s  -s $AJIT_TOOLS_RELEASE/tools/ajit_access_routines/asm/trap_handlers.s -I $AJIT_TOOLS_RELEASE/tools/ajit_access_routines/include -C $AJIT_TOOLS_RELEASE/tools/ajit_access_routines/src -C $AJIT_TOOLS_RELEASE/tools/minimal_printf_timer/src -I $AJIT_TOOLS_RELEASE/tools/minimal_printf_timer/include -c PVT.c -c ExtractData.c -c fpop.c -c math_func.c  -E ${MAIN}.elf -V ${MAIN}.vars -H ${MAIN}.hex -M ${MAIN}.mmap -O ${MAIN}.objdump  -L LinkerScript.lnk  -D AJIT -D HAS_FLOAT -G 2.21_sectioned -D NO_MEMCPY 
