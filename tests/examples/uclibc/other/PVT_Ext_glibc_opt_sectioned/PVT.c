...
///PVT Computation
/*	//z_debug
void    InitNavFrameOneParameters(int);			//z: not used
void    InitNavFrameTwoParameters(int);			//z: not used
int     VerifyIPData(int );				//z: not used
//void    NavExtractDataFrameMain(int[] , int);		//z?? //orig //Not needed 
uint8_t NavIdentifyFrameID(int []);			//z: not used
void    NavExtractDataFrameOne(int [] ,int);		//z: not used
void    NavExtractDataFrameTwo(int [], int );		//z: not used
void    NavExtractDataFrameThreeFour(int [], int );	//z: not used
void    NavExtractMsgNine (int [], int );		//z: not used
void    NavExtractMsgEleven (int [], int);		//z: not used
void    NavExtractMsgTwentySix(int [], int , int );	//z: not used
void    NavComputeSatCorrPos( int,int);			//z: not used
void    NavComputeUserCorrPosSixSat();			//z: not used
void    NavComputeUserPosSixSatNew();			//z: not used
*/
...
int Verify_NavIC_PVT_Data(int PR_Num)
{
	...
	for (i=0; i< NAVSATNO ; i++)
  	...
  	//----- Step 2: Check the First and second subframe data  Buffers
		...
		if(NavFlag==1) NavprocData.UTCAbsFlg= 1;	//Data not available
...
void NavIC_Init_Processed_Data( )
{
	...
	//z: NavprocData : datastructure defined in processorbuffer.h	
        ...
        // UTC Time
        NavprocData.UTCAbsFlg =1;		//z: Default state at initialization, NOT enough data for UTC
...
{
	//z: Stack Req. = 31*4 = 124bytes + (32bytes++ for tan_inverse)
...
void FindCoFactor(int f,double num[f][f],double fac[f][f] )	//z: for debug
//void FindCoFactor(int f,double num[4][4],double fac[4][4] )
{
...
//void FindMatrixInverse(int k, double mat4[4][4],double inv[4][4])
{
...
void NavComputeDOP(int kk,double mat[4][4])
{
...
void NavComputeUserVelocitySixSat()
{
	...
	*/
	...
	MatMultiplication(1,3,3,1,V1,I1,v1);	//z_debug checked all right
	MatMultiplication(1,3,3,1,V2,I2,v2);
	MatMultiplication(1,3,3,1,V3,I3,v3);
	MatMultiplication(1,3,3,1,V4,I4,v4);
	MatMultiplication(1,3,3,1,V5,I5,v6);
	MatMultiplication(1,3,3,1,V6,I6,v6);
	...
	FindMatrixInverse(4,alphasq,alphasqinv);
	MatMultiplication(4,4,4,6,alphasqinv,alphaT,alphamat);	//z_debug checked
	MatMultiplication(4,6,6,1,alphamat,rhodot,Vuser);	//z_debug checked
...
void NavComputeUserVelocityFourSat()
{
	...
	*/
	...
	MatMultiplication(1,3,3,1,V1,I1,v1);	//z_debug checked
	MatMultiplication(1,3,3,1,V2,I2,v2);
	MatMultiplication(1,3,3,1,V3,I3,v3);
	MatMultiplication(1,3,3,1,V4,I4,v4);
	MatMultiplication(1,3,3,1,V5,I5,v6);
	...
	MatMultiplication(4,6,4,1,alphamat,rhodot,Vuser);		
