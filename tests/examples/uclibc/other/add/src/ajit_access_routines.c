//#include <libc-symbols.h> //AD: newly added
#include <stdint.h>
#include <stdio.h>
...
//
inline uint32_t __ajit_read_spi_master_register__(uint8_t reg_id)
{
	uint32_t addr = ADDR_SPI_DATA_REGISTER_LOW + (reg_id*4);
	uint32_t ret_val = __ajit_load_word_mmu_bypass__(addr);
	return(ret_val);
...
{
        __asm__ __volatile__ ( "ldd [%0], %%f0; fsqrtd %%f0, %%f2; std %%f2, [%1];"
                                        : : "r" (a), "r" (b):);
}

inline void __ajit_fsqrts__ (uint32_t a, uint32_t b)
{
        __asm__ __volatile__ ( "ld [%0], %%f0; fsqrts %%f0, %%f1; st %%f1, [%1];"
                                        : : "r" (a), "r" (b):);
}

inline void __ajit_fitod__ (uint32_t a, uint32_t b)
{
        __asm__ __volatile__ ( "ld [%0], %%f0; fitod %%f0, %%f2; std %%f2, [%1];"
                                        : : "r" (a), "r" (b):);
}

inline void __ajit_fitos__ (uint32_t a, uint32_t b)
{
        __asm__ __volatile__ ( "ld [%0], %%f0; fitos %%f0, %%f1; st %%f1, [%1];"
                                        : : "r" (a), "r" (b):);
}

inline void __ajit_fdtoi__ (uint32_t a, uint32_t b)
{
        __asm__ __volatile__ ( "ldd [%0], %%f0; fdtoi %%f0, %%f2; st %%f2, [%1];"
                                        : : "r" (a), "r" (b):);
}

inline void __ajit_fstoi__ (uint32_t a, uint32_t b)
{
        __asm__ __volatile__ ( "ld [%0], %%f0; fstoi %%f0, %%f1; st %%f1, [%1];"
                                        : : "r" (a), "r" (b):);
}
