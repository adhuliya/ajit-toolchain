...
def setGlobals(ajit_project_home, ajitcc, ajit_glibc):
	...
	SPARC_CC_FLAGS =  " -ffunction-sections -S -Wall -Werror -m32 -mcpu=v8 -nostdlib -ffreestanding  "
...
def buildExecutable(work_area, link_sections, obj_files, linker_script_file, elf_file, hex_dump_file,variable_dump_file,memmap_file, objdump_file, glibc_version):
    ...
    if (glibc_version != ""):
      BUILDROOT_LIBS = "/home/anshuman/buildroot-sparc-new-cpp/buildroot-2014.08/output/host/usr/sparc-buildroot-linux-uclibc/sysroot/usr/lib"
      BUILDROOT_LIBGCC = "/home/anshuman/buildroot-sparc-new-cpp/buildroot-2014.08/output/build/host-gcc-final-4.7.4/build/sparc-buildroot-linux-uclibc/libgcc" 

      sparc_ld_command += " -L" + BUILDROOT_LIBS
      sparc_ld_command += " -L" + BUILDROOT_LIBGCC
      sparc_ld_command += " -static -lm -lc "
      sparc_ld_command += " -lgcc "
      # the sectioned glibc should have "sectioned" in the version name
      if glibc_version.find("sectioned") >= 0:
...
def parseOptions(opts):
    ...
    return help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings , debug_mode, opt_level, link_sections, compile_options, glibc_version
...
def main():
    ...
    arg_list = sys.argv[1:]
    opts,args = getopt.getopt(arg_list,'W:L:C:c:S:s:I:D:E:H:V:M:O:hgo:T:F:G:')
    help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings, debug_mode, opt_level,link_sections,compiler_options,glibc_version  = parseOptions(opts)
