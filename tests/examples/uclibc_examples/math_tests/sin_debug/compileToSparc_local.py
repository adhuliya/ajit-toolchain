...
#	(modified by : Anshuman Dhuliya [AD] (1 Feb 2019)
#     added glibc support)
...
def setGlobals(ajit_project_home, ajitcc):
	...
	SPARC_LD_FLAGS = " -l gcc_s -L " + ajit_project_home + "/tools/cross-compiler/buildroot_cross_compiler/usr/sparc-buildroot-linux-uclibc/lib/"
  if (glibc_version):
    SPARC_LD_FLAGS += " -L" + AJIT_PROJECT_HOME + "/glibc/" + glibc_version + "/lib"
    SPARC_LD_FLAGS += " -static -lm -lc -lgcc "
    # SPARC_LD_FLAGS += " -L/home/zeal/AjitRepoV2_cpy/tools/cross-compiler/release/lib/gcc/sparc-elf/4.4.3/ -static -lm -lc -lgcc -T "  #AD modified added -L...
...
def  compileFiles(work_area, src_files,src_dirs,include_dirs,define_strings,assembly_files, assembly_dirs, debug_mode, opt_level, glibc_version):
...
def buildExecutable(work_area, link_sections, obj_files, linker_script_file, elf_file, hex_dump_file,variable_dump_file,memmap_file, objdump_file, glibc_version):
    ...
    GLIBC_INSTALL = AJIT_PROJECT_HOME + "/glibc/2-21_debug/lib"
    sparc_ld_command += " -L" + GLIBC_INSTALL + " -L/home/zeal/AjitRepoV2/tools/cross-compiler/release/lib/gcc/sparc-elf/4.4.3/ -static -lm -lc -lgcc "
...
def parseOptions(opts):
	...
	if option == '-L':
        ...
        if option == '-G':
           ...
           logInfo("glibc_version used  = " + parameter + ".")
        if option == '-H':
           hex_file = parameter
...
def main():
    ...
    obj_files =  compileFiles(work_area, src_files, src_dirs, include_dirs, define_strings, assembly_files, assembly_dirs, debug_mode, opt_level, glibc_version)
    if(len(obj_files)==0):
