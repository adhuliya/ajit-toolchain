...
void write_to_mem(uint32_t addr, uint8_t data)
{
	...
	if(memory_page_array[page_id] == NULL)
		memory_page_array[page_id] = (uint8_t*) calloc (1, PAGE_SIZE*sizeof(uint8_t));
...
int initializeMemory(char* memoryMapFile)
{
	...
	while (1)
	{
		...
		//fprintf(stderr,"\n WARNING: in initializeMemory() \
				//\naddress 0x%x lies outside available memory. Memory size =0x%lx... wrap-around.",addr,memorySize);
		//#endif
