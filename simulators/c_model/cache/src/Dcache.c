	...
	*read_data = 0;
	...
	else if (!is_stbar && !is_nop)
	{
		...
		uint8_t do_mmu_read_dword  = 
				(is_read && (is_mmu_access  || is_mmu_bypass || (lock_mask != 0)));
		...
		if(do_mmu_read_dword || do_mmu_write_dword)
		{
			...
			Mmu(ms, mmu_dword_command, (request_type | lock_mask),
					...
					mae, &cacheable, &acc, read_data, &mmu_fsr);
		}
...
void dcacheInvalidate_core (WriteThroughAllocateCache* dcache)
{
	while(1)
	{
		uint32_t va_line_address = read_uint32(dcache->from_rlut_pipe_name);

		lock_cache(dcache);

		invalidateCacheLine(dcache, va_line_address);
