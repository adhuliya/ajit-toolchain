...
#define __AJIT_STORE_UHWORD_MMU_BYPASS__(addr, value) {\
	__asm__ __volatile__("stuha %0, [%1] %2\n\t" : : "r"(value), "r"(addr), "i"(ASI_MMU_BYPASS) : "memory");}

#define __AJIT_LOAD_UHWORD_MMU_BYPASS__(addr, retval) {\
...
#define __AJIT_READ_SDHC_REGISTER_UBYTE__(addr, val) {\
	__AJIT_LOAD_UBYTE_MMU_BYPASS__(addr, val);}
inline uint16_t __ajit_read_sdhc_reg_hword_bypass(uint32_t addr);
#define __AJIT_READ_SDHC_REGISTER_HWORD__(addr, val) {\
	__AJIT_LOAD_UHWORD_MMU_BYPASS__(addr, val);}
inline uint32_t __ajit_read_sdhc_reg_word_bypass(uint32_t addr);
#define __AJIT_READ_SDHC_REGISTER_HWORD__(addr, val) {\
	__AJIT_LOAD_WORD_MMU_BYPASS__addr, val);}
