			...
			uint32_t* mmu_fsr)
{
	...
	*acc = 0;
	...
	if(isCacheRelatedRequest(asi))
	{
		...
		{
			flushRlut (&(ms->rlut->icache_rlut));
		}	

		if(ASI_DCACHE_FLUSH(asi))
		{
			flushRlut (&(ms->rlut->dcache_rlut));
		}
			   ...
			   ))
	{
		...
		sendMmuToMemoryRequest(ms, (request_type | lock_mask), addr, write_data, byte_mask);
		...
		uint64_t* ptr = ((mmu_command == MMU_READ_LINE) ? (read_data + cacheLineOffset(addr)) 
						: read_data);
		readMemoryToMmuResponse(ms, ptr);
		...
		//Perform virtual to physical translation!
	{
		...
		if(translation_success)
		{
			...
			uint32_t line_physical_addr = physical_addr & 0xffffffffffffffc0;
			...
			if(mmu_command == MMU_READ_LINE)
			{
				if(*cacheable)
				{
					...
					insertEntry (ms->rlut,
							(physical_addr >> LOG_BYTES_PER_CACHE_LINE),
							(addr >> LOG_BYTES_PER_CACHE_LINE),
							asi);
					...
					// not cacheable... do not read the whole line...
				{
					...
					sendMmuToMemoryRequest(ms, (lock_mask | REQUEST_TYPE_READ), 
									line_physical_addr+(8*I), 
									write_data, byte_mask);
			...
			else 
			{
				...
				//wait for a response from Memory
				readMemoryToMmuResponse(ms, read_data);
			}
			//send the response back to the cache
			//that created this request
			*mae=0;
...
uint32_t readPageTableEntryFromMemory(MmuState* ms,uint64_t physical_addr)
{
	...
	sendMmuToMemoryRequest(ms, (request_type | lock_mask), physical_addr, data64, byte_mask);
	//wait for a response from Memory
	readMemoryToMmuResponse(ms, &data64);
