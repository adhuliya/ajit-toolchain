...
hwServerState* makeHwServerState(int cpu_id)
{
	...
	sprintf(hs->cpu_to_hwserver_id_pipe_name,"CPU_to_HWSERVER_id_%d", cpu_id);
	sprintf(hs->cpu_to_hwserver_state_pipe_name,"CPU_to_HWSERVER_state_%d", cpu_id);

	sprintf(hs->hwserver_to_cpu_pipe_name,"HWSERVER_to_CPU_%d", cpu_id);

	sprintf(hs->debug_command_pipe_name,"ENV_to_AJIT_debug_command_%d", cpu_id);
	sprintf(hs->debug_response_pipe_name,"AJIT_to_ENV_debug_response_%d", cpu_id);
...
int runServer(hwServerState* hs)
{
	...
	while(1)	{
		...
		msg_id = read_uint8(hs->cpu_to_hwserver_id_pipe_name);
		ajit = (ProcessorState*) read_uint64(hs->cpu_to_hwserver_state_pipe_name);
		...
		{		// Processor is in initial stop state or reset
			...
			write_uint32(hs->hwserver_to_cpu_pipe_name, 0x434F4E54);
#ifdef GDB_DBG
...
#ifdef GDB_DBG
				printf("HWServer (cpu-id=%d) : Breakpoint match found!\n", hs->cpu_id);
#endif
		...
		{	// Processor finished calculating address
			mem_address = read_uint64(hs->cpu_to_hwserver_state_pipe_name);
...
int execute_spi(ProcessorState *ajit_state, hwServerState* hs, spi_cmd *cmd_exe)	// Execute a single SPI command
{
