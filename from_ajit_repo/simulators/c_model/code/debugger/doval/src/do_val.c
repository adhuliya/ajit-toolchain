...
uint8_t gen_log (uint32_t line_num, gpb_spi_rsp resp_pckt, int cpu_id);
...
ProcessorState** g_cpu_state_vector;


int doval(ProcessorState** cpu_states,
		char* input_file, char* logf)
{	
	g_cpu_state_vector = cpu_states;
...
void perform_validation (uint32_t line_num)
{
	...
	uint8_t err = 0;
	if ((rec.cpu_id >= 0) && (rec.cpu_id < 4) && (g_cpu_state_vector[rec.cpu_id] != NULL))
	{
		char* cmd_pipe  = g_cpu_state_vector[rec.cpu_id]->hw_server->debug_command_pipe_name;
		char* resp_pipe = g_cpu_state_vector[rec.cpu_id]->hw_server->debug_response_pipe_name;
		...
		// write log
		err = gen_log(line_num, resp_pckt, rec.cpu_id);
	}
	else
	{
		fprintf(stderr,"Error:doval: cpu with id %d does not exist.\n", rec.cpu_id);
		err = 1;
	}
	...
	if (reg[0] == 'm') {
		if (reg[1] == 'm') {
			fprintf(log_file, "%u. %s :\t%s%d,\tid = %d,", line_num, result, rec.c, rec.reg_int, 
					reg_id);
			fprintf(log_file, " \texpected = 0x%x,  \t\tactual = 0x%x (mask=0x%x) (cpu-id=%d\n", exptd_value, 
					resp_pckt.content,
					compare_mask, cpu_id);
		} else {
		...
		fprintf(log_file, " \texpected = 0x%x,  \t\tactual = 0x%x (mask=0x%x) (cpu-id=%d)\n", exptd_value, resp_pckt.content, compare_mask, cpu_id);
