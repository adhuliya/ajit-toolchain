...
MonitorLoggerState* makeMonitorLoggerState (int cpu_id,
							...
							int global_verbose_flag)
{
	...
	mls->cpu_id = cpu_id;
	sprintf(mls->logger_pipe_name,"AJIT_to_ENV_logger_%d", cpu_id);

	char file_name [4096];

	if(reg_write_file != NULL)
	{
		sprintf(file_name,"%s.%d", reg_write_file, cpu_id);
		mls->reg_write_file = fopen(file_name,"w");
		assert(mls->reg_write_file);
		fprintf(mls->reg_write_file,"//============== C model register-write trace (cpu-id=%d) ============\n",
				cpu_id);
	...
	{
		sprintf(file_name,"%s.%d", long_reg_write_file, cpu_id);
