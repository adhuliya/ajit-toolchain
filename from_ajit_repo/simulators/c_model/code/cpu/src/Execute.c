...
uint32_t executeLoad(Opcode op, uint32_t operand1, uint32_t operand2, 
				...
				uint8_t asi, uint8_t rd, uint8_t *flags, ProcessorState *state)
{
	...
	if(is_not_alternate)
	{
		setPageBit(state, address); // flag all pages that were accessed.
...
//
void testAndSetBlockLdstFlags(ProcessorState* state, uint8_t byte_flag, uint8_t word_flag)
{
...
uint32_t executeBfpcc(ProcessorState* s,
...
uint32_t executeBcpcc( ProcessorState* s, Opcode op, uint32_t operand1, StatusRegisters *status_reg, uint32_t trap_vector, uint8_t flags)
{
#ifdef DEBUG
					...
					uint8_t *flags)
{
	...
	fprintf(stderr,"\tInfo : ReadStateReg  op-code=%x rs1=%x\n", 
				op, rs1);
#endif 
	StatusRegisters* sr = &(cpu_state->status_reg);
	uint32_t res = 0;
	uint32_t tv = trap_vector;
	uint32_t psr = sr->psr;
	uint8_t s = getBit32(psr, 7);
			 ...
			 uint8_t ticc_trap_type, uint32_t *pc, uint32_t *npc)
{
	...
	RegisterFile* rf = cpu_state->register_file;

	char trap_buffer[256];
	trapToString(*trap_vector, *interrupt_level, ticc_trap_type, trap_buffer);

	if (cpu_state->report_traps)
		fprintf(stderr,"\nTRAP (cpu-id=%d): pc=0x%08x npc=0x%08x psr=0x%08x  wim=0x%08x tv=0x%08x\t%s\n", 
				cpu_state->cpu_id,
				*pc, *npc, *psr,
	...
	if(!skip_write4 )
	{
	  ...
	  // 
	  pc_tmp  = cpu_state->init_pc;
	  npc_tmp = cpu_state->init_pc  + 4;
...
uint32_t executeFPInstruction( ProcessorState* s, Opcode opcode,
