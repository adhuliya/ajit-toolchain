...
{
#ifdef DEBUG
	fprintf(stderr,"\tInfo : 64-bit CSWAPD  op-code=0x%x operand1=0x%x operand2=0x%x,0x%x operand3=0x%x,0x%x\n", op, 
			operand1, 
			operand2_0, 
			operand2_1,
			data0,data1);
	...
	uint64_t operand3 = splice_words (data0, data1);
	...
	uint8_t b3bits = getSlice32(address, 2, 0);
	if(!is_privileged_trap && !is_illegal_instr_trap && (b3bits != 0))
	{
		tv = setBit32(tv, _TRAP_, 1) ;
	...
	if(!is_privileged_trap && !is_illegal_instr_trap && !is_alignment_trap) 
	{
		...
		{
			if(read_dword == operand2)
			{
				// swap register with memory.
				writeData64(state->mmu_state, state->dcache,  
						addr_space, address, 0xFF, 
						operand3,
						&mae);
				split_words(read_dword, result_h, result_l);
			}
			else
			{
				//
