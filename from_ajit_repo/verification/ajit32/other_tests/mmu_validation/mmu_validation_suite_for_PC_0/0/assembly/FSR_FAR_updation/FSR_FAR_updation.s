/* Simple mmu validation test 

Setup page tables such that VA=PA
Permissions : Only supervisor access allowed.

Then enable the mmu. 
First perform a supervisor load access (no issues).
Then perform a user-level load access. This should cause a fault.
Check FSR and FAR.
...
_start:
	...
	! PTE0= 0x8E implies all accesses allowed
	! PTE0= 0x9E implies *only* supervisor accesses allowed 
	...
	!Do some memory loads (supervisor level) after the mmu has been enabled 
	set data_A, %o1			
	ld [%o1], %l1  		! l1 = 0xABCD

	!now perform the same load in user mode
	! ASi_USER_DATA = 0xA
	lda [%o1]0xA, %l2  		! Should cause a fault (HW trap 9. %g1=0x9)
