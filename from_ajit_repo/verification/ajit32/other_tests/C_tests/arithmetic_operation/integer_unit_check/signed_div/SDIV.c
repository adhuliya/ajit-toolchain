///////Author:Asha Devi///////////
//sdiv operation is done and signature is generated
#include <stdint.h>
#include<stdlib.h>
#include<stdio.h>
#include<string.h>
...
uint32  __attribute__((section(".text.main")))  main()
{
	...
	uint32_t b = 0x77FFFF5B;
	uint32_t a2=0x77abcd;
	uint32_t a3=0xf12ab67;
	int32_t a1,a4,b1,c1,r;
	int j;
	int n=1000000;//no of testcases
	for (j=0;j<n;j++)
	{
		//lfsr
	...
	__asm__ __volatile__("st %1,[%%l1];" : "=r"(c1) : "r"(a1));
	__asm__ __volatile__("ld [%l1],%l2 \n\t");
	__asm__ __volatile__("mov %l2,%y \n\t");
	__asm__ __volatile__("sdiv %1,%2,%%o2;" "rd %%psr,%%i3;" : "=r"(r) :"r"(a4),"r"(b1));//div
	__asm__ __volatile__("xor %o2,%g4,%g4 \n\t");//signature
	}
