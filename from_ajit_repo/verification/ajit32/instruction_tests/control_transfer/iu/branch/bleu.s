...
_start:
	...
	!
	! instruction = BLEU (C or Z) (Branch on Less or Equal Unsigned)
...
label4: 
	...
	cmp %l0, %l1
	bleu label5	! conditional branch that is taken
	mov 0xF00, %i0	! delay slot instruction is always executed 
	mov 0x98, %i1	! control shouldn't reach here

label5:
	cmp %l1, %l0	
	bleu,a label6	! conditional ANNULLED branch that is taken
	mov 0xF00, %i2	! delay slot instruction is always executed 
	mov 0x77, %i3	! control shouldn't reach here
