...
_start:
	...
	!
	! instruction = FBG (Branch on Greater) (G)
	...
	fbg label1	! conditional branch that is *not* taken 
...
label1:	
	...
	fbg label2	! conditional branch that is *not* taken
	mov 0xF00, %o5	! delay slot instruction is always executed
	mov 0x99, %o6	! control should reach here

label2:
	fcmps %f1, %f0  ! compare f0, f1 (greater)
	fbg label3	! conditional branch that is taken
...
label3: 
	...
	fbg label4	! conditional branch that *not* is taken 
...
label4:
	...
	fbg label5	! conditional branch that is *not* taken 
...
label5:
	fcmps %f0, %f0  ! compare f0, f0 (equal)
	fbg,a label6	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o1	! delay slot instruction is *not* executed (annulled)
	mov 0x77, %o3	! control reaches here because branch is not taken

label6:	
	fcmps %f0, %f1  ! compare f0, f1 (less)
	fbg,a label7	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %o7	! delay slot instruction is *not* executed (annulled)
	mov 0x55, %o4	! control reaches here because branch is not taken

label7:	
	fcmps %f1, %f0  ! compare f0, f1 (greater)
	fbg,a label8	! conditional ANNULLED branch that is taken
