! Test for checking 'mulscc instruction'
!
...
_start:
	...
	! results in the current windows registers
	!
	! instruction = MULSCC
	
	
	! Check for set Z. Y set to 0 
	rd %psr, %i0	
	mov 0x00000000,%y
	set 0x00000000,%g2 
	set 0x00A0A0A0,%g3 
	mulscc %g2,%g3,%l0 				! Result is 0 (since y<0>=0 op2 is 0). Z is set.
	rd %y, %o0 						! y is 0x00000000 ([rs1<0>, y<31:1>])
	rd %psr, %i1

	! Y set to a non zero value. Standard addition
	mov 0x00000003,%y
	set 0x00000002,%g2 
	set 0x00A0A0A0,%g3 
	mulscc %g2,%g3,%l1 				! Result is 0x00A0A0A1 (op 1 = 0x01 since its is concatenated with N xor V = 0)
	rd %y, %o1 						! y is 0x00000001 ([rs1<0>, y<31:1>])
	rd %psr, %i2


	! Y set to a non zero value. Op2 is negetive. Check for set N
	mov 0x00000003,%y
	set 0x00000002,%g2 
	set 0xA0A0A0A0,%g3 
	mulscc %g2,%g3,%l2 				! Result is 0xA0A0A0A1 (op 1 = 0x01 since its is concatenated with N xor V = 0). N is set
	rd %y, %o2 						! y is 0x00000001 ([rs1<0>, y<31:1>])
	rd %psr, %i3

	! Y set to a non zero value. Both Op1 and Op2 have bit 31 set to 1. Check for set C and set V 
	mov 0x00000003,%y
	set 0x00000000,%g2 
	set 0x80A0A0A0,%g3 
	mulscc %g2,%g3,%l3 				! Result is 0x00A0A0A0 (op 1 0x80000000 since its is concatenated with N xor V = 1), V is set. C is set
	rd %y, %o3 						! y is 0x00000001 ([rs1<0>, y<31:1>])
	rd %psr, %i4

	set 0x00000000,%g2 
	andcc %g2, 0x01, %g2			! Reset iCC bits
	! Y set to a non zero value. Op1 and Op2 are standard, but result has bit 31 as 1. Check for set C and set V
	mov 0x00000003,%y
	set 0x80000000,%g2 
	set 0x40000000,%g3 
	mulscc %g2,%g3,%l4 				! Result is 0x80000000 (op 1 0x40000000 since its is concatenated with N xor V = 0). V is set. N is set
	rd %y, %o4 						! y is 0x00000001 ([rs1<0>, y<31:1>])
	rd %psr, %i5

	set 0x00000000,%g2 
	andcc %g2, 0x01, %g2			! Reset iCC bits
	! Y set to a non zero value. Op2 has bit 31 set to 1 and Op1 is standard. Result has bit 31 as 0. Check for set V and set C
	mov 0x00000003,%y
	set 0xFFFFFFFF,%g2 
	set 0xFFFFFFFF,%g3 
	mulscc %g2,%g3,%l5 				! Result is 0x7FFFFFFF (op 1 0x7FFFFFFF since its is concatenated with N xor V = 0). C is set
	rd %y, %o5 						! y is 0x80000001 ([rs1<0>, y<31:1>])
	rd %psr, %i6

	set 0x00000000,%g2 
	andcc %g2, 0x10, %g2			! Reset iCC bits. Set N = 1
	! Y set to a non zero value. Op1 has bit 31 set to 1 and Op2 is standard. Result has bit 31 as 0. Check for set V and set C
	mov 0x00000003,%y
	set 0xFFFFFFFF,%g2 
	set 0x7FFFFFFF,%g3 
	mulscc %g2,%g3,%l6 				! Result is 0x7FFFFFFF (op 1 0xFFFFFFFF since its is concatenated with N xor V = 1). C is set
	rd %y, %o6 						! y is 0x80000001 ([rs1<0>, y<31:1>])
	rd %psr, %i7
