#!/bin/sh

_MAIN="main";
_CORTOS_DIR="cortos";
_CORTOS_INIT_00="./init_00.s";
_CORTOS_TRAP_HANDLER="./trap_handlers.s";

_CORTOS_VMAP="$_CORTOS_DIR/vmap.txt";
_CORTOS_ASM="$_CORTOS_DIR/cortos_asm.s";
_CORTOS_C="$_CORTOS_DIR/cortos.c";
_CORTOS_PRINTF="$_CORTOS_DIR/__cortos_ee_printf.c";
_CORTOS_LOCK_UNLOCK="$_CORTOS_DIR/cortos_lock_unlock.s";
_CORTOS_RES_LOCK_UNLOCK="$_CORTOS_DIR/__cortos_lock_unlock.s";
_CORTOS_Q_LOCK_UNLOCK="$_CORTOS_DIR/__cortos_q_lock_unlock.s";
_CORTOS_MSG_QUEUE="$_CORTOS_DIR/cortos_q.c";
_CORTOS_PG_TABLES="$_CORTOS_DIR/setup_page_tables.s";
_LINKER_SCRIPT="$_CORTOS_DIR/LinkerScript.txt";

_PT="$AJIT_MINIMAL_PRINTF_TIMER";
_AAR_MT="$AJIT_PROJECT_HOME/tools/ajit_access_routines_mt";
_AAR="$AJIT_ACCESS_ROUTINES";

% if confObj.addBget:
_CORTOS_BGET="$_CORTOS_DIR/cortos_bget.c";
_BGET="$_CORTOS_DIR/__bget.c";
% end


# genVmapAsm ${_CORTOS_VMAP} ${_CORTOS_PG_TABLES};
#  -s $_CORTOS_PG_TABLES \

# NOTE: the use of `-U` to enable uclibc
compileToSparcUclibc.py \
% if confObj.debugBuild:
  -g \
% end
  -V ${_CORTOS_VMAP} \
  -I ${AJIT_UCLIBC_HEADERS_DIR} \
  -I ${AJIT_LIBGCC_INSTALL_DIR}/include \
  -I . \
  -I ${_AAR}/include \
  -I ${_PT}/include \
  -s ${_CORTOS_INIT_00} \
  -s ${_CORTOS_ASM} \
  -s ${_CORTOS_TRAP_HANDLER} \
  -s ${_CORTOS_LOCK_UNLOCK} \
  -s ${_CORTOS_RES_LOCK_UNLOCK} \
  -s ${_CORTOS_Q_LOCK_UNLOCK} \
% for fileName in confObj.cFileNames:
  -c {{fileName}} \
% end
  -c ${_CORTOS_MSG_QUEUE} \
  -c ${_CORTOS_C} \
  -c ${_CORTOS_PRINTF} \
  -C ${_AAR}/src \
% if False: # removed the original ee_printf
  -C ${_PT}/src \
% end
% if confObj.addBget:
  -c ${_CORTOS_BGET} \
  -c ${_BGET} \
% end
  -N ${_MAIN} \
  -L ${_LINKER_SCRIPT} \
  -D AJIT \
  -U;

