...
sunos_set_arch_mach (bfd *abfd, enum machine_type machtype)
{
  ...
  switch (machtype)
    ...
    case M_AJITLET:
      arch = bfd_arch_ajit;
      machine = bfd_mach_ajit_ajitlet;
      break;

    case M_AJITLITE_LE:
      arch = bfd_arch_ajit;
      machine = bfd_mach_ajit_ajitlite_le;
      break;
...
sunos_write_object_contents (bfd *abfd)
{
  ...
  switch (bfd_get_arch (abfd))
	...
	case bfd_mach_ajit_ajitlet:
	  N_SET_MACHTYPE (*execp, M_AJITLET);
	  break;
	case bfd_mach_ajit_ajitlite_le:
	  N_SET_MACHTYPE (*execp, M_AJITLITE_LE);
	  break;
	default:
	  N_SET_MACHTYPE (*execp, M_AJIT);
	  break;
	}
      break;
    case bfd_arch_i386:
