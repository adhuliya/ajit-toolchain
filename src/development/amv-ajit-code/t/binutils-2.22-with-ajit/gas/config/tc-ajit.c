...
/* ??? The default value should be V8, but ajitlite support was added
   by making it the default.  GCC now passes -Aajitlite, so maybe sometime in
   the future we can set this to V8.  */
#ifndef DEFAULT_ARCH
#define DEFAULT_ARCH "ajitlite"
#endif
static char *default_arch = DEFAULT_ARCH;

/* Non-zero if the initial values of `max_architecture' and `ajit_arch_size'
   have been set.  */
static int default_init_p;

/* Current architecture.  We don't bump up unless necessary.  */
static enum ajit_opcode_arch_val current_architecture = AJIT_OPCODE_ARCH_V6;
...

/* ajit64 privileged and hyperprivileged registers.  */
