/* tc-ajit.c -- Assemble for the AJIT
...
static int ajit_ip (char *, const struct ajit_opcode **);
...
/* There is  only one AJIT architecture,  a 32 bit based  on SPARC V8.
   The following comment  from the SPARC support code  does not apply.
   We  keep it  to remember  the GCC  comment below  when we  add AJIT
   support in GCC.

   ??? The default value should  be V8, but ajitlite support was added
   by  making it  the default.   GCC now  passes -Aajitlite,  so maybe
   sometime in the future we can set this to V8.  */
#ifndef DEFAULT_ARCH
#define DEFAULT_ARCH "ajit"
#endif
static char *default_arch = DEFAULT_ARCH;

/* Non-zero   if  the   initial  values   of   `max_architecture'  and
   `ajit_arch_size' have been set.  */
static int default_init_p;

/* Current architecture.  We don't bump up unless necessary.  */
static enum ajit_opcode_arch_val current_architecture = AJIT_OPCODE_ARCH_V8;
/* static enum ajit_opcode_arch_val current_architecture = AJIT_OPCODE_ARCH; */
...
/* The maximum architecture level we can bump up to.
   ...
   an explicit argument before we'll create v9 object files.  However,
   if  we  don't  see any  v9  insns,  a  v8plus  object file  is  not
   created.  */
static enum ajit_opcode_arch_val max_architecture;

/* Either 32 or 64, selects file format.  */
static int ajit_arch_size;
...
/* The currently selected v9 memory model.  Currently only used for
   ELF.  */
static enum { MM_TSO, MM_PSO, MM_RMO } ajit_memory_model = MM_RMO;
...
static enum ajit_opcode_arch_val warn_after_architecture;
...
int ajit_cie_data_alignment;

/* V9 and 86x have big and little endian data, but instructions are always big
   endian.  The ajitlet has bi-endian support but both data and insns have
   the same endianness.  Global `target_big_endian' is used for data.
   The following macro is used for instructions.  */
#ifndef INSN_BIG_ENDIAN
#define INSN_BIG_ENDIAN (target_big_endian \
                         || !AJIT_OPCODE_ARCH (max_architecture))
/* #define INSN_BIG_ENDIAN (target_big_endian \ */
/*                       || default_arch_type == ajit86x \ */
/*                       || AJIT_OPCODE_ARCH_V9_P (max_architecture)) */
#endif
...
const pseudo_typeS md_pseudo_table[] =
{
  ...
  {"uaxword", s_uacons, 8},
#ifdef OBJ_ELF
  /* These are specific to ajit/svr4.  */
  {"2byte", s_uacons, 2},
  {"4byte", s_uacons, 4},
  {"8byte", s_uacons, 8},
...
struct ajit_it the_insn, set_insn;

static void output_insn (const struct ajit_opcode *, struct ajit_it *);

/* Table of arguments to -A.
   The ajit_opcode_arch table in ajit-opc.c is insufficient and incorrect
   for this use.  That table is for opcodes only.  This table is for opcodes
   and file formats.  */

/* enum ajit_arch_types {ajit}; */

/* static struct ajit_arch { */
/*   char *name; */
/*   char *opcode_arch; */
/*   enum ajit_arch_types arch_type; */
...
/*   int hwcap_allowed; */
/* } ajit_arch_table[] = { */
/*   { "ajit", "ajit", ajit, 32, 1, F_MUL32|F_DIV32|F_FSMULD }, */
/*   { NULL, NULL, ajit, 0, 0, 0 } */
/* }; */

enum ajit_arch_types {v6, v7, v8, ajitlet, ajitlite, ajit86x, v8plus,
...
static struct ajit_arch {
  ...
  { "v6", "v6", v6, 0, 1, 0 },
  { "v7", "v7", v7, 0, 1, 0 },
  { "ajit", "v8", v8, 32, 1, F_MUL32|F_DIV32|F_FSMULD },
  { "v8a", "v8", v8, 32, 1, F_MUL32|F_DIV32|F_FSMULD },
  { "v9", "v9", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_V8PLUS },
  /* { "v8", "v8", v8, 32, 1, F_MUL32|F_DIV32|F_FSMULD }, */
  /* { "v8a", "v8", v8, 32, 1, F_MUL32|F_DIV32|F_FSMULD }, */
  /* { "ajit", "v9", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_V8PLUS }, */
  { "ajitvis", "v9a", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_VIS },
  { "ajitvis2", "v9b", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_VIS|F_VIS2 },
  { "ajitfmaf", "v9b", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_VIS|F_VIS2|F_FMAF },
  { "ajitima", "v9b", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_VIS|F_VIS2|F_FMAF|F_IMA },
  { "ajitvis3", "v9b", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_VIS|F_VIS2|F_FMAF|F_VIS3|F_HPC },
  { "ajitvis3r", "v9b", v9, 0, 1, F_MUL32|F_DIV32|F_FSMULD|F_POPC|F_VIS|F_VIS2|F_FMAF|F_VIS3|F_HPC|F_RANDOM|F_TRANS|F_FJFMAU },
  { "ajitlet", "ajitlet", ajitlet, 32, 1, F_MUL32|F_DIV32|F_FSMULD },
  { "ajitlite", "ajitlite", ajitlite, 32, 1, F_MUL32|F_DIV32|F_FSMULD },
  { "ajit86x", "ajitlite", ajit86x, 32, 1, F_MUL32|F_DIV32|F_FSMULD },
...
static enum ajit_arch_types default_arch_type;
...
lookup_arch (char *name)
{
  ...
  fprintf (stderr, "AMV: AJIT ARCHITECTURE TABLE.\n");
  /* char *name; */
  /* char *opcode_arch; */
  /* enum ajit_arch_types arch_type; */
  /* /\* Default word size, as specified during configuration. */
  ...
  /* int user_option_p; */
  /* int hwcap_allowed; */
  fprintf (stderr, "AMV: |%2s|%10s|%10s|%4s|%4s|%4s|%10s|%10s|\n",
	   "No", "Name", "OpCd Arch", "Type", "Size", "UOpt", "HwCa", "HwCa");
  for (sa = &ajit_arch_table[0]; sa->name != NULL; sa++, c++) {
    fprintf (stderr, "AMV: |%02d|%10s|%10s|%04d|%04d|%04d|%010d|0x%08X|\n", c,
	     sa->name, sa->opcode_arch, sa->arch_type,
	     sa->default_arch_size, sa->user_option_p,
	     sa->hwcap_allowed, sa->hwcap_allowed);
  }

  for (sa = &ajit_arch_table[0]; sa->name != NULL; sa++)
...
{
  struct ajit_arch *sa = lookup_arch (default_arch);

  fprintf (stderr, "AMV: default_arch      : %s\n", default_arch);
  fprintf (stderr, "AMV: init_default_arch(): sa->opcode_arch      : %s\n", sa->opcode_arch);
  fprintf (stderr, "AMV: init_default_arch(): sa->default_arch_size: %d\n", sa->default_arch_size);
  if (sa == NULL
      || sa->default_arch_size == 0)
    as_fatal (_("Invalid default architecture, broken assembler."));

  max_architecture = ajit_opcode_lookup_arch (sa->opcode_arch);
  fprintf (stderr, "AMV: init_default_arch(): max_architecture is  : %d\n", max_architecture);
  fprintf (stderr, "AMV: init_default_arch(): AJIT_OPCODE_ARCH_BAD : %d\n", AJIT_OPCODE_ARCH_BAD);
  if (max_architecture == AJIT_OPCODE_ARCH_BAD) {
    as_fatal (_("init_default_arch(): Bad opcode table, broken assembler."));
  }
  default_arch_size = ajit_arch_size = sa->default_arch_size;
  default_init_p = 1;
...
#ifdef OBJ_AOUT
#ifdef TE_NetBSD
  return "a.out-ajit-netbsd";
#else
#ifdef TE_AJITAOUT
  if (target_big_endian)
    return "a.out-sunos-big";
  /* else if (default_arch_type == ajit86x && target_little_endian_data) */
  /*   return "a.out-sunos-big"; */
  else
    return "a.out-ajit-little";
#else
  return "a.out-sunos-big";
#endif
#endif
...
#ifdef TE_VXWORKS
  return "elf32-ajit-vxworks";
#endif

#ifdef OBJ_ELF
  return ajit_arch_size == 64 ? ELF64_TARGET_FORMAT : ELF_TARGET_FORMAT;
#endif
 ...
 *
 *      -Av6, -Av7, -Av8, -Aajitlite, -Aajitlet
 *              Standard 32 bit architectures.
 *      -Av9, -Av9a, -Av9b
 *              Ajit64 in either a 32 or 64 bit world (-32/-64 says which).
 ...
 *              error.  For example, from ajitlite to v9.
 */
...
#define OPTION_AJIT (OPTION_MD_BASE + 1)
  {"ajit", no_argument, NULL, OPTION_AJIT},
...
md_parse_option (int c, char *arg)
{
  ...
  switch (c)
    ...
    case OPTION_BUMP:
      warn_on_bump = 1;
      /* warn_after_architecture = AJIT_OPCODE_ARCH_V6; */
      warn_after_architecture = AJIT_OPCODE_ARCH_V8;
      /* warn_after_architecture = AJIT_OPCODE_ARCH; */
      break;
    ...
    case 'A':
      {
        struct ajit_arch *sa;
        enum ajit_opcode_arch_val opcode_arch;
        ...
        opcode_arch = ajit_opcode_lookup_arch (sa->opcode_arch);
        if (opcode_arch == AJIT_OPCODE_ARCH_BAD)
    ...
    case OPTION_AJIT:
      /* Ignore -ajit, used by SunOS make default .s.o rule.  */
      break;
    ...
    case OPTION_64:
      {
        ...
        ajit_arch_size = c == OPTION_32 ? 32 : 64;
        list = bfd_target_list ();
        for (l = list; *l != NULL; l++)
          {
            if (ajit_arch_size == 32)
              {
                if (CONST_STRNEQ (*l, "elf32-ajit"))
                  break;
              }
            else
              {
                if (CONST_STRNEQ (*l, "elf64-ajit"))
                  break;
              }
          }
        if (*l == NULL)
        ...
        /* if (ajit_arch_size == 64 */
        /*     && max_architecture < AJIT_OPCODE_ARCH_V9) */
        /*   max_architecture = AJIT_OPCODE_ARCH_V9; */
      }
      break;

    case OPTION_TSO:
      ajit_memory_model = MM_TSO;
      break;

    case OPTION_PSO:
      ajit_memory_model = MM_PSO;
      break;

    case OPTION_RMO:
      ajit_memory_model = MM_RMO;
      break;
...
md_show_usage (FILE *stream)
{
  ...
  fprintf (stream, _("AJIT options:\n"));
  column = 0;
  for (arch = &ajit_arch_table[0]; arch->name; arch++)
    {
      if (!arch->user_option_p)
        continue;
      if (arch != &ajit_arch_table[0])
        fprintf (stream, " | ");
  ...
  fprintf (stream, _("\n\
                        specify variant of AJIT architecture\n\
...
md_begin (void)
{
  ...
  ajit_cie_data_alignment = ajit_arch_size == 64 ? -8 : -4;
  op_hash = hash_new ();

  while (i < (unsigned int) ajit_num_opcodes)
    {
      const char *name = ajit_opcodes[i].name;
      retval = hash_insert (op_hash, name, (void *) &ajit_opcodes[i]);
      if (retval != NULL)
        {
          as_bad (_("Internal error: can't hash `%s': %s\n"),
                  ajit_opcodes[i].name, retval);
          lose = 1;
        }
      do
        {
          if (ajit_opcodes[i].match & ajit_opcodes[i].lose)
            {
              as_bad (_("Internal error: losing opcode: `%s' \"%s\"\n"),
                      ajit_opcodes[i].name, ajit_opcodes[i].args);
              lose = 1;
            }
          ++i;
        }
      while (i < (unsigned int) ajit_num_opcodes
             && !strcmp (ajit_opcodes[i].name, name));
    }

  for (i = 0; native_op_table[i].name; i++)
    {
      const struct ajit_opcode *insn;
      char *name = ((ajit_arch_size == 32)
                    ? native_op_table[i].name32
                    : native_op_table[i].name64);
      insn = (struct ajit_opcode *) hash_find (op_hash, name);
      if (insn == NULL)
        {
  ...
  if (warn_on_bump
      && architecture_requested)
    {
      ...
      /* Find the highest architecture level that doesn't conflict with
         the requested one.  */
      for (max_architecture = AJIT_OPCODE_ARCH_MAX;
           max_architecture > warn_after_architecture;
           --max_architecture)
        if (! AJIT_OPCODE_CONFLICT_P (max_architecture,
...
ajit_md_end (void)
{
  unsigned long mach = bfd_mach_ajit;

  /* if (ajit_arch_size == 64) */
  /*   switch (current_architecture) */
  /*     { */
  /*     case AJIT_OPCODE_ARCH_V9A: mach = bfd_mach_ajit_v9a; break; */
  /*     case AJIT_OPCODE_ARCH_V9B: mach = bfd_mach_ajit_v9b; break; */
  /*     default: mach = bfd_mach_ajit_v9; break; */
  /*     } */
  /* else */
  /*   switch (current_architecture) */
  /*     { */
  /*     case AJIT_OPCODE_ARCH_AJITLET: mach = bfd_mach_ajit_ajitlet; break; */
  /*     case AJIT_OPCODE_ARCH_V9: mach = bfd_mach_ajit_v8plus; break; */
  /*     case AJIT_OPCODE_ARCH_V9A: mach = bfd_mach_ajit_v8plusa; break; */
  /*     case AJIT_OPCODE_ARCH_V9B: mach = bfd_mach_ajit_v8plusb; break; */
  /*     /\* The ajitlite is treated like a normal ajit.  Perhaps it shouldn't */
  /*     be but for now it is (since that's the way it's always been */
  /*     treated).  *\/ */
  /*     default: break; */
  /*     } */
  bfd_set_arch_mach (stdoutput, bfd_arch_ajit, mach);
  ...
  if (hwcap_seen)
    {
      ...
      if (hwcap_seen & F_MUL32)
        bits |= ELF_AJIT_HWCAP_MUL32;
      if (hwcap_seen & F_DIV32)
        bits |= ELF_AJIT_HWCAP_DIV32;
      if (hwcap_seen & F_FSMULD)
        bits |= ELF_AJIT_HWCAP_FSMULD;
      if (hwcap_seen & F_V8PLUS)
        bits |= ELF_AJIT_HWCAP_V8PLUS;
      if (hwcap_seen & F_POPC)
        bits |= ELF_AJIT_HWCAP_POPC;
      if (hwcap_seen & F_VIS)
        bits |= ELF_AJIT_HWCAP_VIS;
      if (hwcap_seen & F_VIS2)
        bits |= ELF_AJIT_HWCAP_VIS2;
      if (hwcap_seen & F_ASI_BLK_INIT)
        bits |= ELF_AJIT_HWCAP_ASI_BLK_INIT;
      if (hwcap_seen & F_FMAF)
        bits |= ELF_AJIT_HWCAP_FMAF;
      if (hwcap_seen & F_VIS3)
        bits |= ELF_AJIT_HWCAP_VIS3;
      if (hwcap_seen & F_HPC)
        bits |= ELF_AJIT_HWCAP_HPC;
      if (hwcap_seen & F_RANDOM)
        bits |= ELF_AJIT_HWCAP_RANDOM;
      if (hwcap_seen & F_TRANS)
        bits |= ELF_AJIT_HWCAP_TRANS;
      if (hwcap_seen & F_FJFMAU)
        bits |= ELF_AJIT_HWCAP_FJFMAU;
      if (hwcap_seen & F_IMA)
        bits |= ELF_AJIT_HWCAP_IMA;
      if (hwcap_seen & F_ASI_CACHE_SPARING)
        bits |= ELF_AJIT_HWCAP_ASI_CACHE_SPARING;

      bfd_elf_add_obj_attr_int (stdoutput, OBJ_ATTR_GNU, Tag_GNU_Ajit_HWCAPS, bits);
    }
#endif
}

...
in_signed_range (bfd_signed_vma val, bfd_signed_vma max)
{
  ...
  /* Sign-extend the value from the architecture word size, so that
     0xffffffff is always considered -1 on ajit32.  */
  if (ajit_arch_size == 32)
    {
...

/* For communication between ajit_ip and get_expression.  */
...
/* FIXME: ajit-opc.c doesn't have necessary "S" trigger to enable this.  */
...
static const struct ajit_opcode *last_insn;
...
static void
synthetize_setuw (const struct ajit_opcode *insn)
{
  int need_hi22_p = 0;
  ...
  if (the_insn.exp.X_op == O_constant)
    {
      /* if (AJIT_OPCODE_ARCH_V9_P (max_architecture)) */
      /*   { */
      /*     if (sizeof (offsetT) > 4 */
...
static void
synthetize_setsw (const struct ajit_opcode *insn)
{
  int low32, rd, opc;
...
static void
synthetize_setx (const struct ajit_opcode *insn)
{
  int upper32, lower32;
  ...
  if (need_hh22_p)
    {
      the_insn.opcode = (SETHI_INSN | RD (upper_dstreg)
                         | ((upper32 >> 10) & 0x3fffff));
      the_insn.reloc = (the_insn.exp.X_op != O_constant
                        ? BFD_RELOC_AJIT_HH22 : BFD_RELOC_NONE);
      output_insn (insn, &the_insn);
    }
  ...
  if (need_hi22_p)
    {
      the_insn.opcode = (SETHI_INSN | RD (dstreg)
                         | (((need_xor10_p ? ~lower32 : lower32)
                             >> 10) & 0x3fffff));
      the_insn.reloc = (the_insn.exp.X_op != O_constant
                        ? BFD_RELOC_AJIT_LM22 : BFD_RELOC_NONE);
      output_insn (insn, &the_insn);
    }
  ...
  if (need_hm10_p)
    {
      ...
      the_insn.reloc = (the_insn.exp.X_op != O_constant
                        ? BFD_RELOC_AJIT_HM10 : BFD_RELOC_NONE);
      output_insn (insn, &the_insn);
    }
...
md_assemble (char *str)
{
  ...
  /* AJIT before v9 requires a nop instruction between a floating
     point instruction and a floating point branch.  We insert one
     automatically, with a warning.  */
  /* if (max_architecture < AJIT_OPCODE_ARCH_V9 */
  /*     && last_insn != NULL */
  /*     && (insn->flags & F_FBR) != 0 */
  /*     && (last_insn->flags & F_FLOAT) != 0) */
  /*   { */
  /*     struct ajit_it nop_insn; */
    ...
    case SPECIAL_CASE_FDIV:
      {
        ...
        /* According to information leaked from Sun, the "fdiv" instructions
           on early AJIT machines would produce incorrect results sometimes.
...
static int
ajit_ip (char *str, const struct ajit_opcode **pinsn)
{
  char *error_message = "";
  char *s;
  const char *args;
  ...
  fprintf (stderr, "AMV== %-40s: %d.\n", "Number of opcodes in AJIT", ajit_num_opcodes);
  s = str;
  if (ISLOWER (*s))
    {
      do
  ...
  insn = (struct ajit_opcode *) hash_find (op_hash, str);
  *pinsn = insn;
  if (insn == NULL)
    {
      as_bad (_("Unknown opcode: `%s'"), str);
  ...
  argsStart = s;
  for (;;)			/* AMV: Begin opcode generation from ASM */
    {
      opcode = insn->match;	/* NON Parametric part of the opcode - the instruction! */
      ...
      for (args = insn->args;; ++args)
        {
          ...
          switch (*args)
            {
            case 'K':		/* v9: See include/opcode/ajit.h */
              {
                int kmask = 0;
                    ...
                    while (*s == '#')
                      {
                        ...
                        if (! parse_keyword_arg (ajit_encode_membar, &s,
                                                 &jmask))
                          {
            ...
            case '3':		/* v9b: See include/opcode/ajit.h */
              {
                int smask = 0;
            ...
            case '*':		/* v9: See include/opcode/ajit.h */
              {
                int fcn = 0;
                ...
                if (*s == '#')
                  {
                    if (! parse_keyword_arg (ajit_encode_prefetch, &s, &fcn))
                      {
            ...
            case '!':		/* v9: See include/opcode/ajit.h */
            case '?':
              /* Parse a ajit64 privileged register.  */
              if (*s == '%')
                {
            ...
            case '$':		/* NOT LISTED IN include/opcode/ajit.h */
            case '%':
              /* Parse a ajit64 hyperprivileged register.  */
              if (*s == '%')
                {
            ...
            case '_':		/* v9a: See include/opcode/ajit.h */
            case '/':
              ...
              if (*s == '%')
                {
                  ...
                  /* if (p->regnum >= 24 */
                  /*     && (insn->architecture */
                  /*         & AJIT_OPCODE_ARCH_MASK (AJIT_OPCODE_ARCH_V9A))) */
                  /*   { */
            ...
            case 'M':		/* Alternate space register */
            case 'm':
                  ...
                  if (ISDIGIT (*s))
                    {
                      ...
                      /* if (current_architecture >= AJIT_OPCODE_ARCH_V9) */
                      /*   { */
                      /*     if (num < 16 || 31 < num) */
                      /*       { */
            ...
            case 'I':		/* v9: 11 bit immediate */
              the_insn.reloc = BFD_RELOC_AJIT_11;
              goto immediate;

            case 'j':		/* v9: 10 bit immediate */
              the_insn.reloc = BFD_RELOC_AJIT_10;
              goto immediate;

            case 'X':		/* 5 bit unsigned immediate, 13 for V8 */
              /* V8 systems don't understand BFD_RELOC_AJIT_5.  */
              /* if (AJIT_OPCODE_ARCH_V9_P (max_architecture)) */
              /*   the_insn.reloc = BFD_RELOC_AJIT_5; */
              /* else */
                the_insn.reloc = BFD_RELOC_AJIT13;
              /* These fields are unsigned, but for upward compatibility,
                 allow negative values as well.  */
              goto immediate;

            case 'Y':		/* 6 bit unsigned immediate, 13 for V8 */
              /* V8 systems don't understand BFD_RELOC_AJIT_6.  */
              /* if (AJIT_OPCODE_ARCH_V9_P (max_architecture)) */
              /*   the_insn.reloc = BFD_RELOC_AJIT_6; */
              /* else */
                the_insn.reloc = BFD_RELOC_AJIT13;
              /* These fields are unsigned, but for upward compatibility,
                 allow negative values as well.  */
            ...
            case 'k':		/* v9: See include/opcode/ajit.h */
              the_insn.reloc = /* RELOC_WDISP2_14 */ BFD_RELOC_AJIT_WDISP16;
              the_insn.pcrel = 1;
              goto immediate;

            case 'G':		/* v9: See include/opcode/ajit.h */
              the_insn.reloc = BFD_RELOC_AJIT_WDISP19;
              the_insn.pcrel = 1;
              goto immediate;

            case 'N':		/* v9: See include/opcode/ajit.h */
              if (*s == 'p' && s[1] == 'n')
                {
                  s += 2;
                  continue;
            ...
            case 'T':		/* v9: See include/opcode/ajit.h */
              if (*s == 'p' && s[1] == 't')
                {
                  s += 2;
                  continue;
            ...
            case 'z':		/* v9: See include/opcode/ajit.h */
              if (*s == ' ')
                {
                  ++s;
                }
            ...
            case '6':		/* v9: See include/opcode/ajit.h */
              if (*s == ' ')
                {
                  ++s;
                }
            ...
            case '7':		/* v9: See include/opcode/ajit.h */
              if (*s == ' ')
                {
                  ++s;
                }
            ...
            case '8':		/* v9: See include/opcode/ajit.h */
              if (*s == ' ')
                {
                  ++s;
                }
            ...
            case '9':		/* v9: See include/opcode/ajit.h */
              if (*s == ' ')
                {
                  ++s;
                }
            ...
            case 'P':		/* v9: See include/opcode/ajit.h */
              if (strncmp (s, "%pc", 3) == 0)
                {
                  s += 3;
                  continue;
            ...
            case 'W':		/* v9: See include/opcode/ajit.h */
              ...
              if (s[0] == ',' && s[1] == '%')
                {
                  ...
                  ops[] =
                  {
                    { "tgd_add", 7, BFD_RELOC_AJIT_TLS_GD_ADD, 0 },
                    { "tgd_call", 8, BFD_RELOC_AJIT_TLS_GD_CALL, 1 },
                    { "tldm_add", 8, BFD_RELOC_AJIT_TLS_LDM_ADD, 0 },
                    { "tldm_call", 9, BFD_RELOC_AJIT_TLS_LDM_CALL, 1 },
                    { "tldo_add", 8, BFD_RELOC_AJIT_TLS_LDO_ADD, 0 },
                    { "tie_ldx", 7, BFD_RELOC_AJIT_TLS_IE_LDX, 0 },
                    { "tie_ld", 6, BFD_RELOC_AJIT_TLS_IE_LD, 0 },
                    { "tie_add", 7, BFD_RELOC_AJIT_TLS_IE_ADD, 0 },
                    { "gdop", 4, BFD_RELOC_AJIT_GOTDATA_OP, 0 },
                    { NULL, 0, 0, 0 }
                  };
                  const struct ops *o;
                  char *s1;
            ...
            case 'l':           /* 22 bit PC relative immediate  */
              the_insn.reloc = BFD_RELOC_AJIT_WDISP22;
              the_insn.pcrel = 1;
              goto immediate;
            ...
            case 'n':           /* 22 bit immediate  */
              the_insn.reloc = BFD_RELOC_AJIT22;
              goto immediate;
            ...
            immediate:
                ...
                if (*s == '%')
                  {
                    static const struct ops {
                      ...
                      /* hix/lox must appear before hi/lo so %hix won't be
                         mistaken for %hi.  */
                      { "hix", 3, BFD_RELOC_AJIT_HIX22, 1, 0 },
                      { "lox", 3, BFD_RELOC_AJIT_LOX10, 1, 0 },
                      { "hi", 2, BFD_RELOC_HI22, 0, 1 },
                      { "lo", 2, BFD_RELOC_LO10, 0, 1 },
                      { "pc22", 4, BFD_RELOC_AJIT_PC22, 0, 1 },
                      { "pc10", 4, BFD_RELOC_AJIT_PC10, 0, 1 },
                      { "hh", 2, BFD_RELOC_AJIT_HH22, 1, 1 },
                      { "hm", 2, BFD_RELOC_AJIT_HM10, 1, 1 },
                      { "lm", 2, BFD_RELOC_AJIT_LM22, 1, 1 },
                      { "h44", 3, BFD_RELOC_AJIT_H44, 1, 0 },
                      { "m44", 3, BFD_RELOC_AJIT_M44, 1, 0 },
                      { "l44", 3, BFD_RELOC_AJIT_L44, 1, 0 },
                      { "uhi", 3, BFD_RELOC_AJIT_HH22, 1, 0 },
                      { "ulo", 3, BFD_RELOC_AJIT_HM10, 1, 0 },
                      { "tgd_hi22", 8, BFD_RELOC_AJIT_TLS_GD_HI22, 0, 0 },
                      { "tgd_lo10", 8, BFD_RELOC_AJIT_TLS_GD_LO10, 0, 0 },
                      { "tldm_hi22", 9, BFD_RELOC_AJIT_TLS_LDM_HI22, 0, 0 },
                      { "tldm_lo10", 9, BFD_RELOC_AJIT_TLS_LDM_LO10, 0, 0 },
                      { "tldo_hix22", 10, BFD_RELOC_AJIT_TLS_LDO_HIX22, 0,
                                                                         0 },
                      { "tldo_lox10", 10, BFD_RELOC_AJIT_TLS_LDO_LOX10, 0,
                                                                         0 },
                      { "tie_hi22", 8, BFD_RELOC_AJIT_TLS_IE_HI22, 0, 0 },
                      { "tie_lo10", 8, BFD_RELOC_AJIT_TLS_IE_LO10, 0, 0 },
                      { "tle_hix22", 9, BFD_RELOC_AJIT_TLS_LE_HIX22, 0, 0 },
                      { "tle_lox10", 9, BFD_RELOC_AJIT_TLS_LE_LOX10, 0, 0 },
                      { "gdop_hix22", 10, BFD_RELOC_AJIT_GOTDATA_OP_HIX22,
                        0, 0 },
                      { "gdop_lox10", 10, BFD_RELOC_AJIT_GOTDATA_OP_LOX10,
                        0, 0 },
                      { NULL, 0, 0, 0, 0 }
                    };
                    const struct ops *o;
                ...
                if (op_arg)
                  {
                    ...
                    if (the_insn.exp2.X_op == O_absent)
                    ...
                    else
                      {
                        ...
                        the_insn.reloc = BFD_RELOC_AJIT_OLO10;
                      }
                  }
              }
              ...
              if (the_insn.exp.X_op == O_constant
                  ...
                  && the_insn.exp.X_op_symbol == 0)
                {
                  ...
                  if (the_insn.reloc >= BFD_RELOC_AJIT_TLS_GD_HI22
                      && the_insn.reloc <= BFD_RELOC_AJIT_TLS_TPOFF64)
                    {
            ...
            case 'A':
              {
                ...
                if (*s == '#')
                  {
                    if (! parse_keyword_arg (ajit_encode_asi, &s, &asi))
                      {
            ...
            /* case 'U': */
            /*   { */
            /*  /\* Parse a ajitlet cpreg.  *\/ */
            /*  int cpreg; */
            /*  /\* if (! parse_keyword_arg (ajit_encode_ajitlet_cpreg, &s, &cpreg)) *\/ */
            /*  /\*   { *\/ */
      ...
      if (match == 0)
        {
          ...
          if (&insn[1] - ajit_opcodes < ajit_num_opcodes
      ...
      else
        {
          ...
          /*   { */
          /*     needed_arch_mask &= */
          /*       ~(AJIT_OPCODE_ARCH_MASK (AJIT_OPCODE_ARCH_V9) - 1); */
          /*     if (! needed_arch_mask) */
          /*       needed_arch_mask = */
          /*         AJIT_OPCODE_ARCH_MASK (AJIT_OPCODE_ARCH_V9); */
          /*   } */

          if (needed_arch_mask
              & AJIT_OPCODE_SUPPORTED (current_architecture))
            /* OK.  */
            ;
          /* Can we bump up the architecture?  */
          else if (needed_arch_mask
                   & AJIT_OPCODE_SUPPORTED (max_architecture))
            {
              enum ajit_opcode_arch_val needed_architecture =
                ajit_ffs (AJIT_OPCODE_SUPPORTED (max_architecture)
                           & needed_arch_mask);

              gas_assert (needed_architecture <= AJIT_OPCODE_ARCH_MAX);
              if (warn_on_bump
                  && needed_architecture > warn_after_architecture)
                {
                  as_warn (_("architecture bumped from \"%s\" to \"%s\" on \"%s\""),
                           ajit_opcode_archs[current_architecture].name,
                           ajit_opcode_archs[needed_architecture].name,
                           str);
          ...
          /* ??? This seems to be a bit fragile.  What if the next entry in
          ...
          else
            {
              ...
              char *p;
              char required_archs[AJIT_OPCODE_ARCH_MAX * 16];

              /* Create a list of the architectures that support the insn.  */
              needed_arch_mask &= ~AJIT_OPCODE_SUPPORTED (max_architecture);
              p = required_archs;
              arch = ajit_ffs (needed_arch_mask);
                  ...
                  if ((1 << arch) & needed_arch_mask)
                    {
                      if (printed_one_p)
                        *p++ = '|';
                      strcpy (p, ajit_opcode_archs[arch].name);
                      p += strlen (p);
                      printed_one_p = 1;
                    }
                  ++arch;
              ...
              as_tsktsk (_(" (Requires %s; requested architecture is %s.)"),
                         required_archs,
                         ajit_opcode_archs[max_architecture].name);
              return special_case;
            }
...
/* Subroutine of ajit_ip to parse an expression.  */
...
static void
output_insn (const struct ajit_opcode *insn, struct ajit_it *theinsn)
{
  char *toP = frag_more (4);

  /* Put out the opcode.  */
  /* fprintf (stderr, "AMV -- INSN_BIG_ENDIAN is: %d\n", INSN_BIG_ENDIAN); */
  /* if (INSN_BIG_ENDIAN) */
  ...
  if (theinsn->reloc != BFD_RELOC_NONE)
    {
      ...
      if (theinsn->reloc == BFD_RELOC_AJIT_OLO10)
...
md_apply_fix (fixS *fixP, valueT *valP, segT segment ATTRIBUTE_UNUSED)
{
...
#ifdef OBJ_ELF
  /* AJIT ELF relocations don't use an addend in the data field.  */
  ...
  if (fixP->fx_r_type == BFD_RELOC_16
      || fixP->fx_r_type == BFD_RELOC_AJIT_UA16)
    {
      md_number_to_chars (buf, val, 2);
    }
  else if (fixP->fx_r_type == BFD_RELOC_32
           || fixP->fx_r_type == BFD_RELOC_AJIT_UA32
           || fixP->fx_r_type == BFD_RELOC_AJIT_REV32)
    {
      md_number_to_chars (buf, val, 4);
    }
  else if (fixP->fx_r_type == BFD_RELOC_64
           || fixP->fx_r_type == BFD_RELOC_AJIT_UA64)
    {
  ...
  else
    {
      ...
      switch (fixP->fx_r_type)
        ...
        case BFD_RELOC_32_PCREL_S2:
          ...
          if (ajit_relax && fixP->fx_where + 8 <= fixP->fx_frag->fr_fix)
            {
#define G0              0
#define O7              15
#define XCC             (2 << 20)
...
#define INSN_NOP        F2(0,4)
              ...
              /*     && (ajit_arch_size == 64 */
              /*         || current_architecture >= AJIT_OPCODE_ARCH_V9)) */
              /*   /\* ba,pt %xcc  *\/ */
...
#define GOTT_INDEX "__GOTT_INDEX__"
      ...
      switch (code)
        ...
        case BFD_RELOC_32_PCREL_S2:
          if (generic_force_reloc (fixp))
            code = BFD_RELOC_AJIT_WPLT30;
          break;
        case BFD_RELOC_HI22:
          code = BFD_RELOC_AJIT_GOT22;
          if (fixp->fx_addsy != NULL)
            {
              if (strcmp (S_GET_NAME (fixp->fx_addsy), GOT_NAME) == 0)
                code = BFD_RELOC_AJIT_PC22;
#ifdef TE_VXWORKS
          ...
          break;
        case BFD_RELOC_LO10:
          code = BFD_RELOC_AJIT_GOT10;
          if (fixp->fx_addsy != NULL)
            {
              if (strcmp (S_GET_NAME (fixp->fx_addsy), GOT_NAME) == 0)
                code = BFD_RELOC_AJIT_PC10;
#ifdef TE_VXWORKS
              if (strcmp (S_GET_NAME (fixp->fx_addsy), GOTT_BASE) == 0
                  || strcmp (S_GET_NAME (fixp->fx_addsy), GOTT_INDEX) == 0)
  ...
  if (bfd_get_section_flags (stdoutput, section) & SEC_DEBUGGING)
    switch (code)
      {
      case BFD_RELOC_16: code = BFD_RELOC_AJIT_UA16; break;
      case BFD_RELOC_32: code = BFD_RELOC_AJIT_UA32; break;
      case BFD_RELOC_64: code = BFD_RELOC_AJIT_UA64; break;
      default: break;
      }

  if (code == BFD_RELOC_AJIT_OLO10)
  ...
  if (code != BFD_RELOC_32_PCREL_S2
      && code != BFD_RELOC_AJIT_WDISP22
      && code != BFD_RELOC_AJIT_WDISP16
      && code != BFD_RELOC_AJIT_WDISP19
      && code != BFD_RELOC_AJIT_WPLT30
      && code != BFD_RELOC_AJIT_TLS_GD_CALL
      && code != BFD_RELOC_AJIT_TLS_LDM_CALL)
    reloc->addend = fixp->fx_addnumber;
  ...
  /* We expand R_AJIT_OLO10 to R_AJIT_LO10 and R_AJIT_13
  ...
  if (code == BFD_RELOC_AJIT_OLO10)
    {
      ...
      *reloc->sym_ptr_ptr
      ...
      reloc->address = fixp->fx_frag->fr_address + fixp->fx_where;
      reloc->howto = bfd_reloc_type_lookup (stdoutput, BFD_RELOC_AJIT13);
...
/* Exactly what point is a PC-relative offset relative TO?
   On the ajit, they're relative to the address of the offset, plus
...
/* This static variable is set by s_uacons to tell ajit_cons_align
   that the expression does not need to be aligned.  */

static int ajit_no_align_cons = 0;

/* This static variable is set by ajit_cons to emit requested types
   of relocations in cons_fix_new_ajit.  */

static const char *ajit_cons_special_reloc;
...
s_uacons (int bytes)
{
  /* Tell ajit_cons_align not to align this value.  */
  ajit_no_align_cons = 1;
  cons (bytes);
  ajit_no_align_cons = 0;
}

/* This handles the native word allocation pseudo-op .nword.
   For ajit_arch_size 32 it is equivalent to .word,  for
   ajit_arch_size 64 it is equivalent to .xword.  */

static void
s_ncons (int bytes ATTRIBUTE_UNUSED)
{
  cons (ajit_arch_size == 32 ? 4 : 8);
}

#ifdef OBJ_ELF
/* Handle the AJIT ELF .register pseudo-op.  This sets the binding of a
   global register.
   The syntax is:
          ...
          if (regname == NULL)
          ...
          else
            {
              ...
              /* Although we actually want undefined_section here,
                 ...
                 We fix this up in ajit_adjust_symtab.  */
...
/* This is called from HANDLE_ALIGN in tc-ajit.h.  */

void
ajit_handle_align (fragS *fragp)
{
  int count, fix;
  char *p;
  ...
  switch (fragp->fr_type)
    ...
    case rs_align_code:
      ...
      /* if (AJIT_OPCODE_ARCH_V9_P (max_architecture) && count > 8) */
      /*   { */
...
#ifdef OBJ_ELF
/* Some special processing for a Ajit ELF file.  */

void
ajit_elf_final_processing (void)
{
  /* Set the Ajit ELF flag bits.  FIXME: There should probably be some
     sort of BFD interface for this.  */
  if (ajit_arch_size == 64)
    {
      switch (ajit_memory_model)
        {
        case MM_RMO:
          elf_elfheader (stdoutput)->e_flags |= EF_AJITV9_RMO;
          break;
        case MM_PSO:
          elf_elfheader (stdoutput)->e_flags |= EF_AJITV9_PSO;
          break;
        default:
          break;
        }
    }
  /* else if (current_architecture >= AJIT_OPCODE_ARCH_V9) */
  /*   elf_elfheader (stdoutput)->e_flags |= EF_AJIT_32PLUS; */
  /* if (current_architecture == AJIT_OPCODE_ARCH_V9A) */
  /*   elf_elfheader (stdoutput)->e_flags |= EF_AJIT_SUN_US1; */
  /* else if (current_architecture == AJIT_OPCODE_ARCH_V9B) */
  /*   elf_elfheader (stdoutput)->e_flags |= EF_AJIT_SUN_US1|EF_AJIT_SUN_US3; */
}

void
ajit_cons (expressionS *exp, int size)
{
  char *save;

  SKIP_WHITESPACE ();
  ajit_cons_special_reloc = NULL;
      ...
      else if (strncmp (input_line_pointer + 3, "tls_dtpoff", 10) == 0)
        {
          if (size != 4 && size != 8)
          ...
          else
            {
              ...
              ajit_cons_special_reloc = "tls_dtpoff";
            }
        }
      if (ajit_cons_special_reloc)
        {
          int bad = 0;
          ...
          if (bad)
            {
              as_bad (_("Illegal operands: Only %%r_%s%d allowed in %d-byte data fields"),
                      ajit_cons_special_reloc, size * 8, size);
            }
          else
            {
              ...
              if (*input_line_pointer != '(')
                {
                  as_bad (_("Illegal operands: %%r_%s%d requires arguments in ()"),
                          ajit_cons_special_reloc, size * 8);
                  bad = 1;
                }
            }
  ...
  if (ajit_cons_special_reloc == NULL)
    expression (exp);
}
                    ...
                    expressionS *exp)
{
  ...
  if (target_little_endian_data
    ...
    r = BFD_RELOC_AJIT_REV32;

  if (ajit_cons_special_reloc)
    {
      if (*ajit_cons_special_reloc == 'd')
        switch (nbytes)
          {
      ...
      else if (*ajit_cons_special_reloc == 'p')
        switch (nbytes)
          {
          case 4: r = BFD_RELOC_AJIT_PLT32; break;
          case 8: r = BFD_RELOC_AJIT_PLT64; break;
          }
      else
        switch (nbytes)
          {
          case 4: r = BFD_RELOC_AJIT_TLS_DTPOFF32; break;
          case 8: r = BFD_RELOC_AJIT_TLS_DTPOFF64; break;
          }
    }
  else if (ajit_no_align_cons)
    {
      switch (nbytes)
        {
        case 2: r = BFD_RELOC_AJIT_UA16; break;
        case 4: r = BFD_RELOC_AJIT_UA32; break;
        case 8: r = BFD_RELOC_AJIT_UA64; break;
        default: abort ();
        }
   }
...
void
ajit_cfi_frame_initial_instructions (void)
{
  cfi_add_CFA_def_cfa (14, ajit_arch_size == 64 ? 0x7ff : 0);
}

int
ajit_regname_to_dw2regnum (char *regname)
{
  char *p, *q;
...
void
ajit_cfi_emit_pcrel_expr (expressionS *exp, unsigned int nbytes)
{
  ajit_cons_special_reloc = "disp";
  ajit_no_align_cons = 1;
