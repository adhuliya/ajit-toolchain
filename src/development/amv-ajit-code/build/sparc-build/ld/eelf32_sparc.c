...
static void 
gldelf32_sparc_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32_sparc_append_to_separated_string (&depaudit, cp);
...
OUTPUT_ARCH(sparc)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/lindata/My-work/Professional/iitb/development/ajit-branch/ajit-toolchain/src/development/amv-ajit-code/trial-install/sparc-install/sparc-buildroot-linux-uclibc/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf32_sparc_handle_option (int optc)
{
  switch (optc)
    ...
    case OPTION_AUDIT:
  	gldelf32_sparc_append_to_separated_string (&audit, optarg); 
	break;

    case 'P':
	gldelf32_sparc_append_to_separated_string (&depaudit, optarg);
	break;
