...
static void gldsun4_find_so
  (lang_input_statement_type *);
static char *gldsun4_search_dir
  (const char *, const char *, bfd_boolean *);
static void gldsun4_check_needed
  (lang_input_statement_type *);
static bfd_boolean gldsun4_search_needed
  (const char *, const char *);
static bfd_boolean gldsun4_try_needed
  (const char *, const char *);
static void gldsun4_find_assignment
  (lang_statement_union_type *);
static void gldsun4_find_exp_assignment
  (etree_type *);
static void gldsun4_count_need
  (lang_input_statement_type *);
static void gldsun4_set_need
  (lang_input_statement_type *);

static void
gldsun4_before_parse (void)
{
...
static void
gldsun4_create_output_section_statements (void)
{
  lang_for_each_input_file (gldsun4_find_so);
}

/* Search the directory for a .so file for each library search.  */

static void
gldsun4_find_so (lang_input_statement_type *inp)
{
  ...
  for (search = search_head; search != NULL; search = search->next)
    {
      ...
      found = gldsun4_search_dir (search->name, inp->filename,
					       &found_static);
...
gldsun4_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = FALSE;
      lang_for_each_input_file (gldsun4_check_needed);
      if (global_found)
	continue;
      ...
      if (! CONST_STRNEQ (lname, "-l"))
	{
	 ...
	 linker script LIB_SEARCH_DIRS.  */
      if (gldsun4_search_needed (command_line.rpath_link,
					      lname))
	continue;
      if (command_line.rpath != NULL)
	{
	  if (gldsun4_search_needed (command_line.rpath, lname))
	    continue;
	}
      else
	{
	  for (search = search_head; search != NULL; search = search->next)
	    if (gldsun4_try_needed (search->name, lname))
	      break;
	  if (search != NULL)
	    continue;
	}
...
static bfd_boolean
gldsun4_search_needed (const char *path, const char *name)
{
  const char *s;
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldsun4_try_needed (dir, name))
	return TRUE;
...
static bfd_boolean
gldsun4_try_needed (const char *dir, const char *name)
{
  char *file;
  char *alc;
  bfd_boolean ignore;
  ...
  file = gldsun4_search_dir (dir, name, &ignore);
  if (file == NULL)
    return FALSE;
...
static void
gldsun4_check_needed (lang_input_statement_type *s)
{
  if (s->filename == NULL)
    return;
...
static void
gldsun4_before_allocation (void)
{
      ...
      for (h = link_info.hash->undefs; h != NULL; h = h->u.undef.next)
	{
	  if (h->type == bfd_link_hash_undefined
	      ...
	      && strcmp (h->root.string, "__GLOBAL_OFFSET_TABLE_") != 0)
	    {
	      ...
	      found_assign = FALSE;
	      lang_for_each_statement (gldsun4_find_assignment);
	      if (! found_assign)
		{
  ...
  if (sneed != NULL)
    {
	 ...
	 field in include/aout/sun4.h.  */
      ...
      lang_for_each_input_file (gldsun4_count_need);
	 ...
	 dynamic object.  */
      ...
      lang_for_each_input_file (gldsun4_set_need);
...
static void
gldsun4_find_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum
      && (find_assign == NULL || ! found_assign))
    gldsun4_find_exp_assignment (s->assignment_statement.exp);
}

/* Look through an expression for an assignment statement.  */

static void
gldsun4_find_exp_assignment (etree_type *exp)
{
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldsun4_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldsun4_find_exp_assignment (exp->binary.lhs);
      gldsun4_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldsun4_find_exp_assignment (exp->trinary.cond);
      gldsun4_find_exp_assignment (exp->trinary.lhs);
      gldsun4_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldsun4_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldsun4_count_need (lang_input_statement_type *inp)
{
  if (inp->the_bfd != NULL
...
static void
gldsun4_set_need (lang_input_statement_type *inp)
{
  if (inp->the_bfd != NULL
      && (inp->the_bfd->flags & DYNAMIC) != 0)
    {
...
static char *
gldsun4_get_script (int *isfile)
{
  *isfile = 1;

  if (link_info.relocatable && config.build_constructors)
    return "ldscripts/sun4.xu";
  else if (link_info.relocatable)
    return "ldscripts/sun4.xr";
  else if (!config.text_read_only)
    return "ldscripts/sun4.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/sun4.xn";
  else
    return "ldscripts/sun4.x";
}

struct ld_emulation_xfer_struct ld_sun4_emulation =
{
  gldsun4_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  gldsun4_after_open,
  ...
  gldsun4_before_allocation,
  gldsun4_get_script,
  "sun4",
  "a.out-sunos-big",
  finish_default,
  gldsun4_create_output_section_statements,
