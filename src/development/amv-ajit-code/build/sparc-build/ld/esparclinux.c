...
/* Linux a.out emulation code for sparclinux
...
static void
gldsparclinux_before_parse (void)
{
...
static bfd_boolean
gldsparclinux_open_dynamic_archive
...
static void
gldsparclinux_find_address_statement (lang_statement_union_type *s)
{
...
static void
gldsparclinux_create_output_section_statements (void)
{
  lang_for_each_statement (gldsparclinux_find_address_statement);
}
...
static void
gldsparclinux_before_allocation (void)
{
  ...
  /* Let the backend work out the sizes of any sections required by
     dynamic linking.  */
  if (! bfd_sparclinux_size_dynamic_sections (link_info.output_bfd,
						     &link_info))
...
static char *
gldsparclinux_get_script (int *isfile)
{
  *isfile = 1;

  if (link_info.relocatable && config.build_constructors)
    return "ldscripts/sparclinux.xu";
  else if (link_info.relocatable)
    return "ldscripts/sparclinux.xr";
  else if (!config.text_read_only)
    return "ldscripts/sparclinux.xbn";
  else if (!config.magic_demand_paged)
    return "ldscripts/sparclinux.xn";
  else
    return "ldscripts/sparclinux.x";
}

struct ld_emulation_xfer_struct ld_sparclinux_emulation =
{
  gldsparclinux_before_parse,
  syslib_default,
  hll_default,
  after_parse_default,
  after_open_default,
  ...
  gldsparclinux_before_allocation,
  gldsparclinux_get_script,
  "sparclinux",
  "a.out-sparc-linux",
  finish_default,
  gldsparclinux_create_output_section_statements,
  gldsparclinux_open_dynamic_archive,
  NULL,	/* place orphan */
  NULL,	/* set symbols */
  NULL,	/* parse args */
