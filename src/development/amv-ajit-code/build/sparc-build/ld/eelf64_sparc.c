...
static lang_output_section_statement_type *gldelf64_sparc_place_orphan
  (asection *, const char *, int);

static void
gldelf64_sparc_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static bfd_boolean
gldelf64_sparc_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf64_sparc_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf64_sparc_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf64_sparc_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf64_sparc_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf64_sparc_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf64_sparc_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf64_sparc_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf64_sparc_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf64_sparc_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf64_sparc_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf64_sparc_write_build_id_section (bfd *abfd)
{
...
gldelf64_sparc_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf64_sparc_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf64_sparc_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf64_sparc_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf64_sparc_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf64_sparc_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldelf64_sparc_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf64_sparc_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf64_sparc_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf64_sparc_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf64_sparc_find_exp_assignment (exp->binary.lhs);
      gldelf64_sparc_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf64_sparc_find_exp_assignment (exp->trinary.cond);
      gldelf64_sparc_find_exp_assignment (exp->trinary.lhs);
      gldelf64_sparc_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf64_sparc_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf64_sparc_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf64_sparc_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void 
gldelf64_sparc_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
gldelf64_sparc_before_allocation (void)
{
  ...
  lang_for_each_statement (gldelf64_sparc_find_statement_assignment);
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf64_sparc_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldelf64_sparc_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf64_sparc_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
gldelf64_sparc_after_allocation (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);
  gldelf64_sparc_map_segments (need_layout);
}
...
OUTPUT_ARCH(sparc:v9)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/lindata/My-work/Professional/iitb/development/ajit-branch/ajit-toolchain/src/development/amv-ajit-code/trial-install/sparc-install/sparc-buildroot-linux-uclibc/lib64\"); SEARCH_DIR(\"/lindata/My-work/Professional/iitb/development/ajit-branch/ajit-toolchain/src/development/amv-ajit-code/trial-install/sparc-install/sparc-buildroot-linux-uclibc/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf64_sparc_handle_option (int optc)
{
  switch (optc)
    ...
    case OPTION_AUDIT:
  	gldelf64_sparc_append_to_separated_string (&audit, optarg); 
	break;

    case 'P':
	gldelf64_sparc_append_to_separated_string (&depaudit, optarg);
	break;
...
static void
gldelf64_sparc_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf64_sparc_emulation =
{
  ...
  NULL,
  gldelf64_sparc_open_dynamic_archive,
  gldelf64_sparc_place_orphan,
  NULL,
  NULL,
  gldelf64_sparc_add_options,
  gldelf64_sparc_handle_option,
  NULL,
  gldelf64_sparc_list_options,
  gldelf64_sparc_load_symbols,
  NULL,
  NULL
};
