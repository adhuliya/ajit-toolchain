...
// bypass.
//--------------------------------------------------------------------------------
void      __ajit_write_serial_1_control_register__(uint32_t val);
uint32_t  __ajit_read_serial_1_control_register__();
void      __ajit_write_serial_1_tx_register__(uint8_t val);
uint8_t   __ajit_read_serial_1_tx_register__();
uint8_t   __ajit_read_serial_1_rx_register__();


int   		__ajit_serial_1_putchar__  (char c);
int   		__ajit_serial_1_getchar__ ();
void  		__ajit_serial_1_puts__ (char* s, uint32_t length);
void  		__ajit_serial_1_gets__ (char* s, uint32_t length);
void  		__ajit_serial_1_configure__ (uint8_t enable_tx, uint8_t enable_rx, uint8_t enable_intr);
uint32_t    	__ajit_read_serial_1_baud_limit_register__();
void        	__ajit_write_serial_1_baud_limit_register__(uint32_t bcv);
uint32_t    	__ajit_read_serial_1_baud_frequency_register__();
void        	__ajit_write_serial_1_baud_frequency_register__(uint32_t bcv);

// baud rate and reset.
void 		__ajit_serial_1_set_baudrate__ (uint32_t baud_rate, uint32_t clock_frequency);
void  		__ajit_serial_1_set_uart_reset__ (uint8_t reset_value);
