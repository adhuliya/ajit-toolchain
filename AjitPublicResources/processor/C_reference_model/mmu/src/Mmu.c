...
#define __CACHEABLE__ ((MmuControlRegister & 0x800) != 0)



//Helper functions for Mmu
void 	  initializeMmuState();
...
void      updateFsrFar(uint32_t Fsr_val, uint32_t Far_val, uint8_t fault_class);
uint32_t  readMmuRegister(uint32_t addr);
void 	  writeMmuRegister(uint32_t addr, uint8_t byte_mask, uint64_t data64);
uint8_t   translateToPhysicalAddress(uint8_t asi, uint32_t virt_addr, uint8_t request_type, uint64_t* physical_addr, uint8_t*  cacheable, uint32_t* mmu_fsr_to_be_returned);
uint32_t  readPageTableEntryFromMemory(uint64_t physical_addr);
void      writePageTableEntryToMemory(uint32_t pte, uint64_t physical_addr);
uint8_t   walkPageTables(uint32_t virt_addr, uint32_t* pte, uint8_t* pte_level, uint64_t* phy_addr_of_pte);
uint8_t   checkPageFaults(uint8_t pte_found, uint32_t pte, uint8_t pte_level, uint8_t asi, uint32_t virt_addr, uint8_t request_type, uint32_t* mmu_fsr_to_be_returned);
uint64_t  constructPhysicalAddr(uint32_t pte, uint8_t pte_level, uint32_t virt_addr);
uint8_t   isCacheable(uint32_t pte);
uint8_t   flushPTEfromTLB(uint32_t TLB_index);
uint32_t  getEmptyTLBLocation();
uint8_t   flushEntireTLB();
uint8_t   performMmuProbe(uint32_t virt_addr, uint8_t asi, uint8_t request_type, uint32_t* PTE);
...
void Mmu()
{
	...
	uint8_t  mae;
	uint8_t cacheable;
	uint8_t  source; //0=> request is from Icache, 1=> request is from Dcache
	...
	uint32_t mmu_fsr_to_be_returned = 0;

	
	//Initialize Mmu state variables
	initializeMmuState();
	...
	while(1)
	{
		...
		//wait for a request from caches
		readCacheToMmuRequest(&asi, &addr, &request_type, &byte_mask, &data, &source);
		#ifdef MMU_DEBUG
		printf("\nMMU: Received MMU request from Cache : asi=0x%x, addr=0x%x, request_type=0x%x, byte_mask=0x%x, data=0x%lx, source=0x%x",asi,addr, request_type, byte_mask,data,source);
		#endif
		...
		if(request_type == REQUEST_TYPE_WRFSRFAR)
		{
			uint32_t fsr_from_dcache = getSlice64(data, 63, 32);
			uint32_t far_from_dcache = getSlice64(data, 31, 0);

			// update based on fault-type etc.
			// Note that a WRFSRFAR request can be
			// created only due to an Ifetch that faulted.
			updateFsrFar(fsr_from_dcache, far_from_dcache, IACCESS_FAULT);

			mae=0;
			data=0;
			cacheable=0;
			sendMmuToCacheResponse(mae,addr, data,cacheable,source,0);	 // last-field is fsr-to-cache, no fault hence 0.
			continue;
		}
		...
		//and be forwarded untranslated to the system bus
			...
			|| (!MmuEnabled() && //Mmu disabled and I/D request
			...
			))
		{
			...
			sendMmuToMemoryRequest(request_type, addr, data, byte_mask);

			//wait for a response from Memory
			readMemoryToMmuResponse(&data);

			//send the response back to the cache
			//that created this request
			mae=0;
			cacheable= __CACHEABLE__;
			sendMmuToCacheResponse(mae,addr, data,cacheable,source,0); // no fault, last field fsr-to-cache = 0.

			Num_Mmu_bypass_accesses+=1;
		}
		...
		else if(asi==ASI_MMU_FLUSH_PROBE)
		{
			...
			if(request_type==REQUEST_TYPE_READ)
			{
				...
				uint32_t pte=0;
				probe_success = performMmuProbe(addr, asi, request_type, &pte);
				if(probe_success)
				{
					mae=0;
					cacheable=0;
					data=pte;
					if(getBit32(addr,2)==0) data = data<<32;
					sendMmuToCacheResponse(mae,addr, data,cacheable,source,0); //fsr to be sent to cache=0
				...
				else
				{
					...
					mae=0;
					cacheable=0;
					sendMmuToCacheResponse(mae,addr, data,cacheable,source,0); // DCACHE request fsr-to-cache=0.
				}
				Num_Mmu_probe_requests+=1;
			}
			...
			else if(request_type==REQUEST_TYPE_WRITE)
			{
				...
				uint8_t success = flushEntireTLB();
				fprintf(stderr,"----------- MMU flushed TLB due to MMU-FLUSH-PROBE-WRITE ----------------- \n");
				if(success) mae =0 ; else mae=1;
				//send response to cpu
				data=0;	cacheable=0;
				sendMmuToCacheResponse(mae,addr, data,cacheable,source,0); // DCACHE request, fsr-to-cache=0.
				Num_Mmu_flush_requests+=1;
			}
			else
			{
				#ifdef SW
		...
		else if(asi==ASI_MMU_REGISTER)
		{
			if(request_type==REQUEST_TYPE_READ)
			{
				data=readMmuRegister(addr);
				//send the value on bits 63:32 of the 64 bit data-bus
				//because all mmu-register addresses are even-addresses.
				data=data<<32; 
				//send response to cpu
				mae=0; 	cacheable=0;
				sendMmuToCacheResponse(mae,addr, data,cacheable,source,0); // DCACHE side request, fsr-to-cache=0.
				Num_Mmu_register_reads+=1;
			...
			else if(request_type==REQUEST_TYPE_WRITE)
			{
				...
				writeMmuRegister(addr,byte_mask,data);
				//send response to cpu
				mae=0; data=0; 	cacheable=0;
				sendMmuToCacheResponse(mae,addr, data,cacheable,source,0); // DCACHE side request, fsr-to-cache=0.
				Num_Mmu_register_writes+=1;
			}
		}
		...
		//Perform virtual to physical translation!
		{
			...
			translation_success = translateToPhysicalAddress(asi, addr, request_type, 
										&physical_addr, 
										&cacheable, 
										&mmu_fsr_to_be_returned);
			...
			{
				//forward the request to System bus
				sendMmuToMemoryRequest(request_type, physical_addr, data, byte_mask);
				//wait for a response from Memory
				readMemoryToMmuResponse(&data);
				//send the response back to the cache
				//that created this request
				...
				sendMmuToCacheResponse(mae,addr, data,cacheable,source, mmu_fsr_to_be_returned);
				Num_Mmu_translated_accesses+=1;
			}
			else
			{
				//******A fault has occured*****.
				...
				uint8_t NF = getBit32(MmuControlRegister,1);
				if(NF==0 || asi==0x09)
				{
				...
				{
					sendMmuToCacheResponse(mae,addr, data,cacheable,source, 0); //fsr=0
				}
			}
...
uint8_t performMmuProbe(uint32_t virt_addr, uint8_t asi, uint8_t request_type, uint32_t* PTE)
{
	...
	*PTE=0;
	...
	if(TLB_ENABLED)
	{
		...
		TLB_hit = isTLBHit(va_tag, MmuContextRegister, &TLB_index);
		if(TLB_hit)
		{
			//get the pte
			pte=TLB_entries[TLB_index].pte;
			pte_level=TLB_entries[TLB_index].pte_level;
			pte_found=1;
			*PTE=pte;
			return 1;
	...
	if(!TLB_ENABLED || !TLB_hit)
	{
		...
		pte_found = walkPageTables(virt_addr, &pte, &pte_level, &phy_addr_of_pte);

		//check if PTE is valid, else update FSR and FAR registers
		if(!pte_found)
		{
			uint32_t fsr_to_be_updated_after_probe =0;
			checkPageFaults(pte_found, pte, pte_level, asi, virt_addr, request_type, &fsr_to_be_updated_after_probe); 
			*PTE=0;
			return 0;
		}
		else
...
//	return 0
uint8_t translateToPhysicalAddress(uint8_t asi, uint32_t virt_addr, uint8_t request_type, uint64_t* physical_addr, uint8_t* cacheable,
	...
	*mmu_fsr_to_be_returned = 0;
	...
	if(TLB_ENABLED)
	{
		...
		if(TLB_hit)
		{
			...
			//for debugging of TLB :
			//Check if the PTE found is same 
			//as the one present in memory
			/*
			#ifdef SW
			//------------------------------
			{
				...
				pte_found_in_mem = walkPageTables(virt_addr, &pte_in_mem, &pte_level_in_mem, &phy_addr_of_pte_in_mem);
				if(!pte_found_in_mem)
					printf(" ERROR : PTE present in TLB but not found in page tables in memory for asi=0x%x, address=0x%x, request_type=0x%x",asi,virt_addr,request_type);
				else if(pte!=pte_in_mem)
				{
					printf(" ERROR : PTE present in TLB not same as one in memory for asi=0x%x, address=0x%x, request_type=0x%x",asi,virt_addr,request_type);
					printf("         PTE in TLB = 0x%x, PTE in memory = 0x%x", pte, pte_in_mem);
				}
			}
			#endif
			*/
	...
	access_allowed = checkPageFaults(pte_found, pte, pte_level, asi, virt_addr, request_type, mmu_fsr_to_be_returned);
	...
	if(TLB_ENABLED && !TLB_hit)
	{
		...
		TLB_index = getEmptyTLBLocation();
		writeTLBentry(TLB_index, va_tag, MmuContextRegister, pte, pte_level);
		TLB_hit=1;
	...
	if(!(updated_pte==pte))
	{
		//write the updated pte back to memory.
		{
			...
			else
			{
				...
				writePageTableEntryToMemory(updated_pte, phy_addr_of_pte);
			}
		}
		...
		if(TLB_ENABLED)
		{
			updateTLBentry(TLB_index, updated_pte);
		}
	}
...
uint32_t readPageTableEntryFromMemory(uint64_t physical_addr)
{
	...
	//send READ request to System bus
	sendMmuToMemoryRequest(request_type, physical_addr, data64, byte_mask);
	//wait for a response from Memory
	readMemoryToMmuResponse(&data64);
...
uint8_t walkPageTables(uint32_t virt_addr, uint32_t* pte, uint8_t* pte_level, uint64_t* phy_addr_of_pte)
{
	...
	uint32_t context_table_index=MmuContextRegister;	   //Index into context table
	...
	PTE_LEVEL =0;
	phy_addr = getPhyAddrFromPTD(MmuContextTablePointerRegister, context_table_index);
	PTE=readPageTableEntryFromMemory(phy_addr);
...
uint8_t checkPageFaults(uint8_t pte_found, uint32_t pte, uint8_t pte_level, uint8_t asi, uint32_t virt_addr, uint8_t request_type, uint32_t* fsr_to_cache)
{
	...
	else
	{
		...
		if(request_type != REQUEST_TYPE_IFETCH)
		{
			...
			updateFsrFar(new_fsr_value, virt_addr, DACCESS_FAULT);
...
void initializeMmuState()
{
	MmuControlRegister=0;
	MmuContextTablePointerRegister=0;
	MmuContextRegister=0;
	MmuFaultStatusRegister=0;
	MmuFaultAddressRegister=0;
	Mmu_FSR_FAULT_CLASS=NOFAULT;

	Num_Mmu_bypass_accesses=0;
	Num_Mmu_probe_requests=0;
	Num_Mmu_flush_requests=0;
	Num_Mmu_register_reads=0;
	Num_Mmu_register_writes=0;
	Num_Mmu_translated_accesses=0;
	Num_Mmu_TLB_hits=0;
	initializeTLB();
}

void printMmuStatistics()
{
#ifdef SW
...
uint32_t readMmuRegister(uint32_t addr)
{
	...
	uint32_t result;
	if	(register_id==0) result = MmuControlRegister;
	else if (register_id==1) result = MmuContextTablePointerRegister;
	else if (register_id==2) result = MmuContextRegister;
	else if (register_id==3) 
	{
		result = MmuFaultStatusRegister;
		//A read to the FSR clears it
		MmuFaultStatusRegister = 0;
		Mmu_FSR_FAULT_CLASS = NOFAULT;
	}
	else if (register_id==4) result = MmuFaultAddressRegister;
	else
	{
		#ifdef SW
...
void writeMmuRegister(uint32_t addr, uint8_t byte_mask, uint64_t data64)
{
	...
	if(!(byte_mask==0x0f || byte_mask==0xf0) || register_id >4)
	{
		...
		addr = 0x%x, byte_mask=0x%x, data=0x%lx", addr, byte_mask, data64);
		#endif
	}

	if	(register_id==0)  {
		MmuControlRegister		=data32;
	}
	else if (register_id==1) {
		 MmuContextTablePointerRegister=data32;
	}
	else if (register_id==2) {
		 MmuContextRegister 		=data32;
		 fprintf(stderr,"------ Mmu context-register = %x --------------\n", data32);
	}
	...
	{
		flushEntireTLB();
		fprintf(stderr,"------ MMU TLB flushed due to context-table-pointer-write (=%x) ------\n",
					data32);
...
uint8_t MmuEnabled()
{
	uint8_t Enable_bit = getBit32(MmuControlRegister,0);
	return Enable_bit;
}
...
uint8_t  flushPTEfromTLB(uint32_t TLB_index)
{
	...
	TLB_entries[TLB_index].va_tag=0;
	TLB_entries[TLB_index].ctx_tag=0;
	TLB_entries[TLB_index].pte=0;
	TLB_entries[TLB_index].pte_level=0;
	return 1;
}
...
uint8_t  flushEntireTLB()
{
	...
	if(TLB_ENABLED)
	{
		...
		{
			if(flushPTEfromTLB(i)==0) return 0; //there was an error
		}
	}
	return 1; //success
}
...
void updateFsrFar(uint32_t Fsr_val, uint32_t Far_val, uint8_t fault_class)
{
	...
	if( ((Mmu_FSR_FAULT_CLASS == IACCESS_FAULT) && (fault_class==IACCESS_FAULT)) 
	 || ((Mmu_FSR_FAULT_CLASS == DACCESS_FAULT) && (fault_class==DACCESS_FAULT)) )
	{
		//set the OW bit
	...
	if((Mmu_FSR_FAULT_CLASS == NOFAULT) || (Mmu_FSR_FAULT_CLASS==IACCESS_FAULT) || (fault_class==DACCESS_FAULT))
	{
		//Write the new fault status/addr
		MmuFaultStatusRegister = Fsr_val;
		MmuFaultAddressRegister = Far_val;
		Mmu_FSR_FAULT_CLASS = fault_class;
