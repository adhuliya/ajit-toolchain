		...
		StatusRegisters *status_reg, uint32_t trap_vector, uint8_t asi, uint8_t rd, uint8_t *flags, ProcessorState *state)
{
	...
	uint8_t is_fp_trap = ((op == _LDF_ || op == _LDDF_ || op == _LDFSR_) && ((!ef) || !getBpFPUPresent()));
	...
	uint8_t is_cp_trap = ((op == _LDC_ || op == _LDDC_ || op == _LDCSR_) && ((!ec) || !getBpCPPresent()));
	...
	uint8_t is_mem_address_not_aligned = ((((op == _LDD_) || (op == _LDDA_) || (op == _LDDF_) || (op == _LDDC_)) 
			...
			|| (((op == _LD_) || (op == _LDA_) || (op == _LDF_) || (op == _LDFSR_) 
					|| (op == _LDC_) || (op == _LDCSR_)) 
				&& (getSlice32(address, 1, 0))) 
			|| (((op == _LDSH_) || (op == _LDSHA_) || (op == _LDUH_) || (op == _LDUHA_)) 
				&& (getBit32(address, 0))));

	if(!is_privileged && is_mem_address_not_aligned)
	{
		tv = setBit32(tv, _TRAP_, 1);
	...
	if(!skip_read)	
	{
		...
		//inform the HW server about the memory access
		inform_HW_server(GDB_MEM_ACCESS, state, address+4);
	}
	if(mae2)
	{	tv = setBit32(tv, _TRAP_, 1) ;
		tv = setBit32(tv, _DATA_ACCESS_EXCEPTION_, 1);
...
//
uint32_t executeStore( Opcode op, uint32_t operand1, uint32_t operand2, uint32_t *result_h, uint32_t *result_l, uint32_t data0,	uint32_t data1, StatusRegisters *status_reg, uint32_t trap_vector, uint8_t asi, uint8_t rd, ProcessorState *state)
{
#ifdef DEBUG
	...
	uint8_t is_fp_trap = (((op == _STF_) || (op == _STDF_) || (op == _STFSR_) || (op == _STDFQ_)) && ((!ef) || !getBpFPUPresent()));
	...
	uint8_t is_cp_trap = (((op == _STC_) || (op == _STDC_) || (op == _STCSR_) || (op == _STDCQ_)) && ((!ec) || !getBpCPPresent()));
	if(!is_privileged && is_cp_trap)
	{
		tv = setBit32(tv, _TRAP_, 1);
		tv = setBit32(tv, _CP_DISABLED_, 1);
	...
	if(!is_trap1)	
	{
			if(!is_dw)
			{
				...
				writeData(addr_space, address, byte_mask, data0, &mae1);
			}
			else
			{
				...
				writeData64(addr_space, address , 0xFF, data64, &mae1);
...
//
void testAndSetBlockLdstFlags(uint8_t byte_flag, uint8_t word_flag)
{
	while((getPbBlockLdstByte() == 1) || (getPbBlockLdstWord() == 1))
	{
	}
	setPbBlockLdstByte( byte_flag);
	setPbBlockLdstWord( word_flag);
}

uint32_t executeLdstub(Opcode op, uint32_t operand1, uint32_t operand2, uint32_t *result, StatusRegisters *status_reg, StateUpdateFlags* reg_update_flags,
		uint32_t trap_vector, uint8_t asi, uint8_t *flags)
{
	...
	//
	if(!is_privileged)
		testAndSetBlockLdstFlags( 1, 0);
	uint8_t mae1=0;
	if(!is_privileged)
		readData(addr_space, address, &mae1,&data);
	if(mae1)
	{
		tv = setBit32(tv, _TRAP_, 1);
		tv = setBit32(tv, _DATA_ACCESS_EXCEPTION_, 1);
	...
	if(!is_trap)
	{
		writeData(addr_space, address, byte_mask, 0xFFFFFFFF, &mae2);
...
uint32_t executeSwap( Opcode op, uint32_t operand1, uint32_t operand2, uint32_t *result, StatusRegisters *status_reg, StateUpdateFlags* reg_update_flags, uint32_t trap_vector, uint8_t asi, uint32_t temp, uint8_t *flags)
{
	...
	uint8_t is_alignment_trap = getSlice32(address, 1, 0);
	if(is_alignment_trap)
	{
		tv = setBit32(tv, _TRAP_, 1) ;
		tv = setBit32(tv, _MEM_ADDRESS_NOT_ALIGNED_, 1);
	}

	uint8_t is_alternate = (op == _SWAPA_);
	uint8_t is_privileged = (is_alternate && !s);


	uint32_t word = 0;
	uint8_t mae1 = 0;
	if(!is_privileged && !is_alignment_trap) 
	{
		// wait until BlockLdstByte and BlockLdstWord are both 0
		testAndSetBlockLdstFlags(0, 1);
		readData( addr_space, address, &mae1, &word);
		if(mae1)
		{
			tv = setBit32(tv, _TRAP_, 1) ;
			tv = setBit32(tv, _DATA_ACCESS_EXCEPTION_, 1);
	...
	if(!skip_write)
	{
		writeData(addr_space, address, 0xF, temp,&mae2);
...
uint32_t executeBfpcc(Opcode op, uint32_t operand1, StatusRegisters *status_reg, uint32_t trap_vector, uint8_t flags)
{
#ifdef DEBUG
	...
	uint8_t fp_disabled = !getBpFPUPresent() || (getBit32(status_reg->psr, 12) == 0);
	uint8_t fp_exception = getBpFPUException();
...
uint32_t executeBcpcc( Opcode op, uint32_t operand1, StatusRegisters *status_reg, uint32_t trap_vector, uint8_t flags)
{
#ifdef DEBUG
	...
	uint8_t cp_disabled = !getBpCPPresent() || (getBit32(status_reg->psr, 13) == 0);
	uint8_t cp_exception = getBpCPException();
...
void executeCall( uint32_t operand1, StatusRegisters *status_reg, StateUpdateFlags* suf)
{
#ifdef DEBUG
...
uint32_t executeReadStateReg( Opcode op, uint8_t rs1, uint32_t *result, StatusRegisters *status_reg, uint32_t trap_vector,
		uint8_t *flags)
{
#ifdef DEBUG
	...
	case(_RDASR_) :{ res = sr.asr[rs1] ;    break;}
...
void executeStbar( uint8_t *store_barrier_pending, StateUpdateFlags* reg_update_flags)
{
	sendSTBAR();
#ifdef DEBUG
...
uint32_t executeFlush(uint32_t flush_addr, uint32_t trap_vector, StateUpdateFlags* reg_update_flags)
{
	...
	uint8_t mae=0;
	writeData(ASI_FLUSH_I_D_CONTEXT, flush_addr, 0x00, 0x00, &mae);
	...
	flushIcacheLine(flush_asi, flush_addr, &flush_mae);
...
uint32_t executeInstruction( Opcode opcode, uint32_t operand2, uint32_t operand1, uint32_t *result_h, uint32_t *result_l, uint8_t *flags, ProcessorState *s, uint8_t rs1, uint8_t rd, uint8_t asi, uint32_t data1, uint32_t data0)
{
	...
	else if(is_atomic) 	tv = 	executeLdstub(opcode, operand1, operand2, result_l, status_reg,&(s->reg_update_flags), trap_vector, asi, flags);
	else if(is_swap) 	tv = 	executeSwap(opcode, operand1, operand2, result_l, status_reg,&(s->reg_update_flags), trap_vector, asi, data0, flags);
	else if(is_logical) 	     	executeLogical(opcode, operand1, operand2, result_l, status_reg, &(s->reg_update_flags), flags);
	else if(is_nop) 	     	executeNop();
	else if(is_sethi) 	       executeSethi(operand1, result_l, flags);
	else if(is_shift) 	       executeShift(opcode, operand1, operand2, result_l, flags);
	else if(is_add) 	       executeAdd(opcode, operand1, operand2, result_l, status_reg, &(s->reg_update_flags), flags);
	else if(is_tadd)	tv = 	executeTAdd(opcode, operand1, operand2, result_l, status_reg, &(s->reg_update_flags), trap_vector, flags);
	else if(is_sub)		       executeSub(opcode, operand1, operand2, result_l, status_reg, &(s->reg_update_flags),flags);
	else if(is_tsub)	tv =	executeTSub(opcode, operand1, operand2, result_l, status_reg, &(s->reg_update_flags),trap_vector, flags);
	else if(is_multiply)	       executeMul(opcode, operand1, operand2, result_l, status_reg, &(s->reg_update_flags),flags);
	else if(is_mul_step)	       executeMulStep(operand1, operand2, result_l, status_reg, &(s->reg_update_flags),flags);
	else if(is_divide)	tv = 	executeDiv(opcode, operand1, operand2, result_l, status_reg, &(s->reg_update_flags),trap_vector, flags);
	...
	else if(is_bfpcc)	tv = 	executeBfpcc(opcode, operand1, status_reg, trap_vector, *flags);
	else if(is_bcpcc)	tv = 	executeBcpcc( opcode, operand1, status_reg, trap_vector, *flags);
	else if(is_call) 	     executeCall(operand1, status_reg, &(s->reg_update_flags));
	...
	else if(is_read_state_reg) 	tv =	executeReadStateReg(opcode, rs1, result_l, status_reg, trap_vector, flags);
	else if(is_write_state_reg)	tv =	executeWriteStateReg(opcode, operand1, operand2, rd, status_reg, &(s->reg_update_flags), trap_vector);
	else if(is_stbar)		    	executeStbar(&(s->store_barrier_pending), &(s->reg_update_flags));
	else if(is_unimp)		tv =	executeUnImplemented(trap_vector);
	else if(is_flush)		tv =	executeFlush((operand1 + operand2),
							trap_vector,&(s->reg_update_flags));
...
void selectTrap( uint32_t *trap_vector, uint32_t *tbr, uint8_t et, ProcessorMode *p_mode, uint8_t *interrupt_level,
		uint8_t ticc_trap_type)
{
	uint32_t tv = *trap_vector;
	uint8_t tt = 0;
		...
		uint8_t ticc_trap_type, uint32_t *pc, uint32_t *npc)
{
	...
	if(!skip_write1)
	{
		writeRegister( 17, new_cwp, pc_tmp); // %l1
		writeRegister( 18, new_cwp, npc_tmp);// %l2
   	}

	uint8_t skip_write2 = ( is_error_mode || !is_annul);
	if(!skip_write2)
	{
		writeRegister( 17, new_cwp, npc_tmp); // %l1
		writeRegister( 18, new_cwp, (npc_tmp + 4)); // %l2
	}
	...
	if(!skip_write4 )
	{
	  ...
	  pc_tmp  = PC_RESET_VALUE;
	  npc_tmp = NPC_RESET_VALUE;
...
uint32_t executeFPInstruction( Opcode opcode, uint32_t operand2_3, uint32_t operand2_2, uint32_t operand2_1, uint32_t operand2_0,
		uint32_t operand1_3, uint32_t operand1_2, uint32_t operand1_1, uint32_t operand1_0, uint32_t *result_h2, uint32_t *result_h1,
		uint32_t *result_l2, uint32_t *result_l1, uint32_t trap_vector, uint8_t *flags, uint8_t *ft, StatusRegisters *ptr)
{
