...
void core_icache_start_daemons(FILE* fp); // from aa2c model.
void core_dcache_start_daemons(FILE* fp); // from aa2c model.
			...
			FILE* logger_trace_file)    
{
	...
	//
	// register mmu-pipes (MMU-v2 C model for now)
	//
	register_mmu_pipes();
	...
	//start cache and mmu threads
	core_icache_start_daemons(logger_trace_file); // from aa2c model.
	core_dcache_start_daemons(logger_trace_file); // from aa2c model.
	start_mmu_threads(USE_DUMMY_MMU);
...
void register_cpu_pipes(int sw_flag, int use_display_driver, int use_kbd_driver)
{
	...
	set_pipe_is_written_into("ICACHE_to_CPU_fsr");
