...
gpbState* makeGpbState (uint32_t core_id, uint32_t thread_id, int port_number)
{
	...
	ret_state->core_id = core_id;
	ret_state->thread_id = thread_id;
	...
	{
		sprintf(ret_state->command_pipe,"COMMAND_TO_DEBUG_SERVER_%d_%d", core_id, thread_id);
		sprintf(ret_state->response_pipe,"RESPONSE_FROM_DEBUG_SERVER_%d_%d", core_id, thread_id);
	}
	else
	{
		sprintf(ret_state->command_pipe,"ENV_to_AJIT_debug_command_%d_%d", core_id, thread_id);
		sprintf(ret_state->response_pipe,"AJIT_to_ENV_debug_response_%d_%d", core_id, thread_id);
	}
...
int gpb_init (gpbState* gpb_state)
{
	...
	fprintf(stderr,"\nInfo:gpb_init: initialized the server for core %d, thread %d, port %d, socket %d.\n", 
				gpb_state->core_id, gpb_state->thread_id, gpb_state->port_number,
				newsockt);
...
#ifdef GDB_DBG
	printf("SWServer (core,thread=%d,%d): Received GDB message - %s\n",gpb_state->core_id,
					gpb_state->thread_id, message);
#endif
...
#ifdef GDB_DBG
			printf ("SWServer (core,thread=%d,%d) : Set Breakpoint at PC = %x, BP Reg %u\n", 
					gpb_state->core_id,
					gpb_state->thread_id,
					address, i);
#endif
...
#ifdef GDB_DBG
			printf ("SWServer (core,thread=%d,%d): Set Watchpoint at PC = %x, WP Reg %u\n", 
					gpb_state->coreid, gpb_state->thread_id, address, i);
#endif
...
#ifdef GDB_DBG
			printf ("SWServer (core,thread=%d,%d): Cleared Breakpoint at address = %x\n", 
					gpb_state->core-id, gpb_state->thread_id, address);
#endif
...
#ifdef GDB_DBG
			printf ("SWServer (core,thread=%d,%d) : Cleared Watchpoint at address = %x in Reg %d\n", 
					gpb_state->core_id, gpb_state->thread_id, address, i);
#endif
