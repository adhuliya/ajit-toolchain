...
void perform_validation (uint32_t line_num)
{
	...
	uint8_t err = 0;
	if ((rec.core_id >= 0) && (rec.core_id < 4))
	{
		if((rec.thread_id >= 0 ) && (rec.thread_id < 2))
		{
			sprintf(cmd_pipe, "COMMAND_TO_DEBUG_SERVER_%d_%d", rec.core_id, rec.thread_id);
			sprintf(resp_pipe,"RESPONSE_FROM_DEBUG_SERVER_%d_%d", rec.core_id, rec.thread_id);
