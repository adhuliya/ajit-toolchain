...
//
// to be used to initialize flash memory through the debug
// interface..  
// 
int dbg_load_mmap_into_flash(char* memoryMapFile, uint32_t spi_master_base_address, int addr_nbytes, uint8_t erase_opcode)
{
	FILE * file;
	file= fopen(memoryMapFile, "r");
	if(!file)
	...
	fprintf(stderr, "Info: opened memory map file %s to write into flash.. erasing flash before writing\n",memoryMapFile);
	spi_flash_erase (spi_master_base_address, erase_opcode);
	...
	while (1)
	{
		...
		if ((current_word_address != masked_addr) || (eof_reached))
		{
			spi_flash_write_mem(spi_master_base_address, current_word_address, current_read_word, addr_nbytes);
			if(global_verbose_flag)
			{
				uint32_t read_back = spi_flash_read_mem(spi_master_base_address, current_word_address, addr_nbytes);
				fprintf(stderr,"Info (flash): initialized mem[0x%x] = 0x%x.\n",  current_word_address,
										current_read_word);
				if(read_back != current_read_word)
					fprintf(stderr,"Error (flash): read-back mem[0x%x] = 0x%x, expected 0x%x.\n",  
										current_word_address, 
										read_back,
										current_read_word);
			}
			...
			if((written_word_count % 1024) == 0)
			{
				fprintf(stderr,"Info (flash): initialized %d words..\n", written_word_count);
			}
		}
		else if (current_word_address == masked_addr)
		{
	...
	fprintf(stderr, "\n Finished initializing memory from file %s into flash.\
			\nLast address written = %x.\n",memoryMapFile, addr);
