...
void print_mystring(void)
{
	char* read_string = (char*)0x100000FF;
...
//--------------------------------setting page table---------------------------------------------------
	...
	// 1 0 0 110 10 = 0x9A user:No Acess. Super:Read/Execute.
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
	uint32_t process1_PTE = ((0x10<<12)<<8) | 0x8E ;//
	uint32_t process2_PTE = ((0xA0<<12)<<8) | 0x8E ;//
	uint32_t process3_PTE = ((0xB0<<12)<<8) | 0x8E ;//
	process1_L3PageTable[0x00] = process1_PTE;
	process2_L3PageTable[0x00] = process2_PTE;
	process3_L3PageTable[0x00] = process3_PTE;


	process2_PTE = ((0x0A000600) | 0x8E );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x06] = process2_PTE;
	process2_PTE = ((0x0F000700) | 0x8E );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x07] = process2_PTE;

	process3_PTE = ((0x0B000600) | 0x8E );//0x9E or 0x9A 0r 0x 92
	process3_L3PageTable[0x06] = process3_PTE;
	process3_PTE = ((0x0F000700) | 0x8E );//0x9E or 0x9A 0r 0x 92
	process3_L3PageTable[0x07] = process3_PTE;

//------------------------------------------------------------------------------------------------------------------
	 uint32_t *j=(uint32_t*) 0xA0000000; //store it at A000 0000 address.
	*j=12;
	char* write_string = (char*)0xA00000FF;
	strcpy(write_string, "\n ***** Hello. This is Process 1's test string at phy addr 0xA00000FF***** \n");
	putString("\n---- Written test string  at phy addr = 0xA00000FF : ");

	write_string = (char*)0xB00000FF;
	strcpy(write_string, "\n ***** Hello. This is Process 2's test string at phy addr 0xB00000FF***** \n");
	putString("\n---- Written test string  at phy addr = 0xB00000FF : ");
	...
	putString("\n----------------Going to process 1-----------------------");
	context_switch(1);
	__asm__ __volatile__("call 0x100063A4; nop; nop;");
	putString("\n----------------Came back from process 1-----------------------");

	putString("\n----------------Going to process 2-----------------------");
	context_switch(2);
	__asm__ __volatile__("call 0x100063A4; nop; nop;");
	putString("\n----------------Came back from process 2-----------------------");
	context_switch(0);
	// //--------------------------------------------------------------------------------------------------------------
