//Test functionality of NF(no fault) bit, when NF=1.
//Thus the instruction fetch should update FSR but not cause a trap.
//The Mmu returns an invalid instruction, and control 
//should go to the illegal instruction trap handler (HW trap 0x2).
//
//
...
//
//Scheme :
//	Enable traps, setup page tables
//	Set NF=1 and enable the MMU
//	Go to user-mode.
//	Perform a user-level instruction fetch to 
//	an address that is not mapepd in the mmu.
//	This should cause a fault in the MMU, and
// 	modify FSR and FAR, but not generate a trap.
// 	
// 	Control should go to illegal instruction trap handler (0x2)
...
int  __attribute__((section(".text.main")))  startTest()
{
	...
	//Now enable the MMU, setting NF=1 
	uint32_t MMU_control_word = 0x3; //Enable=1, NF (no fault)=1
	...
	//switch to USer mode
	__asm__ __volatile__( "mov 0x60, %l0"); 
	__asm__ __volatile__("wr %l0, %psr");
	
	
	__asm__ __volatile__( "set 0xCC2, %g3"); //checkpoint
	
	//Now perform an instruction fetch
	//from an address that is not mapped in the mmu
	//This should cause a page fault and 
	//update FSR and FAR.
	//A trap is not generated. However MMU returns an invalid instruction.
	//and control should go to an illegal instruction trap handler (HW trap 0x2) 
	__asm__ __volatile__( "set 0xA0000000, %l6"); //jump address
	__asm__ __volatile__( "jmpl %l6, %g0");
	__asm__ __volatile__( "nop");
	__asm__ __volatile__( "nop");
