/*----------------------------------------------------------------------------
Filename:			page_error_AT7_S4.c
Version:			1.00
Goal: 				Fault Type=2 for Acess Type=7 and PTE.ACC=4
...
//--------------------------------setting page table---------------------------------------------------
	...
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
		uint32_t PPN = (i<<12);
		uint32_t PTE_level1 = (PPN<<8) | (0x92);//In this any one can read/write/ execute
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	...
	__asm__ __volatile__("sta %g6 , [%l4] 0x09 " );//asi=0x09for supervisor instruction space
