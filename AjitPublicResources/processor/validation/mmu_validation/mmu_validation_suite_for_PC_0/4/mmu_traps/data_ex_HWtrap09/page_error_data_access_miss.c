/*----------------------------------------------------------------------------
Filename:			page_error_data_access_miss.c
Version:			1.00
Goal: 				translation error for PTE with 3 at level3
Author:				Harshal Kalyane
Description:	
			Note: Miss traps for both Instruction and Data are not implemented in current
MMU design
-----------------------------------------------------------------------------*/
...
//--------------------------------setting page table---------------------------------------------------
	...
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
		uint32_t PPN = (i<<12);
		uint32_t PTE_level1 = (PPN<<8) | (0x9E);//In this any one can read/write/ execute
	...
	process2_L1PageTable[0xF0] = convertPhyAddrToPTD((uint32_t)process2_L2PageTable);//
	...
	// 1 0 0 110 10 = 0x9A user:No Acess. Super:Read/Execute.
	// 1 0 0 111 10 = 0x9E user:No Acess. Super:Read/Write/Execute.
	uint32_t process1_PTE = ((0xA0<<12)<<8) | 0x9E ;//0x8E
	uint32_t process2_PTE = ((0x0F000400) | 0x9A );//0x9E or 0x9A 0r 0x 92
	uint32_t process3_PTE = ((0xA0<<12)<<8) | 0x8E;//0x8E
	process1_L3PageTable[0x00] = process1_PTE;
	process2_L3PageTable[0x04] = process2_PTE;
	process3_L3PageTable[0x00] = process3_PTE;

	process2_PTE = ((0x0F000500) | 0x8E );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x05] = process2_PTE;

	process2_PTE = ((0x0F000600) | 0x9A );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x06] = process2_PTE;

	process2_PTE = ((0x0F000700) | 0x9A );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x07] = process2_PTE;

	process2_PTE = ((0x0F000800) | 0x9A );//0x9E or 0x9A 0r 0x 92
	process2_L3PageTable[0x08] = process2_PTE;
...
//------------------------------------------------------------------------------------------------------------------
	...
	*j=12;
	...
	__asm__ __volatile__( "sta  %g0 ,[%g0]0x3  ");
...
HW_trap_0x01: mov 0x01, %g5; nop; ta 0; nop;\
...
HW_trap_0x09: mov 0x09, %g5; nop; ta 0; nop;\
