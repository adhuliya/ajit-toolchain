// Check the functioning of the interrupt-setup
// when there are multiple interrupting sources.
// The serial Rx and Timer, both generate interrupts.
//
//
...
void disableTimer()
{
	mmu_bypass_store(0x0, (uint32_t*)ADDR_TIMER_CONTROL_REGISTER);
}


void enableTimer()
{
	//program the timer to generate an interrupt after
	//2 seconds
	uint32_t Timer_max_count = 200; // delay(in seconds) = Timer_max_count * 1 milli seconds
	uint32_t Timer_control_word = (Timer_max_count)<<1 | 0x1; //enable bit
	//write the control word
	mmu_bypass_store(Timer_control_word, (uint32_t*)ADDR_TIMER_CONTROL_REGISTER);
}
...
void blocking_getchar()
{

	__asm__ __volatile__("1:\n\t");
	__asm__ __volatile__("sethi %%hi(%0), %%l1\n\t" : :     "i"(&buffer_full) : );
	__asm__ __volatile__("or    %%l1, %%lo(%0), %%l1\n\t" : :"i"(&buffer_full) : );
	__asm__ __volatile__("ldub  [ %l1 ], %l2");
	__asm__ __volatile__("cmp  %l2, 0");
	__asm__ __volatile__("be  1b");
	__asm__ __volatile__("nop ");
	__asm__ __volatile__("nop ");
	__asm__ __volatile__("stub %g0, [%l1]");
	__asm__ __volatile__("nop");
	__asm__ __volatile__("nop");
}
...
//Interrupt service routine for the serial device
void Serial_ISR()
{
...
//Initialize all registers
	...
	//Enable Timer to generate 2 second-apart interrupts
	enableTimer();
	
	sendAString("\n---Testing interrupt-based input---\n");
		
	while(1)
	{
		char c=0;
		sendAString("\n---Press a key (press 'q' to quit) ---\n");
		blocking_getchar();
		c=buffer_data;
		sendAString("\n---You entered : ");
		sendAChar(c);
...
HW_trap_0x1a: call Timer_ISR; nop;  jmpl %r17,%r0; rett %r18;\
