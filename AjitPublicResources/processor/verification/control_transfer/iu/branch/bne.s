...
_start:
	...
	!
	! instruction = BNE (Branch on Not Equal) (not Z)
	...
	cmp %l0, %l1	! not Z, less, overflow from negative to positive side
	bne label1	! conditional branch that is taken
	mov 0xF00, %o0	! delay slot instruction is always executed 
	mov 0xBAD, %o4	! control shouldn't reach here

label1:
	cmp %l1, %l0	! not Z, greater, overflow from positive to negative side
	bne label2	! conditional branch that is taken
...
label2:	
	set 0x3FFFFFFF,%l1
	cmp %l1, %l1	! Z set
	bne label3	! conditional branch that is *not* taken
...
label3: 
	...
	set 0x8fffffff,%l0
	cmp %l0, %l1	! not Z, less, overflow from negative to positive side
	bne,a label4	! conditional ANNULLED branch that is taken
	mov 0xF00, %o2	! delay slot instruction is always executed 
	mov 0xBAD, %o6	! control shouldn't reach here

label4:
	cmp %l1, %l0	! not Z, greater, overflow from positive to negative side
	bne,a label5	! conditional ANNULLED branch that is taken
	mov 0xF00, %l6	! delay slot instruction is always executed 
	mov 0xBAD, %l7	! control shouldn't reach here

label5: 
	set 0x3FFFFFFF,%l1		
	cmp %l0, %l0	! Z set
	bne,a label6	! conditional ANNULLED branch that is *not* taken
