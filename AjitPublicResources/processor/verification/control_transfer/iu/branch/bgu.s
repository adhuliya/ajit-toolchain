...
_start:
	...
	!
	! instruction = BGU - not(C or Z) (Branch on Greater Unsigned)
...
label4: 
	...
	cmp %l0, %l1
	bgu label5	! conditional branch that is *not* taken
	mov 0xF00, %i0	! delay slot instruction is always executed 
	mov 0x98, %i1	! control should reach here

label5:
	cmp %l1, %l0	
	bgu,a label6	! conditional ANNULLED branch that is *not* taken
	mov 0xF00, %i2	! delay slot instruction is *not* executed 
	mov 0x77, %i3	! control should reach here
