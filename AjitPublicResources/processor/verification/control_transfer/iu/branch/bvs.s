...
_start:
	...
	!
	! instruction = BVS (Branch on Overflow Set) (V)
	...
	cmp %l0, %l1	! overflow from negative to positive side
	bvs label1	! conditional branch that is taken
	mov 0xF00, %o0	! delay slot instruction is always executed 
	mov 0xBAD, %o4	! control shouldn't reach here

label1:
	cmp %l1, %l0	! overflow from positive to negative side
	bvs label2	! conditional branch that is taken
...
label2:
	...
	bvs label3	! conditional branch that is *not* taken
...
label3: 
	...
	set 0x8fffffff,%l0
	cmp %l0, %l1	! overflow from negative to positive side
	bvs,a label4	! conditional ANNULLED branch that is taken
	mov 0xF00, %o2	! delay slot instruction is always executed 
	mov 0xBAD, %o6	! control shouldn't reach here

label4:
	cmp %l1, %l0	! overflow from positive to negative side
	bvs,a label5	! conditional ANNULLED branch that is taken
...
label5: 
	...
	mov 0x1, %l1	
	cmp %l0, %l1	! no overflow
	bvs,a label6	! conditional ANNULLED branch that is *not* taken
