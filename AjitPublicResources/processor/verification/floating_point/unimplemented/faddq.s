...
! Initialize PSR, enable traps.
	...
	! set J, %o0		! o0 = 0x0F800000
	! ld [%o0], %fsr 	! tried by loading all Trap Enable Mask


	set A, %l0		! move  address of 4.3 into l0
	set B, %l1		! move  address of 8.2 into l1
	set C, %l2		! move  address of 4.3 into l0
	set D, %l3		! move  address of 8.2 into l1
	set E, %l4		! move  address of 4.3 into l0
	set F, %l5		! move  address of 8.2 into l1
	set G, %l6		! move  address of 4.3 into l0
	set H, %l7		! move  address of 8.2 into l1
	
	
	set 0x4, %o0
	ld [%l0], %f0		! load f0= 0xA
	ld [%l1], %f1		! load f1= 0xB
	ld [%l2], %f2		! load f2=0xC
	ld [%l3], %f3		! load f3=0xD
	ld [%l4], %f4		! load f4= 0xA
	ld [%l5], %f5		! load f5= 0xB
	ld [%l6], %f6		! load f6=0xC
	ld [%l7], %f7		! load f7=0xD
	
	faddq %f0, %f4, %f8	! Should generate an Fp_exception
	st %fsr, [%o0]		! fsr = unimplemented_FPop [0x0000C000]
	faddq %f4, %f0, %f12	! Should generat and Fp_exception
	st %fsr, [%o0 + 4]	! fsr = unimplemented_FPop [0x0000C000] 
