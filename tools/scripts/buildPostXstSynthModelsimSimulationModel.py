...
def printGhdlDoFile(vhdl_lib_file_pairs, tb_vhdl_file, do_file_name):
     ...
     # Xilinx unisim libraries to be added.
     vsim_cmd = "set XILINX_ISE_HOME $::env(XILINX_ISE_HOME)"
     print >> do_file, vsim_cmd
     vsim_cmd = "vlib unisim"
     print >> do_file, vsim_cmd
     vsim_cmd = "vmap unisim unisim"
     print >> do_file, vsim_cmd
     vsim_cmd = "vcom -work unisim $XILINX_ISE_HOME/ISE/vhdl/src/unisims/unisim_VCOMP.vhd"
     print >> do_file, vsim_cmd
     vsim_cmd = "vcom -work unisim $XILINX_ISE_HOME/ISE/vhdl/src/unisims/unisim_VPKG.vhd"
     print >> do_file, vsim_cmd
     vsim_cmd = "vcom -work unisim $XILINX_ISE_HOME/ISE/vhdl/src/unisims/primitive/*.vhd"
     print >> do_file, vsim_cmd
...
def main():
    ...
    # walk, looking for xst_synthesis directories
    for root, dirs, files in os.walk(work_area, topdown=False, followlinks=True):
        for name in dirs:
           if(name == "xst_synthesis"):
              full_name = os.path.join(root, name)
              # look for *S.vhd files in xst_synthesis directory.
              for vhdl_file in os.listdir(full_name):
                 root, ext = os.path.splitext(vhdl_file) 
                 if(ext == ".vhd") and ("S" in root):
                    logInfo("added file " + vhdl_file + " to library work") 
                    full_vhdl_file_path = os.path.join(full_name, vhdl_file)
                    ppair = ("work", os.path.abspath(full_vhdl_file_path))
