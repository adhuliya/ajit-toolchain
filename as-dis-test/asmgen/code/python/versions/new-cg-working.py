...
def make_lists_of_flat_bit_fields_in_formats (fl):
    ...
    n = len (fl)
    result = [] 
    # pp.pprint ("=== {m:s}:".format(m="Formats list is"))
    # pp.pprint (fl)
    # pp.pprint ("=== {m:s}: {v:d}".format(m="Number of Formats is:", v=n))
    # pp.pprint (fl[0])
    # pp.pprint (fl[1])
    for i in range (1,n):
        # pp.pprint ("===    {m:s} {v:d} is ".format(m="Formats list number", v=i))
        # pp.pprint (fl[i])
...
def reorganise_formats_list (fl, hdr):
    ...
    # pp.pprint ("=== {m:30s}: {v1}".
    #            format(m="Number of bit fields",
    #                   v1=num_bit_fields))
    sl = fl[fmt_bit_field_list_at:]
    # num_bit_fields_lists = len (sl) / num_bit_fields ## if (hdr == 0) else 1
    ...
    # pp.pprint ("=== {m:s}".format(m="Extracting information from"))
    # pp.pprint (sl)
    total_bits = 0
    ...
    if ((total_bits != 32) and (hdr != 0)):
        print ("ERROR: Format entry {d} has incorrect total bits: {b:d}.  The format is:".format(b=total_bits,d=hdr))
        print (fl)
...
def replace_format_specs_by_lists (il, fl):
    ...
    # pp.pprint ("=== Entered replace_format_specs_by_lists()")
    # pp.pprint ("====== {m:30s}: {v:3d} (Type: {vt})".
    #            format(m="Number of instructions", v=all_insns, vt=type(all_insns)))
    # pp.pprint ("====== {m:30s}: {v:3d} (Type: {vt})".
    #            format(m="Number of organised formats", v=all_fmts, vt=type(all_fmts)))
    # pp.pprint ("====== {m:30s}: {v:3d} (Type: {vt})".
    #            format(m="Formats field at", v=fmt_fld, vt=type(fmt_fld)))
    # pp.pprint ("====== {m:30s}: ".format(m="Instructions list"))
    # pp.pprint (il)
...
def do_given (bitfld, insn, arglst):
    ...
    # pp.pprint ("=== Bit field details")
    # pp.pprint ("====== {m:20s}: {v} (Type: {t})".format(m="Name", v=bf_name, t=type(bf_name)))
    # pp.pprint ("====== {m:20s}: {v} (Type: {t})".format(m="Start", v=bf_start, t=type(bf_start)))
    # pp.pprint ("====== {m:20s}: {v} (Type: {t})".format(m="End", v=bf_end, t=type(bf_end)))
    # pp.pprint ("====== {m:20s}: {v} (Type: {t})".format(m="Number of bits", v=bf_nbits, t=type(bf_nbits)))
    # pp.pprint ("====== {m:20s}: {v} (Type: {t})".format(m="Given Value", v=bf_given, t=type(bf_given)))
    # pp.pprint ("====== {m:20s}: {v} (Type: {t})".format(m="Value type", v=bf_val_type, t=type(bf_val_type)))

    # useval = int (bf_given, 16)
    useval = bf_given
    theval = useval
    useval = (useval << bf_end)
    # pp.pprint ("====== {m:20s}: {v:#08X} (Binary: {t:032b}, Decimal: {e})".format(m="Generated value", v=theval, t=useval, e=useval))
...
def do_generate (bitfld, insn, arglst):
    ...
    # pp.pprint ("=== {m:25s}: {v}".format (m="Generating ...", v=pp.pformat (out_lst)))
...
def analyse_args (bitfld, insn):
    ...
    # pp.pprint ("=== {m:25s}: {v}".format (m="Analysing bit field", v=bitfld))
    # pp.pprint ("=== {m:25s}: {v}".format (m="Bit field length", v=len(bitfld)))
    
    bf_name  = bitfld [bit_field_name_at]
    bf_type  = bitfld [bit_field_val_type_at]

    # print ("bbb analyse_args(): ", end="")
    # print ("INSN: {r}, Arg 1 at: {v}".
    #        format (r=pp.pformat (insn), v=insn_asm_arg_1_at))
    ...
    argnum = -1
    # print ("bbb analyse_args(): ", end="")
    # print ("Bitfield: {r}".format (r=pp.pformat (bitfld)))
    # print ("bbb analyse_args(): ", end="")
    # print ("theargs array: {r}".format (r=pp.pformat (theargs)))

    for i in range (int (num_args, 10)):
        # print ("bbb analyse_args(): name in args data: ", end="")
        # print ("Bitfield: {r}, Name: {n}".
        #        format (r=pp.pformat (bitfld), n=bf_name))
        if   (bf_name == theargs[i]):
            # print ("bbb analyse_args(): ", end="")
            # print ("Name: {r}, Args[{i}]: {v}".
            #        format (r=pp.pformat (bf_name), i=i, v=theargs[i]))
    ...
    if (argnum == -1):    
        for i in range (int (num_args, 10)):
            # print ("bbb analyse_args(): Split on = : ", end="")
            # print ("theargs [{i}] is {r}".format (i=i, r=pp.pformat (theargs[i])))
    ...
    if (argnum == -1):    
        for i in range (int (num_args, 10)):
            # print ("bbb analyse_args(): Check constants - ", end="")
            # print ("Bitfield: {r}".format (r=pp.pformat (bitfld)))
    ...
    # print ("bbb analyse_args(): ", end="")
    # print ("Returning list: {r}".format (r=pp.pformat (thelst)))
...
def expand_bit_field (bitfld, insn):
    ...
    # pp.pprint ("=== {m:25s}: {v}".format (m="Analysing bitfield", v=pp.pformat (bitfld)))
    # pp.pprint ("====== {m:25s}: {v}".format (m="Source insn", v=pp.pformat (insn)))
    ...
    bf_val_type = bitfld [bit_field_val_type_at]
    # print ("=== {m:25s}: {v}".format (m="Bit field value type", v=bf_val_type))
    ...
    # print ("aaa expand_bit_field(): ", end="")
    # print ("Bitfield: {r}".format (r=pp.pformat (bitfld)))

    # print ("aaa expand_bit_field(): ", end="")
    # print ("args_lst: {r}".format (r=pp.pformat (args_lst)))
    
    # print ("aaa expand_bit_field(): ", end="")
    # print ("retlst: {r}".format (r=pp.pformat (retlst)))
    ...
    # print ("====== argslst is: {v}".format (v=args_lst))
    # print ("========= argnum at        :{v} is {v1}".
    #        format (v=arg_detail_argnum_at, v1=args_lst [arg_detail_argnum_at]))
    # print ("========= bitfield name at :{v} is {v1}".
    #        format (v=arg_detail_bitfield_name_at, v1=args_lst [arg_detail_bitfield_name_at]))
    # print ("========= reglsttouse at   :{v} is {v1}".
    #        format (v=arg_detail_reglsttouse_at, v1=args_lst [arg_detail_reglsttouse_at]))
    # print ("========= ASM name at      :{v} is {v1}".
    #        format (v=arg_detail_asm_name_at, v1=args_lst [arg_detail_asm_name_at]))
    # print ("========= arg type at      :{v} is {v1}".
    #        format (v=arg_detail_arg_type_at, v1=args_lst [arg_detail_arg_type_at]))
    # print ("========= asm_arg_str at   :{v} is {v1}".
    #        format (v=arg_detail_asm_arg_str_at, v1=args_lst [arg_detail_asm_arg_str_at]))
    # print ("========= argval_lst at    :{v} is {v1}".
    #        format (v=arg_detail_argval_lst_at, v1=args_lst [arg_detail_argval_lst_at]))
...
def expand_insn (insn):
    ...
    for fmt in fmt_iterator:
        ...
        # print ("=== {m:25s}: {v}".format (m="Expanding Insn", v=insn)) # AMV
        # print ("=== {m:25s}: {v}".format (m="Original format", v=fmt))
        # print ("=== {m:25s}: {v}".format (m="#Original format", v=len(fmt)))
        # print ("=== {m:25s}: {v}".format (m="Original format list", v=insn [insn_fmt_index_at]))
        # print ("=== {m:25s}: {v}".format (m="#Original format list", v=len(insn [insn_fmt_index_at])))
        # print ("=== {m:25s}: {v}".format (m="Original format list[0]", v=insn [insn_fmt_index_at][0]))
        # print ("=== {m:25s}: {v}".format (m="Original bit field", v=fmt [fmt_bit_field_list_at]))
        # print ("=== {m:25s}: {v}".format (m="Original bit field", v=fmt [2]))
        # # print ("=== {m:25s}: {v}".format (m="Bit field length", v=len(bitfld)))
    
        for bitfld in bit_fields_iterator:
            thlst = expand_bit_field (bitfld, insn)
            
            # Now set up the full bit field ...
            # out_lst = [is_arg_num, bf_name, bf_val_type, out_opc_lst, reg_val_lst, theval, asmname, argtype, asmargstr, argvallst]
...
def get_argnum_str (r, arg):
    ...
    # thestrprn = "get_argnum_str(): r = {v}".format(v=r)
    # print (thestrprn)
    for l in r:
        # thestrprn = "get_argnum_str(): each l in r = {v}".format(v=l)
        # print (thestrprn)
        if (l[1] == arg):
            thestr = l[2]
            break
...
def get_argnum_val (r, arg):
    ...
    # thestrprn = "get_argnum_val(): r = {v}".format(v=r)
    # print (thestrprn)
    theval = -1
    for l in r:
        # thestrprn = "get_argnum_val(): each l in r = {v}".format(v=l)
        # print (thestrprn)
        if (l[1] == arg):
            theval = l[5]
            break
...
def get_numeric_value (strval, numval):
    if ((strval.find ("imm7")    != -1) or # immediate value
        ...
        # argstr = "0x{v:08X}({v1})".format (v=numval, v1=numval)
    else:
        argstr = ""
...
def get_specific_reg_names (inargstr, regvalofarg):
    ...
    # print ("xxx inargstr: {i}".format (i=inargstr))
    # print ("xxx regvalofarg: {i}".format (i=regvalofarg))
...
def build_list_of_opcodes (insn, l, bf, res, rec):
    ...
    # print ("ccc build_list_of_opcodes (): ")
    # print ("   {m:20s}: {v}".format (m="INSN is:",     v=pp.pformat (insn)))
    # print ("   {m:20s}: {v}".format (m="l is:",        v=pp.pformat (l)))
    # print ("   {m:20s}: {v}".format (m="Bitfield is:", v=pp.pformat (bf)))
    # print ("   {m:20s}: {v}".format (m="Res is:",      v=pp.pformat (res)))
    # print ("   {m:20s}: {v}".format (m="Rec is:",      v=pp.pformat (rec)))
    ...
    # print ("---xxxxxx")
    # print ("   {m:20s}: {v}".format (m="INSN Opcodes list at:",                v=insn_opc_lst_at))
    # print ("   {m:20s}: {v}".format (m="Values list in INSN Opcodes list at:", v=opc_val_lst_at))
    # print ("   {m:20s}: {v}".format (m="INSN Opcodes list:",                   v=pp.pformat (insn_opc_lst)))
    # print ("   {m:20s}: {v}".format (m="#INSN Opcodes list:",                  v=num_opc_field_lsts))
    # print ("---xxxxxx")

    # # theopc = 0
    # for i in range (num_opc_field_lsts):
    #     theopc = 0
    #     insn_opc    = insn_opc_lst [i]
    #     opc_val_lst = insn_opc [out_bitlst_out_opc_lst_at]
    #     num_vals    = len (opc_val_lst)

    #     is_arg_num  = insn_opc [out_bitlst_is_arg_num_at]
    #     bf_name     = insn_opc [out_bitlst_bf_name_at]
    #     bf_val_type = insn_opc [out_bitlst_bf_val_type_at]
    #     opc_lst     = insn_opc [out_bitlst_out_opc_lst_at]
    #     reg_val_lst = insn_opc [out_bitlst_reg_val_lst_at]
    #     given_val   = insn_opc [out_bitlst_given_val_at]
    #     asm_name    = insn_opc [out_bitlst_asm_name_at]
    #     arg_type    = insn_opc [out_bitlst_arg_type_at]
    #     asm_arg_str = insn_opc [out_bitlst_asm_arg_str_at]
    #     argval_lst  = insn_opc [out_bitlst_argval_lst_at]

    #     print ("      {m:25s}: [{i}] = {v}".format (m="INSN Opcode field", i=i, v=pp.pformat (insn_opc)))
    #     print ("         {m:25s}: {v}".format (m="Opcode values field",  v=pp.pformat (opc_val_lst)))
    #     print ("         {m:25s}: {v}".format (m="#Opcode field values", v=num_vals))
    #     print ("         {m:25s}: {v} ({v1})".format (m="Is an argument", v="YES" if (is_arg_num != -1) else "NO", v1=is_arg_num))
    #     # print ("         {m:25s}: {v}".format (m="is_arg_num",           v=is_arg_num))
    #     # print ("         {m:25s}: {v}".format (m="bf_name",              v=bf_name))
    #     # print ("         {m:25s}: {v}".format (m="bf_val_type",          v=bf_val_type))
    #     # print ("         {m:25s}: {v}".format (m="opc_lst",              v=opc_lst))
    #     # print ("         {m:25s}: {v}".format (m="reg_val_lst",          v=reg_val_lst))
    #     # print ("         {m:25s}: {v}".format (m="given_val",            v=given_val))
    #     # print ("         {m:25s}: {v}".format (m="asm_name",             v=asm_name))
    #     # print ("         {m:25s}: {v}".format (m="arg_type",             v=arg_type))
    #     # print ("         {m:25s}: {v}".format (m="asm_arg_str",          v=asm_arg_str))
    #     # print ("         {m:25s}: {v}".format (m="argval_lst",           v=argval_lst))
    #     # print ("      ------")

    #     for j in range (num_vals):
    #         val = opc_val_lst [j]
    #         theopc = val or theopc
    #         print ("      yyy #: {j:02d}, Val: {v:032b} ({v1:08X}), OpCode: {o:032b} ({o1:08X})".
    #                format (j=j, v=val, v1=val, o=theopc, o1=theopc))
    ...
    #         numvalofarg = -1
    #         argstr      = ""
    #         if (is_arg_num != -1):  # We are some argument in the opcode pattern
                
    #             # inargtype can be one of [rfncg]
    #             inargtype   = arg_type
    #             inargstr    = bf_name.casefold()
    #             inargtouse  = asm_arg_str.casefold()
    #             numvalofarg = opc_lst[j]
    ...
    #                 (inargtype == "a") or
    #                 False):
    #                 regvalofarg = reg_val_lst[j]
    #             else:
    #                 regvalofarg = -1
                    
    #             print ("      zzz Value #: {a}, inargtype = {j}, inargstr: {v},".
    #                    format (a=j, j=inargtype, v=inargstr), end="")
    #             print (" inargtouse: {o}, numvalofarg: {n:08X}, regvalofarg = {r}".
    #                    format (o=inargtouse, n=numvalofarg, r=regvalofarg), end="")
    ...
    #             print (" argstr: \"{s:s}\"".format (s=argstr))
    ...
    if (l == []):
        ...
        # print ("   {m:20s}: {v}".format (m="Opcode is:", v=pp.pformat (theopc)))
        # print ("   {m:20s}: {v}".format (m="Binary Opcode is:", v=pp.pformat (theopcstr)))
        # print ("   {m:20s}: {v}".format (m="Res is", v=pp.pformat (res)))
        # print ("   {m:20s}: {v}".format (m="Res format is", v=pp.pformat (res)))
        # print ("   {m:20s}: {v}".format (m="Returning list", v=pp.pformat (retvallst)))
        ...
        for i in range (numfst):
            ...
            # print ("ccc build_list_of_opcodes (): ")
            # print ("   {m:20s}: {v}".format (m="List constructed", v=pp.pformat (thisreslst)))
...
def do_final_expansion (insn):
    ...
    # print ("xxx List of opcodes is: {l}".format (l=fe))
...
def final_expanded_insns (fe):
    ...
    for the_i in fe:
        # print ("ddd final_expanded_insns (): ")
        # neat_print_expanded_insn (the_i)
        # print ("   - {m:20s}: {v}".format (m="The INSN", v=the_i))
        # print ("   - {m:20s}: {v}".format (m="The INSN", v=pp.pformat (the_i)))
        ...
        # print ("xxx ASM code: {a:s} (1:{m}, 2:{a1}, 3:{a2}, 4:{a3}, 5:{a4})".
        #            format (a=a, m=the_i[1], a1=the_i[2], a2=the_i[3], a3=the_i[4], a4=the_i[5]))
        ...
        # print ("ddd final_expanded_insns (): ")
        # print ("   - {m:20s}: {v}".format (m="The Output", v=output))
        
        expanded_insns.append (output)

    call_depth = call_depth - 1
    # print ("yyy Expanded INSN after final expansion: {e}".format (e=pp.pformat (output)))
...
def neat_print_insn (insn):
    ...
    print ("   {m:25s}: insn[{i:02d}] = {v}".
           format (m="Bit fields for expansion", i=insn_fmt_index_at, v=insn[insn_fmt_index_at]))
    # print ("   {m:25s}: insn[{i:02d}] = {v}".
...
def print_bitfield_lists (bitlists, indentstr):
    ...
    for lstnum in range (numbitlists):
        print ("{i:s}List {l:d}:".format (i=indentstr, l=lstnum))
        # print ("{i:s}List {l:d}:".format (i=indentstr, l=bitlists[lstnum]))
