...
def build_list_of_opcodes (insn, l, il, res, rec):
    ...
    if (l == []):
        ...
        for i in range (numfst):
            ...
            opsubcode = fst[i]
            if (il[rec][4] != -1): # Record "static" information
                if ((il[rec][2] == "given")   or
                    (il[rec][2] == "lookup")  or
                    (il[rec][2] == "UNUSED")):
                    otherstr = "{n} = {v}".format (n=il[rec][1], v=il[rec][4])
                    
            argnum = il[rec][0]
            if (argnum != -1):  # We are some argument in the opcode pattern
                inargstr = il[rec][1]
                if (inargstr == "rs1"): # Source register #1
                    numvalofarg = il[rec][3][i]
...
def final_expanded_insns (fe):
    ...
    for the_i in fe:
        ...
        output = "amv: {h} | {b} | {a} {v} {c}".format (h=h, b=b, a=a, v=v, c=c)
