...
#global variables
def setGlobals(ajit_project_home, ajitcc, ajit_uclibc, ajit_libgcc):
...
def buildExecutable(work_area, link_sections, obj_files, linker_script_file, elf_file, hex_dump_file,variable_dump_file,memmap_file, objdump_file, uclibc_flag):
...
def Usage():
    print "arguments:  \n\
	       ...
	       (-O objdump-file)? (name of object dump file to be generated) \n\
               ...
               (-G )? (compile with uclibc (in the buildroot) \n\
	       (-F <compiler-option>)* \n\
...
def parseOptions(opts):
    ...
    return help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings , debug_mode, opt_level, link_sections, compile_options, uclibc_flag
...
def main():
    ...
    setGlobals(ajit_project_home, ajit_project_cross_compiler,
        ajit_uclibc_install_dir, ajit_libgcc_install_dir)

    arg_list = sys.argv[1:]
    opts,args = getopt.getopt(arg_list,'W:L:C:c:S:s:I:D:E:H:V:M:O:Uhgo:T:F:')
    help_flag, work_area, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, assembly_dirs, assembly_files,  src_dirs, src_files, include_dirs, define_strings, debug_mode, opt_level,link_sections,compiler_options,uclibc_flag  = parseOptions(opts)
    ...
    if(elf_file != None):
       ret_status = buildExecutable(work_area, link_sections, obj_files, linker_script_file, elf_file, hex_file, var_file, mmap_file, objdump_file, uclibc_flag)
       if(ret_status == 1):
